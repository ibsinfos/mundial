...
class WP_Comments_List_Table extends WP_List_Table {
	...
	public function __construct($args = array()) {
		...
		if (get_option ( 'show_avatars' )) {
			add_filter ( 'comment_author', array (
					$this,
	...
	protected function get_views() {
		...
		$stati = array(
			...
			'all' => _nx_noop ( 'All <span class="count">(%s)</span>', 'All <span class="count">(%s)</span>', 'comments' ), // singular not used
		 ...
		 *        	An array of fully-formed status links. Default 'All'.
...
<?php
		...
		if ($comment->comment_parent) {
			...
			if ($parent) {
				...
				printf(
					...
					__ ( 'In reply to %s.' ), '<a href="' . $parent_link . '">' . $name . '</a>' );
			}
		}
	...
	public function column_author($comment) {
		...
		if ($this->user_can) {
			...
			if ($author_ip) {
				$author_ip_url = add_query_arg ( array (
						's' => $author_ip,
						'mode' => 'detail' 
	...
	public function column_date($comment) {
		...
		$submitted = sprintf ( __ ( '%1$s at %2$s' ),
			...
			get_comment_date ( __ ( 'Y/m/d' ), $comment ), get_comment_date ( __ ( 'g:i a' ), $comment ) );
	...
	public function column_response($comment) {
		...
		if (isset ( $this->pending_count [$post->ID] )) {
			...
			$_pending_count_temp = get_pending_comments_num ( array (
					$post->ID 
			) );
		...
		if ('attachment' === $post->post_type && ($thumb = wp_get_attachment_image ( $post->ID, array (
				80,
				60 
		), true ))) {
			echo $thumb;
