...
class WP_Press_This {
	...
	public function save_post() {
		if (empty ( $_POST ['post_ID'] ) || ! $post_id = ( int ) $_POST ['post_ID']) {
			wp_send_json_error ( array (
					'errorMessage' => __ ( 'Missing post ID.' ) 
			) );
		}
		...
		if (is_wp_error ( $updated )) {
			wp_send_json_error ( array (
					'errorMessage' => $updated->get_error_message () 
			) );
		} else {
	...
	public function add_category() {
		...
		foreach ( $names as $cat_name ) {
			...
			if (term_exists ( $cat_name, $taxonomy->name, $parent )) {
				if (count ( $names ) === 1) {
					wp_send_json_error ( array (
							'errorMessage' => __ ( 'This category already exists.' ) 
					) );
				} else {
					continue;
				}
	...
	private function _limit_embed($src) {
		...
		if (preg_match ( '!//(m|www)\.youtube\.com/(embed|v)/([^?]+)\?.+$!i', $src, $src_matches )) {
			...
			if (! $oembed->get_provider ( $src, array (
					'discover' => false 
			) )) {
				$src = '';
			}
	...
	<div class="wrapper">
		...
		<div class="press-this-actions">
			<div class="pressthis-media-buttons">
				<button type="button" class="insert-media button-link"
					data-editor="pressthis">
