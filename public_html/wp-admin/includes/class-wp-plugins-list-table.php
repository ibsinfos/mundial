...
class WP_Plugins_List_Table extends WP_List_Table {
	...
	public function prepare_items() {
		...
		if (! is_multisite () || ($screen->in_admin ( 'network' ) && current_user_can ( 'manage_network_plugins' ))) {
			 ...
			 *       
			 * @param bool $show
			 *        	Whether to show the advanced plugins for the specified
		...
		if (! $screen->in_admin ( 'network' )) {
			 ...
			 *       
			 * @param bool $show
			 *        	Whether to show network-active plugins. Default is whether the current
		...
		if (strlen ( $s )) {
			$status = 'search';
			$plugins ['search'] = array_filter ( $plugins ['all'], array (
					$this,
					'_search_callback' 
			) );
		}
		...
		if (empty ( $plugins [$status] ) && ! in_array ( $status, array (
				'all',
				'search' 
		) ))
			$status = 'all';
...
<?php
	...
	public function single_row($item) {
		...
		if ('mustuse' === $context) {
			...
			if (true === ($dropins [$plugin_file] [1])) { // Doesn't require a constant
				...
				$description = '<p><strong>' . $dropins [$plugin_file] [0] . ' <span class="error-message">' . __ ( 'Inactive:' ) . '</span></strong> ' .
					...
					sprintf ( __ ( 'Requires %1$s in %2$s file.' ), "<code>define('" . $dropins [$plugin_file] [1] . "', true);</code>", '<code>wp-config.php</code>' ) . '</p>';
			}
		...
		if ($screen->in_admin ( 'network' )) {
			 ...
			 *        	The plugin context. Defaults are 'All', 'Active',
		...
		foreach ( $columns as $column_name => $column_display_name ) {
			...
			switch ($column_name) {
				...
				case 'description' :
					 ...
					 *        	An array of the plugin's metadata,
					 ...
					 *        	An array of plugin data.
					 * @param string $status
					 *        	Status of the plugin. Defaults are 'All', 'Active',
