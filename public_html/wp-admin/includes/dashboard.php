 ...
 *        	Optional. Function that outputs controls for the widget. Default null.
 * @param array $callback_args
 *        	Optional. Data that should be set as the $args property of the widget array
...
<div id="dashboard-widgets"
	class="metabox-holder<?php echo $columns_css; ?>">
 ...
 *        	Optional. Error message. Default false.
 */
...
<form name="post"
	...
	?>
			</label> <input type="text" name="post_title" id="title"
			autocomplete="off" />
	</div>
	...
	<p class="submit">
		<input type="hidden" name="action" id="quickpost-action"
			value="post-quickdraft-save" /> <input type="hidden" name="post_ID"
 ...
 *        	Optional. Whether to display the date.
 */
...
<li id="comment-<?php echo $comment->comment_ID; ?>"
	<?php comment_class( array( 'comment-item', wp_get_comment_status( $comment ) ), $comment ); ?>>
		...
		if ($comment_post_link) {
			printf(
						...
						__ ( 'From %1$s on %2$s %3$s' ), '<cite class="comment-author">' . get_comment_author_link ( $comment ) . '</cite>', $comment_post_link, '<span class="approve">' . __ ( '[Pending]' ) . '</span>' );
		} else {
			printf(
						/* translators: 1: comment author, 2: notification if the comment is pending */
						__ ( 'From %1$s %2$s' ), '<cite class="comment-author">' . get_comment_author_link ( $comment ) . '</cite>', '<span class="approve">' . __ ( '[Pending]' ) . '</span>' );
		}
		?>
			</p>
	 ...
	 <?phpelse :
		...
		if ($comment_post_link) {
			printf(
						...
						_x ( '%1$s on %2$s %3$s', 'dashboard' ), "<strong>$type</strong>", $comment_post_link, '<span class="approve">' . __ ( '[Pending]' ) . '</span>' );
		} else {
			printf(
						/* translators: 1: type of comment, 2: notification if the comment is pending */
						_x ( '%1$s %2$s', 'dashboard' ), "<strong>$type</strong>", '<span class="approve">' . __ ( '[Pending]' ) . '</span>' );
		}
		?>
			</p>
...
function wp_dashboard_recent_posts($args) {
	...
	if ($posts->have_posts ()) {
		...
		while ( $posts->have_posts () ) {
			...
			printf ( '<li><span>%1$s</span> <a href="%2$s" aria-label="%3$s">%4$s</a></li>',
				/* translators: 1: relative date, 2: time */
				sprintf ( _x ( '%1$s, %2$s', 'dashboard' ), $relative, get_the_time () ), $recent_post_link,
				/* translators: %s: post title */
				esc_attr ( sprintf ( __ ( 'Edit &#8220;%s&#8221;' ), $draft_or_post_title ) ), $draft_or_post_title );
		}
...
function wp_dashboard_trigger_widget_control($widget_control_id = false) {
	...
	if (is_scalar ( $widget_control_id ) && $widget_control_id && isset ( $wp_dashboard_control_callbacks [$widget_control_id] ) && is_callable ( $wp_dashboard_control_callbacks [$widget_control_id] )) {
		call_user_func ( $wp_dashboard_control_callbacks [$widget_control_id], '', array (
...
function wp_dashboard_plugins_output($rss, $args = array()) {
	...
	foreach ( array (
		...
		echo '<li class="dashboard-news-plugin"><span>' . __ ( 'Popular Plugin' ) . ':</span> ' . esc_html ( $raw_title ) . '&nbsp;<a href="' . $ilink . '" class="thickbox open-plugin-details-modal" aria-label="' .
