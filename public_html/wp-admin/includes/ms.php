 ...
 *       
 * @param bool $echo
 *        	Optional. If $echo is set and the quota is exceeded, a warning message is echoed. Default is true.
...
<tr>
	...
	<td><input type="number" step="1" min="0" style="width: 100px"
		...
		value="<?php echo $quota; ?>" /> <span id="blog-upload-space-desc"><span
 ...
 *        	The user ID.
 * @param string $pref
 *        	The column in the wp_users table to update the user's status
 ...
 *       
 * @param string $code
 *        	Optional. The two-letter language code. Default empty.
...
function format_code_lang($code = '') {
	 ...
	 *        	Key/value pair of language codes where key is the short version.
	 * @param string $code
 ...
 *        	The taxonomy for `$term`. Should be 'category' or 'post_tag', as these are
...
foreach ( ( array ) $all_blogs as $blog ) {
			...
			?><option value="<?php echo $blog->userblog_id ?>"
					<?php selected( $primary_blog, $blog->userblog_id ); ?>><?php echo esc_url( get_home_url( $blog->userblog_id ) ) ?></option><?php
		}
		?>
			</select>
			<?php
...
printf(
					...
					__ ( 'What should be done with content owned by %s?' ), '<em>' . $delete_user->user_login . '</em>' );
				?></legend>
					</p>
				<?php
