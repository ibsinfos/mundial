 ...
 *        
 * @param string $scope
 *        	Optional. The tables for which to retrieve SQL. Can be all, global, ms_global, or blog tables. Defaults to all.
 * @param int $blog_id
 *        	Optional. The site ID for which to retrieve SQL. Default is the current site ID.
 ...
 *        	Optional. Whether the network is a subdomain install or a subdirectory install.
...
--The Team @ SITE_NAME' );
	...
	if (! is_multisite ()) {
		...
		$page = wp_remote_get ( 'http://' . $hostname, array (
				'timeout' => 5,
				'httpversion' => '1.1' 
		) );
		if (is_wp_error ( $page ))
		...
		if (! $vhost_ok) {
			...
			$msg .= '<p>' . sprintf(
				...
				__ ( 'The installer attempted to contact a random hostname (%s) on your domain.' ), '<code>' . $hostname . '</code>' );
			if (! empty ( $errstr )) {
			...
			$msg .= '<p>' . sprintf(
				...
				__ ( 'To use a subdomain configuration, you must have a wildcard entry in your DNS. This usually means adding a %s hostname record pointing at your web server in your DNS configuration tool.' ), '<code>*</code>' ) . '</p>';
