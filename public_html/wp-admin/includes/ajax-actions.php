...
function wp_ajax_nopriv_heartbeat() {
	 ...
	 *       
	 * @param array|object $response
	 *        	The Heartbeat response object or array.
	 * @param string $screen_id
	 *        	The screen id.
	 */
...
function wp_ajax_fetch_list() {
	...
	$wp_list_table = _get_list_table ( $list_class, array (
...
function wp_ajax_autocomplete_user() {
	...
	$include_blog_users = ($type == 'search' ? get_users ( array (
			'blog_id' => $id,
			'fields' => 'ID' 
	) ) : array ());
	$exclude_blog_users = ($type == 'add' ? get_users ( array (
			'blog_id' => $id,
			'fields' => 'ID' 
	) ) : array ());
	...
	if (! $total || ! $per_page || ! $page || ! $url) {
		...
		$x = new WP_Ajax_Response ( array (
				...
				'supplemental' => array (
						...
						'i18n_comments_text' => sprintf ( _n ( '%s Comment', '%s Comments', $counts->approved ), number_format_i18n ( $counts->approved ) ),
						'i18n_moderation_text' => sprintf ( _nx ( '%s in moderation', '%s in moderation', $counts->moderated, 'comments' ), number_format_i18n ( $counts->moderated ) ),
...
function _wp_ajax_add_hierarchical_term() {
	...
	foreach ( $names as $cat_name ) {
		...
		if (! $cat_id = term_exists ( $cat_name, $taxonomy->name, $parent ))
			$cat_id = wp_insert_term ( $cat_name, $taxonomy->name, array (
					'parent' => $parent 
			) );
...
function wp_ajax_get_comments($action) {
	...
	$wp_list_table = _get_list_table ( 'WP_Post_Comments_List_Table', array (
...
function wp_ajax_replyto_comment($action) {
	...
	if (empty ( $post->post_status ))
		wp_die ( 1 );
	elseif (in_array ( $post->post_status, array (
			'draft',
			'pending',
			'trash' 
	) ))
	...
	if (isset ( $_REQUEST ['mode'] ) && 'dashboard' == $_REQUEST ['mode']) {
		...
		if (isset ( $_REQUEST ['mode'] ) && 'single' == $_REQUEST ['mode']) {
			...
			$wp_list_table = _get_list_table ( 'WP_Comments_List_Table', array (
					'screen' => 'edit-comments' 
			) );
		}
		$wp_list_table->single_row ( $comment );
...
function wp_ajax_edit_comment() {
	...
	$wp_list_table = _get_list_table ( $checkbox ? 'WP_Comments_List_Table' : 'WP_Post_Comments_List_Table', array (
			'screen' => 'edit-comments' 
	) );
...
function wp_ajax_add_user($action) {
	...
	$x = new WP_Ajax_Response ( array (
			...
			'supplemental' => array (
				...
				__ ( 'User %s added' ), '<a href="#user-' . $user_id . '">' . $user_object->user_login . '</a>' ),
					'role' => $role 
			) 
	) );
	$x->send ();
...
function wp_ajax_menu_get_metabox() {
	...
	if (isset ( $_POST ['item-type'] ) && 'post_type' == $_POST ['item-type']) {
		...
		$items = ( array ) get_post_types ( array (
				'show_in_nav_menus' => true 
		), 'object' );
	} elseif (isset ( $_POST ['item-type'] ) && 'taxonomy' == $_POST ['item-type']) {
		$type = 'taxonomy';
		$callback = 'wp_nav_menu_item_taxonomy_meta_box';
		$items = ( array ) get_taxonomies ( array (
				'show_ui' => true 
		), 'object' );
	}
...
function wp_ajax_inline_save() {
	...
	if (! empty ( $data ['post_name'] ) && in_array ( $post ['post_status'], array (
			'draft',
			'pending' 
	) )) {
	...
	$wp_list_table->display_rows ( array (
			get_post ( $_POST ['post_ID'] ) 
	), $level );
...
function wp_ajax_save_widget() {
	...
	if (isset ( $_POST ['delete_widget'] ) && $_POST ['delete_widget']) {
		...
		$sidebar = array_diff ( $sidebar, array (
				$widget_id 
		) );
		$_POST = array (
				'sidebar' => $sidebar_id,
...
function wp_ajax_wp_remove_post_lock() {
	 ...
	 *       
	 * @param int $interval
	 *        	The interval in seconds the post lock duration
...
function wp_ajax_get_revision_diffs() {
	...
	if (! $revisions = wp_get_post_revisions ( $post->ID, array (
			'check_enabled' => false 
	) ))
		wp_send_json_error ();
...
function wp_ajax_crop_image() {
	...
	if (! $cropped || is_wp_error ( $cropped )) {
		wp_send_json_error ( array (
				'message' => __ ( 'Image could not be processed.' ) 
		) );
	}
...
function wp_ajax_update_theme() {
	...
	$skin = new WP_Ajax_Upgrader_Skin ();
	$upgrader = new Theme_Upgrader ( $skin );
	$result = $upgrader->bulk_upgrade ( array (
			$stylesheet 
	) );
