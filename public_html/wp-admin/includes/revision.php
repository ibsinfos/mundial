...
function wp_get_revision_ui_diff($post, $compare_from, $compare_to) {
	...
	foreach ( _wp_post_revision_fields ( $post ) as $field => $name ) {
		 ...
		 * @since 3.6.0
		 *       
		 * @param string $compare_from->$field
		 ...
		 * @param
		 *        	string null The context of whether the current revision is the old
 ...
 * @param int $from
 *        	Optional. The revision ID to compare from.
 *        	
...
function wp_prepare_revisions_for_js($post, $selected_revision_id, $from = null) {
	...
	foreach ( $revisions as $revision ) {
		...
		if ($can_restore) {
			$restore_link = str_replace ( '&amp;', '&', wp_nonce_url ( add_query_arg ( array (
