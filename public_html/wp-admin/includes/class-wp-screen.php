...
final class WP_Screen {
	 ...
	 *        	Optional. The hook name (also known as the hook suffix) used to determine the screen.
	...
	public function get_help_tabs() {
		...
		foreach ( $help_tabs as $help_tab ) {
			if (isset ( $priorities [$help_tab ['priority']] )) {
				...
				$priorities [$help_tab ['priority']] = array (
						$help_tab 
				);
			}
		}
	...
	public function render_screen_meta() {
		 ...
		 *            
		 * @param array $old_compat_help
		 *        	Old contextual help.
		 * @param WP_Screen $this
	...
	<div id="contextual-help-wrap"
		...
		foreach ( $this->get_help_tabs () as $tab ) :
			...
			?>
						...
						<?php echo $class; ?>><a
						href="<?php echo esc_url( "#$panel_id" ); ?>"
		...
		foreach ( $this->get_help_tabs () as $tab ) :
			...
			if (! empty ( $tab ['callback'] ))
				call_user_func_array ( $tab ['callback'], array (
						$this,
						$tab 
				) );
			?>
		...
		if (! empty ( $columns ) && isset ( $columns [$this->id] ))
			$this->add_option ( 'layout_columns', array (
					'max' => $columns [$this->id] 
			) );
...
<div id="screen-meta-links">
		...
		<button type="button" id="contextual-help-link"
			class="button show-settings" aria-controls="contextual-help-wrap"
		...
		class="hide-if-no-js screen-meta-toggle">
		<button type="button" id="show-settings-link"
			class="button show-settings" aria-controls="screen-options-wrap"
...
<?php
	...
	public function render_view_mode() {
		...
		$view_mode_post_types = get_post_types ( array (
				'hierarchical' => false,
				'show_ui' => true 
		) );
		 ...
		 * @since 4.4.0
		 *       
		 * @param array $view_mode_post_types
...
<fieldset class="metabox-prefs view-mode">
	...
	<label for="list-view-mode"> <input id="list-view-mode" type="radio"
		name="mode" value="list" <?php checked( 'list', $mode ); ?> />
					<?php _e( 'List View' ); ?>
				</label> <label for="excerpt-view-mode"> <input
		id="excerpt-view-mode" type="radio" name="mode" value="excerpt"
		<?php checked( 'excerpt', $mode ); ?> />
					<?php _e( 'Excerpt View' ); ?>
				</label>
</fieldset>
<?php
	 ...
	 * @param string $tag
	 *        	Optional. The HTML tag to wrap the screen reader text. Default h2.
	 */
