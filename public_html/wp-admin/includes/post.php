 ...
 *        	Optional. A post type string. Default 'post'.
 * @param bool $create_in_db
 *        	Optional. Whether to insert the post into database. Default false.
...
function wp_edit_posts_query($q = false) {
	...
	if (isset ( $q ['orderby'] )) {
		$orderby = $q ['orderby'];
	} elseif (isset ( $q ['post_status'] ) && in_array ( $q ['post_status'], array (
			'pending',
			'draft' 
	) )) {
		$orderby = 'modified';
	 ...
	 *        	Number of posts to display per page for the given post
	 *        	type. Default 20.
	 */
 ...
 *       
 * @param array|false $q
 *        	Optional. Array of query variables to use to build the query or false
...
function postbox_classes($id, $page) {
	if (isset ( $_GET ['edit'] ) && $_GET ['edit'] == $id) {
		...
		if (! is_array ( $closed )) {
			...
			$classes = in_array ( $id, $closed ) ? array (
					'closed' 
			) : array (
					'' 
			);
 ...
 *        	Post ID or post object.
 * @param string $title
 *        	Optional. Title to override the post's current title when generating the post name. Default null.
 * @param string $name
 *        	Optional. Name to override the post name. Default null.
 ...
 *        	Optional. New title. Default null.
 * @param string $new_slug
 *        	Optional. New slug. Default null.
...
function _wp_post_thumbnail_html($thumbnail_id = null, $post = null) {
	...
	$content = sprintf ( $set_thumbnail_link, esc_url ( $upload_iframe_src ), '', // Empty when there's no featured image set, `aria-describedby` attribute otherwise.
esc_html ( $post_type_object->labels->set_featured_image ) );
	
	if ($thumbnail_id && get_post ( $thumbnail_id )) {
		$size = isset ( $_wp_additional_image_sizes ['post-thumbnail'] ) ? 'post-thumbnail' : array (
				266,
				266 
		);
		 ...
		 *        	Post thumbnail image size to display in the meta box. Accepts any valid
...
function _admin_notice_post_locked() {
	...
	if ($user) {
		 ...
		 *        	WP_User object on success, false otherwise.
		 */
		...
		<div class="post-taken-over">
			...
			<p class="wp-tab-first" tabindex="0">
				<span class="currently-editing"></span><br /> <span
					class="locked-saving hidden"><img
					src="<?php echo esc_url( admin_url( 'images/spinner-2x.gif' ) ); ?>"
					width="16" height="16" alt="" /> <?php _e( 'Saving revision&hellip;' ); ?></span>
