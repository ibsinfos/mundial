...
function wp_nav_menu_setup() {
	...
	if (false === get_user_option ( 'managenav-menuscolumnshidden' )) {
		...
		update_user_option ( $user->ID, 'managenav-menuscolumnshidden', array (
				0 => 'link-target',
				1 => 'css-classes',
				2 => 'xfn',
				3 => 'description',
...
function wp_nav_menu_post_type_meta_boxes() {
	...
	foreach ( $post_types as $post_type ) {
		 ...
		 * @since 3.0.0
		 *       
		 * @param object $meta_box_object
		 *        	The current object to add a menu items
		 *        	meta box for.
		 */
...
<div id="posttype-<?php echo $post_type_name; ?>" class="posttypediv">
	...
	?>
			...
			<label for="quick-search-posttype-<?php echo $post_type_name; ?>"
				class="screen-reader-text"><?php _e( 'Search' ); ?></label> <input
				type="search" class="quick-search" value="<?php echo $searched; ?>"
				name="quick-search-posttype-<?php echo $post_type_name; ?>"
				id="quick-search-posttype-<?php echo $post_type_name; ?>" /> <span
				class="spinner"></span>
	...
	?>#posttype-<?php echo $post_type_name; ?>"
		...
		</span> <span class="add-to-menu"> <input type="submit"
			<?php wp_nav_menu_disabled_check( $nav_menu_selected_id ); ?>
			class="button submit-add-to-menu right"
			value="<?php esc_attr_e( 'Add to Menu' ); ?>"
...
<div id="taxonomy-<?php echo $taxonomy_name; ?>" class="taxonomydiv">
	...
	?>
			...
			<label for="quick-search-taxonomy-<?php echo $taxonomy_name; ?>"
				...
				id="quick-search-taxonomy-<?php echo $taxonomy_name; ?>" /> <span
				class="spinner"></span>
...
function wp_save_nav_menu_items($menu_id = 0, $menu_data = array()) {
	...
	if (0 == $menu_id || is_nav_menu ( $menu_id )) {
		...
		foreach ( ( array ) $menu_data as $_possible_db_id => $_item_object_data ) {
			...
			in_array ( $_item_object_data ['menu-item-url'], array (
					'http://',
					'' 
	...
	if (is_nav_menu ( $menu )) {
		...
		if (class_exists ( $walker_class_name )) {
			...
			return new WP_Error ( 'menu_walker_not_exist',
				...
				sprintf ( __ ( 'The Walker class named %s does not exist.' ), '<strong>' . $walker_class_name . '</strong>' ) );
		}
		...
		$result .= walk_nav_menu_tree ( array_map ( 'wp_setup_nav_menu_item', $menu_items ), 0, ( object ) array (
				'walker' => $walker 
		) );
		$result .= ' </ul> ';
		return $result;
...
function wp_nav_menu_update_menu_items($nav_menu_selected_id, $nav_menu_selected_title) {
	$unsorted_menu_items = wp_get_nav_menu_items ( $nav_menu_selected_id, array (
			'orderby' => 'ID',
			'output' => ARRAY_A,
			'output_key' => 'ID',
	...
	$nav_menu_option ['auto_add'] = array_intersect ( $nav_menu_option ['auto_add'], wp_get_nav_menus ( array (
			'fields' => 'ids' 
	) ) );
	...
	$messages [] = '<div id="message" class="updated notice is-dismissible"><p>' .
		...
		sprintf ( __ ( '%s has been updated.' ), '<strong>' . $nav_menu_selected_title . '</strong>' ) . '</p></div>';
 ...
 * it into `$_POST` to avoid PHP `max_input_vars` limitations.
 * See #14134.
 *
 * @ignore
 *
