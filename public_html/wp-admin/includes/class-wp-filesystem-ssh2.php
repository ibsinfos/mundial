...
class WP_Filesystem_SSH2 extends WP_Filesystem_Base {
	...
	public function __construct($opt = '') {
		...
		if (! function_exists ( 'stream_get_contents' )) {
			$this->errors->add ( 'ssh2_php_requirement', sprintf(
					/* translators: %s: stream_get_contents() */
					__ ( 'The ssh2 PHP extension is available, however, we require the PHP5 function %s' ), '<code>stream_get_contents()</code>' ) );
			return;
		}
	...
	public function connect() {
		...
		if (! $this->link) {
			$this->errors->add ( 'connect',
				...
				sprintf ( __ ( 'Failed to connect to SSH2 Server %s' ), $this->options ['hostname'] . ':' . $this->options ['port'] ) );
			return false;
		}
		...
		if (! $this->keys) {
			if (! @ssh2_auth_password ( $this->link, $this->options ['username'], $this->options ['password'] )) {
				$this->errors->add ( 'auth',
					...
					sprintf ( __ ( 'Username/Password incorrect for %s' ), $this->options ['username'] ) );
				return false;
			}
		} else {
			if (! @ssh2_auth_pubkey_file ( $this->link, $this->options ['username'], $this->options ['public_key'], $this->options ['private_key'], $this->options ['password'] )) {
				$this->errors->add ( 'auth',
					/* translators: %s: username */
					sprintf ( __ ( 'Public and Private keys incorrect for %s' ), $this->options ['username'] ) );
				return false;
			}
		}
		...
		if (! $this->sftp_link) {
			$this->errors->add ( 'connect',
				/* translators: %s: hostname:port */
				sprintf ( __ ( 'Failed to initialize a SFTP subsystem session with the SSH2 Server %s' ), $this->options ['hostname'] . ':' . $this->options ['port'] ) );
			return false;
		}
	...
	public function run_command($command, $returnbool = false) {
		...
		if (! ($stream = ssh2_exec ( $this->link, $command ))) {
			$this->errors->add ( 'command',
				...
				sprintf ( __ ( 'Unable to perform command: %s' ), $command ) );
		} else {
	 ...
	 * @param bool $recursive
	 *        	Optional. If set True changes file owner recursivly. Default False.
