...
if (! function_exists ( 'wp_install' )) :
	 ...
	 *        	Optional. Language chosen. Default empty.
...
if (! function_exists ( 'wp_upgrade' )) :
	...
	function wp_upgrade() {
		 ...
		 * @since 3.9.0
		 *       
		 * @param int $wp_db_version
		 *        	The new $wp_db_version.
		 * @param int $wp_current_db_version
...
function upgrade_100() {
	...
	foreach ( $categories as $category ) {
		if ('' == $category->category_nicename) {
			...
			$wpdb->update ( $wpdb->categories, array (
	...
	if ($allposts) :
		foreach ( $allposts as $post ) {
			...
			if (! $cat && 0 != $post->post_category) { // If there's no result
				$wpdb->insert ( $wpdb->post2cat, array (
						'post_id' => $post->ID,
...
function upgrade_130() {
	...
	if ($posts) {
		foreach ( $posts as $post ) {
			...
			$wpdb->update ( $wpdb->posts, compact ( 'post_title', 'post_content', 'post_excerpt', 'guid' ), array (
					'ID' => $post->ID 
			) );
		}
	}
	...
	if ($comments) {
		foreach ( $comments as $comment ) {
			...
			$wpdb->update ( $wpdb->comments, compact ( 'comment_content', 'comment_author' ), array (
	...
	if ($links) {
		foreach ( $links as $link ) {
			...
			$wpdb->update ( $wpdb->links, compact ( 'link_name', 'link_description' ), array (
...
function upgrade_160() {
	...
	foreach ( $users as $user ) :
		...
		if (empty ( $caps ) || defined ( 'RESET_CAPS' )) {
			...
			update_user_meta ( $user->ID, $wpdb->prefix . 'capabilities', array (
					$role => true 
			) );
		}
	endforeach
...
function upgrade_210() {
	...
	if ($wp_current_db_version < 3531) {
		...
		if (! empty ( $posts ))
			foreach ( $posts as $post )
				wp_schedule_single_event ( mysql2date ( 'U', $post->post_date, false ), 'publish_future_post', array (
						$post->ID 
				) );
	}
}
 ...
 *        	Optional. The query to run. Can be multiple queries
 *        	in an array, or a string of queries separated by
 *        	semicolons. Default empty.
 * @param bool $execute
 *        	Optional. Whether or not to execute the query right away.
 *        	Default true.
		...
		foreach ( $flds as $fld ) {
			...
			switch ($fieldname_lowercased) {
				...
				case 'spatial' :
					...
					preg_match ( '/^' . '(?P<index_type>' . // 1) Type of the index.
...
'$/im', $fld, $index_matches );
					...
					foreach ( $index_columns as &$index_column ) {
						...
						preg_match ( '/' . '`?' . // Name can be escaped with a backtick.
