...
function allow_subdirectory_install() {
	 ...
	 *       
	 * @param bool $allow
	 *        	Whether to enable the subdirectory install feature in Multisite. Default is false.
	 */
...
function network_step1($errors = false) {
	...
	if ((false !== $has_ports && ! in_array ( $has_ports, array (
		...
		echo '<p>' . sprintf(
			...
			__ ( 'You cannot use port numbers such as %s.' ), '<code>' . $has_ports . '</code>' ) . '</p>';
...
<table class="form-table">
	<tr>
		<th><label><input type="radio" name="subdomain_install" value="1"
				<?php checked( $subdomain_install ); ?> /> <?php _e( 'Sub-domains' ); ?></label></th>
		<td><?php
		
printf(
					/* translators: 1: hostname */
					_x ( 'like <code>site1.%1$s</code> and <code>site2.%1$s</code>', 'subdomain examples' ), $hostname );
		?></td>
	</tr>
	<tr>
		<th><label><input type="radio" name="subdomain_install" value="0"
				<?php checked( ! $subdomain_install ); ?> /> <?php _e( 'Sub-directories' ); ?></label></th>
		<td><?php
		
printf(
					/* translators: 1: hostname */
					_x ( 'like <code>%1$s/site1</code> and <code>%1$s/site2</code>', 'subdirectory examples' ), $hostname );
		?></td>
	</tr>
</table>
...
printf(
			...
			__ ( 'We recommend you change your siteurl to %1$s before enabling the network feature. It will still be possible to visit your site using the %3$s prefix with an address like %2$s but any links will not have the %3$s prefix.' ), '<code>' . substr ( $hostname, 4 ) . '</code>', '<code>' . $hostname . '</code>', '<code>www</code>' );
		?></p>
<table class="form-table">
	<tr>
...
printf(
						...
						__ ( 'The internet address of your network will be %s.' ), '<code>' . $hostname . '</code>' );
		?>
				</td>
	</tr>
</table>
...
<table class="form-table">
		...
		printf(
						...
						__ ( 'Because you are using %1$s, the sites in your WordPress network must use sub-directories. Consider using %2$s if you wish to use sub-domains.' ), '<code>localhost</code>', '<code>localhost.localdomain</code>' );
		// Uh oh:
...
<div class="error">
	<p>
		<strong><?php _e('Warning:'); ?></strong> <?php _e( 'An existing WordPress network was detected.' ); ?></p>
</div>
	...
	<p><?php
		if (file_exists ( $home_path . '.htaccess' )) {
			...
			printf(
					...
					__ ( 'We recommend you back up your existing %1$s and %2$s files.' ), '<code>wp-config.php</code>', '<code>.htaccess</code>' );
		} elseif (file_exists ( $home_path . 'web.config' )) {
			echo '<strong>' . __ ( 'Caution:' ) . '</strong> ';
			printf(
					/* translators: 1: wp-config.php 2: web.config */
					__ ( 'We recommend you back up your existing %1$s and %2$s files.' ), '<code>wp-config.php</code>', '<code>web.config</code>' );
		} else {
			echo '<strong>' . __ ( 'Caution:' ) . '</strong> ';
			printf(
					/* translators: 1: wp-config.php */
					__ ( 'We recommend you back up your existing %s file.' ), '<code>wp-config.php</code>' );
		}
		?></p>
</div>
<?php
...
printf(
				...
				__ ( 'Add the following to your %1$s file in %2$s <strong>above</strong> the line reading %3$s:' ), '<code>wp-config.php</code>', '<code>' . $location_of_wp_config . '</code>',
				/*
...
<?php
	...
	<p>
		...
		if (1 == $num_keys_salts) {
			printf(
					/* translators: 1: wp-config.php */
					__ ( 'This unique authentication key is also missing from your %s file.' ), '<code>wp-config.php</code>' );
		} else {
			printf(
					/* translators: 1: wp-config.php */
					__ ( 'These unique authentication keys are also missing from your %s file.' ), '<code>wp-config.php</code>' );
		}
		?>
...
';
		...
		printf(
			...
			__ ( 'Add the following to your %1$s file in %2$s, <strong>replacing</strong> other WordPress rules:' ), '<code>web.config</code>', '<code>' . $home_path . '</code>' );
		echo '</p>';
...
EOF;
		...
		printf(
			/* translators: 1: a filename like .htaccess. 2: a file path. */
			__ ( 'Add the following to your %1$s file in %2$s, <strong>replacing</strong> other WordPress rules:' ), '<code>.htaccess</code>', '<code>' . $home_path . '</code>' );
		echo '</p>';
		if (! $subdomain_install && WP_CONTENT_DIR != ABSPATH . 'wp-content')
			echo '<p><strong>' . __ ( 'Warning:' ) . ' ' . __ ( 'Subdirectory networks may not be fully compatible with custom wp-content directories.' ) . '</strong></p>';
		?>
...
<?php echo esc_textarea( $htaccess_file ); ?></textarea>
</li>
</ol>
	...
	if (! is_multisite ()) {
		?>
<p><?php _e( 'Once you complete these steps, your network is enabled and configured. You will have to log in again.' ); ?> <a
