 ...
 * @param string $url
 *        	Optional. Image src URL. Default empty.
 * @param bool|string $rel
 *        	Optional. Value for rel attribute or whether to add a default value. Default false.
 * @param string|array $size
 *        	Optional. Image size. Accepts any valid image size, or an array of width
 *        	and height values in pixels (in that order). Default 'medium'.
 * @param string $alt
 *        	Optional. Image alt attribute. Default empty.
...
function image_add_caption($html, $id, $caption, $title, $align, $url, $size, $alt = '') {
	 ...
	 *       
	 * @param bool $bool
	 *        	Whether to disable appending captions. Returning true to the filter
 ...
 * @param int $post_id
 *        	The post ID of a post to attach the media item to. Required, but can
...
function wp_media_upload_handler() {
	...
	if (! empty ( $_POST ['insertonlybutton'] )) {
		...
		if (isset ( $_POST ['media_type'] ) && 'image' != $_POST ['media_type']) {
			 ...
			 * @param string $align
			 *        	The image alignment. Default 'alignnone'. Possible values include
...
function image_size_input_fields($post, $check = '') {
	 ...
	 *        	Array of image sizes and their names. Default values
...
function get_media_items($post_id, $errors) {
	...
	foreach ( ( array ) $attachments as $id => $attachment ) {
		if ($attachment->post_status == 'trash')
			continue;
		if ($item = get_media_item ( $id, array (
...
function get_media_item($attachment_id, $args = null) {
	...
	$image_edit_button = '';
	if (wp_attachment_is_image ( $post->ID ) && wp_image_editor_supports ( array (
	...
	if (($r ['send'] || $thumbnail || $delete) && ! isset ( $form_fields ['buttons'] )) {
		$form_fields ['buttons'] = array (
				'tr' => "\t\t<tr class='submit'><td></td><td class='savesend'>" . $r ['send'] . " $thumbnail $delete</td></tr>\n" 
		);
	}
	$hidden_fields = array ();
...
<?php
	 ...
	 *        	An array of default settings used by Plupload.
	 */
...
<div id="sort-buttons" class="hide-if-no-js">
	<span>
<?php _e('All Tabs:'); ?>
<a href="#" id="showall"><?php _e('Show'); ?></a> <a href="#"
		id="hideall" style="display: none;"><?php _e('Hide'); ?></a>
	</span>
<?php _e('Sort Order:'); ?>
<a href="#" id="asc"><?php _e('Ascending'); ?></a> | <a href="#"
		id="desc"><?php _e('Descending'); ?></a> | <a href="#" id="clear"><?php _ex('Clear', 'verb'); ?></a>
</div>
<form enctype="multipart/form-data" method="post"
	action="<?php echo esc_url( $form_action_url ); ?>"
...
<input type="hidden" name="post_id" id="post_id"
	...
	<div id="gallery-settings" style="display: none;">
		...
		<table id="basic" class="describe">
			<tbody>
				<tr>
					<th scope="row" class="label"><label> <span class="alignleft"><?php _e('Link thumbnails to:'); ?></span>
					</label></th>
					<td class="field"><input type="radio" name="linkto"
						...
						value="post" /> <label for="linkto-post" class="radio"><?php _e('Attachment Page'); ?></label>
					</td>
				</tr>

				<tr>
					<th scope="row" class="label"><label> <span class="alignleft"><?php _e('Order images by:'); ?></span>
					</label></th>
				...
				<tr>
					<th scope="row" class="label"><label> <span class="alignleft"><?php _e('Order:'); ?></span>
					</label></th>
				...
				<tr>
					<th scope="row" class="label"><label> <span class="alignleft"><?php _e('Gallery columns:'); ?></span>
					</label></th>
...
function edit_form_image_editor($post) {
	...
	if ($attachment_id = intval ( $post->ID ))
		$thumb_url = wp_get_attachment_image_src ( $attachment_id, array (
				900,
				450 
		), true );
...
<div class="wp_attachment_holder wp-clearfix">
	...
	<div <?php if ( $open ) echo ' style="display:none"'; ?>
		class="wp_attachment_image wp-clearfix"
...
<div class="wp_attachment_details edit-form-section">
	...
	<?php if ( 'image' === substr( $post->post_mime_type, 0, 5 ) ) : ?>
		...
		<input type="text" class="widefat" name="_wp_attachment_image_alt"
			id="attachment_alt" value="<?php echo esc_attr( $alt_text ); ?>" />
	</p>
	<?php endif; ?>
