...
class Core_Upgrader extends WP_Upgrader {
	...
	public function upgrade($current, $args = array()) {
		...
		if ($parsed_args ['attempt_rollback'] && $current->packages->rollback && ! $parsed_args ['do_rollback']) {
			...
			if ($try_rollback) {
				...
				$rollback_result = $this->upgrade ( $current, array_merge ( $parsed_args, array (
						'do_rollback' => true 
				) ) );
				
				$original_result = $result;
				$result = new WP_Error ( 'rollback_was_required', $this->strings ['rollback_was_required'], ( object ) array (
	...
	public static function should_update_to_version($offered_ver) {
		...
		if ($current_is_development_version) {
			 ...
			 * @since 3.7.0
			 *       
			 * @param bool $upgrade_dev
			 *        	Whether to enable automatic updates for
