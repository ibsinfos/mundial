...
<div id="message" class="updated">
	...
	<p>
			<a
				href="<?php echo esc_url( wp_validate_redirect( esc_url_raw( $wp_http_referer ), admin_url( 'term.php?taxonomy=' . $taxonomy ) ) ); ?>"><?php
...
<div id="ajax-response"></div>
	...
	<form name="edittag" id="edittag" method="post" action="edit-tags.php"
		...
		<input type="hidden" name="action" value="editedtag" /> <input
			type="hidden" name="tag_ID" value="<?php echo esc_attr( $tag_ID ) ?>" />
...
<?php if ( !global_terms_enabled() ) { ?>
	 ...
	 *       
	 * @param string $slug
	 *        	The editable slug. Will be either a term slug or post URI depending
	 *        	upon the context in which it is evaluated.
	 * @param object|WP_Post $tag
	 *        	Term or WP_Post object.
	 */
	$slug = isset ( $tag->slug ) ? apply_filters ( 'editable_slug', $tag->slug, $tag ) : '';
	?>
			<td><input name="slug" id="slug" type="text"
					value="<?php echo esc_attr( $slug ); ?>" size="40" />
