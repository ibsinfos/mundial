...
if (isset ( $_REQUEST ['action'] ) && 'update-site' == $_REQUEST ['action']) {
	...
	foreach ( $blog_data_checkboxes as $c ) {
		if (! in_array ( $existing_details->$c, array (
				0,
				1 
		) )) {
...
<form method="post" action="site-info.php?action=update-site">
	...
	<input type="hidden" name="id" value="<?php echo esc_attr( $id ) ?>" />
		...
		<tr class="form-field">
		 ...
		 <?php		// For any other site, the scheme, domain, and path can all be changed.
		else :
			?>
		<tr class="form-field form-required">
				<th scope="row"><?php _e( 'Site Address (URL)' ); ?></th>
				<td><input name="blog[url]" type="text" id="url"
					value="<?php echo $parsed_scheme . '://' . esc_attr( $details->domain ) . esc_attr( $details->path ); ?>" /></td>
			</tr>
		<?php endif; ?>
