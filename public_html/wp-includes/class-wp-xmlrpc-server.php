...
class wp_xmlrpc_server extends IXR_Server {
	 ...
	 *        	Sanitize single string or array of strings.
	 * @param int $count
	...
	protected function _prepare_media_item($media_item, $thumbnail_size = 'thumbnail') {
		 ...
		 * @since 3.4.0
		 *       
		 * @param array $_media_item
		 *        	An array of media item data.
		 * @param object $media_item
		 *        	Media item object.
		 * @param string $thumbnail_size
		 *        	Image size.
			...
			if (isset ( $post_data ['terms_names'] ) && is_array ( $post_data ['terms_names'] )) {
				...
				foreach ( $taxonomies as $taxonomy ) {
					...
					if (is_taxonomy_hierarchical ( $taxonomy )) {
						$tax_term_names = get_terms ( $taxonomy, array (
								'fields' => 'names',
								'hide_empty' => false 
						) );
						...
						$ambiguous_tax_term_counts = array_filter ( $tax_term_names_count, array (
								$this,
	...
	public function wp_getPost($args) {
		...
		if (isset ( $args [4] )) {
			 ...
			 *        	Method name.
			 */
			$fields = apply_filters ( 'xmlrpc_default_post_fields', array (
					'post',
					'terms',
					'custom_fields' 
			), 'wp.getPost' );
	...
	public function wp_getTaxonomy($args) {
		...
		if (isset ( $args [4] )) {
			 ...
			 *       
			 * @param array $fields
			 *        	An array of taxonomy fields to retrieve.
			 * @param string $method
			 *        	The method name.
			 */
			$fields = apply_filters ( 'xmlrpc_default_taxonomy_fields', array (
					'labels',
					'cap',
					'object_type' 
			), 'wp.getTaxonomy' );
	...
	public function wp_getUser($args) {
		...
		if (isset ( $args [4] )) {
			 ...
			 *       
			 * @param array $fields
			 *        	User query fields for given method. Default 'all'.
			 * @param string $method
			 *        	The method name.
			 */
			$fields = apply_filters ( 'xmlrpc_default_user_fields', array (
					'all' 
			), 'wp.getUser' );
		}
	...
	public function wp_deleteCategory($args) {
		...
		if (true == $status) {
			 ...
			 * @param array $args
			 *        	An array of arguments to delete the category.
			 */
	...
	public function wp_getPostType($args) {
		...
		if (isset ( $args [4] )) {
			 ...
			 *        	The method name.
			 */
			$fields = apply_filters ( 'xmlrpc_default_posttype_fields', array (
					'labels',
					'cap',
					'taxonomies' 
			), 'wp.getPostType' );
	...
	public function wp_getRevisions($args) {
		...
		if (isset ( $args [4] )) {
			 ...
			 *       
			 * @param array $field
			 *        	An array of revision query fields.
			 * @param string $method
			 *        	The method name.
			 */
			$fields = apply_filters ( 'xmlrpc_default_revision_fields', array (
					'post_date',
					'post_date_gmt' 
			), 'wp.getRevisions' );
		}
	...
	public function mw_editPost($args) {
		...
		if (! in_array ( $postdata ['post_type'], array (
				'post',
				'page' 
		) ))
			return new IXR_Error ( 401, __ ( 'Invalid post type.' ) );
	...
	public function pingback_ping($args) {
		...
		$pos1 = strpos ( $pagelinkedto, str_replace ( array (
				'http://www.',
				'http://',
				'https://www.',
				'https://' 
