 ...
 * @param string $output
 *        	Optional. The required return type. One of OBJECT, ARRAY_A, or ARRAY_N, which correspond to
 ...
 *        	Post field name.
 * @param int|WP_Post $post
 *        	Optional. Post ID or post object. Defaults to current post.
 * @param string $context
 *        	Optional. How to filter the field. Accepts 'raw', 'edit', 'db',
 ...
 *        	Optional. Post ID or post object. Default empty.
 ...
 *
 * @param array|string $args
 *        	Optional. Array or string of post status arguments to compare against
 *        	properties of the global `$wp_post_statuses objects`. Default empty array.
 * @param string $output
 *        	Optional. The type of output to return, either 'names' or 'objects'. Default 'names'.
 * @param string $operator
 *        	Optional. The logical operation to perform. 'or' means only one element
 ...
 * @param string $output
 *        	Optional. The type of output to return. Accepts post type 'names'
 *        	or 'objects'. Default 'names'.
 * @param string $operator
 *        	Optional. The logical operation to perform. 'or' means only one
 ...
 *        	The feature being added, accepts an array of
 ...
 * @param string $operator
 *        	Optional. The logical operation to perform. 'or' means
 ...
 * @param string $post_type
 *        	Optional. Post type. Accepts 'post' or 'page' to
 ...
 *        	Metadata name.
 * @param mixed $meta_value
 *        	Optional. Metadata value. Must be serializable if
 ...
 *        	Post ID.
 * @param string $key
 *        	Optional. The meta key to retrieve. By default, returns
 ...
 * @param string $context
 *        	Optional. How to sanitize post fields.
...
function sanitize_post_field($field, $value, $post_id, $context = 'display') {
	...
	if ('edit' == $context) {
		...
		if ($prefixed) {
			 ...
			 *        	Post ID.
			 * @param string $context
			 *        	Context for how to sanitize the field. Possible
...
function wp_count_posts($type = 'post', $perm = '') {
	 ...
	 *       
	 * @param object $counts
	 *        	An object containing the current post_type's post
	 *        	counts by status.
	 * @param string $type
	 *        	Post type.
	 * @param string $perm
 ...
 *        	Optional. Array or comma-separated list of
...
function wp_count_attachments($mime_type = '') {
	 ...
	 *       
	 * @param object $counts
	 *        	An object containing the attachment counts by
	 *        	mime type.
	 * @param string $mime_type
	 *        	The mime type pattern used to filter the attachments
	 *        	counted.
	 */
 ...
 *        	Mime types, e.g. audio/mpeg or image (same as image/*)
 *        	or flash (same as *flash*).
 * @param string|array $real_mime_types
 ...
 * @since 2.5.0
 *       
 * @param string|array $post_mime_types
 *        	List of mime types or comma separated string
 *        	of mime types.
 * @param string $table_alias
 *        	Optional. Specify a table alias, if needed.
 *        	Default empty.
...
function wp_delete_post($postid = 0, $force_delete = false) {
	...
	if (is_post_type_hierarchical ( $post->post_type )) {
		...
		if ($children) {
			$wpdb->update ( $wpdb->posts, $parent_data, $parent_where + array (
	...
	$result = $wpdb->delete ( $wpdb->posts, array (
			'ID' => $postid 
	) );
	if (! $result) {
		return false;
	...
	wp_clear_scheduled_hook ( 'publish_future_post', array (
			$postid 
	) );
...
function wp_trash_post_comments($post = null) {
	...
	$result = $wpdb->update ( $wpdb->comments, array (
 ...
 *
 * @param int $post_id
 *        	Optional. The Post ID. Does not default to the ID of the
 *        	global $post. Default 0.
 * @param array $args
 *        	Optional. Category arguments. See wp_get_object_terms(). Default empty.
 ...
 * @param array $args
 *        	Optional. wp_get_object_terms() arguments. Default empty array.
 ...
 * @param string $output
 *        	Optional. The required return type. One of OBJECT or ARRAY_A, which correspond to
...
function wp_insert_post($postarr, $wp_error = false) {
	...
	if ('attachment' === $post_type && ! in_array ( $post_status, array (
			'inherit',
			'private',
			'trash',
			'auto-draft' 
	...
	if ('attachment' === $post_type) {
		 ...
		 * @param array $postarr
		 *        	An array of unsanitized attachment post data.
		 */
 ...
 *        	Optional. Post data. Arrays are expected to be escaped,
 *        	objects are not. Default array.
 * @param bool $wp_error
 *        	Optional. Allow return of WP_Error on failure. Default false.
...
function wp_update_post($postarr = array(), $wp_error = false) {
	...
	if (isset ( $post ['post_status'] ) && in_array ( $post ['post_status'], array (
			'draft',
			'pending',
			'auto-draft' 
 ...
 *        	Optional. An array of tags to set for the post, or a string of tags
 ...
 *        	separated by commas. Default empty.
 * @param bool $append
 *        	Optional. If true, don't delete existing tags, just add on. If false,
 ...
 *        	Optional. The Post ID. Does not default to the ID of the global $post.
 * @param string|array $tags
 *        	Optional. An array of terms to set for the post, or a string of terms
 *        	separated by commas. Default empty.
 * @param string $taxonomy
 *        	Optional. Taxonomy name. Default 'post_tag'.
 * @param bool $append
 *        	Optional. If true, don't delete existing terms, just add on. If false,
 ...
 *       
 * @param int $post_ID
 *        	Optional. The Post ID. Does not default to the ID
 *        	of the global $post. Default 0.
 * @param array|int $post_categories
 ...
 *        	a WP_Post object, an associative array, or a numeric array, respectively. Default OBJECT.
 * @param string $filter
 *        	Optional. How the return value should be filtered. Accepts 'raw',
 ...
 *        	Optional. Page ID or WP_Post object. Default is global $post.
...
function get_pages($args = array()) {
	...
	if ('' !== $sort_order && ! in_array ( $sort_order, array (
			'ASC',
			'DESC' 
	) )) {
		$sort_order = 'ASC';
 ...
 * @param bool $unfiltered
 *        	Optional. If true, filters are not run. Default false.
...
function wp_get_attachment_metadata($post_id = 0, $unfiltered = false) {
	 ...
	 *        	Array of meta data for the given attachment, or false
...
function wp_mime_type_icon($mime = 0) {
	...
	if (empty ( $icon )) {
		...
		if (! is_array ( $icon_files )) {
			 ...
			 *       
			 * @param array $uris
			 *        	List of icon directory URIs.
			 */
			$dirs = apply_filters ( 'icon_dirs', array (
					$icon_dir => $icon_dir_uri 
			) );
			$icon_files = array ();
			while ( $dirs ) {
				...
				if ($dh = opendir ( $dir )) {
					while ( false !== $file = readdir ( $dh ) ) {
						...
						if (substr ( $file, 0, 1 ) == '.')
							continue;
						if (! in_array ( strtolower ( substr ( $file, - 4 ) ), array (
								'.png',
								'.gif',
								'.jpg' 
						) )) {
	 ...
	 *        	Mime type.
	 * @param int $post_id
	 *        	Attachment ID. Will equal 0 if the function passed
	 *        	the mime type.
	 */
 ...
 * @param bool $full
 *        	Optional. Returns a full WHERE statement instead of just
 *        	an 'andalso' term. Default true.
 * @param int $post_author
 *        	Optional. Query posts having a single author ID. Default null.
 * @param bool $public_only
 *        	Optional. Only return public posts. Skips cap checks for
 ...
 *        	Optional. The timezone for the timestamp. Accepts 'server', 'blog', or 'gmt'.
...
function get_lastpostdate($timezone = 'server', $post_type = 'any') {
	 ...
	 *       
	 * @param string $date
	 *        	Date the last post was published.
	 * @param string $timezone
 ...
 *       
 * @param string $timezone
 *        	Optional. The timezone for the timestamp. See get_lastpostdate()
 ...
 *        	Not used. Can be set to null. Never implemented. Not marked
...
function _future_post_hook($deprecated, $post) {
	...
	wp_schedule_single_event ( strtotime ( get_gmt_from_date ( $post->post_date ) . ' GMT' ), 'publish_future_post', array (
			$post->ID 
	) );
}
 ...
 *       
 * @param int|WP_Post $post
 *        	Post ID or post object where thumbnail should be removed from.
 * @return bool True on success, false on failure.
 */
 ...
 *        	Optional. Post ID that should be ignored. Default 0.
 */
 ...
 *        
 * @param array $clauses
 *        	An array including WHERE, GROUP BY, JOIN, ORDER BY,
