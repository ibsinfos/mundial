...
final class WP_Customize_Manager {
	...
	public function __construct($args = array()) {
		$args = array_merge ( array_fill_keys ( array (
				'changeset_uuid',
				'theme',
				'messenger_channel' 
		), null ), $args );
		...
		add_action ( 'wp_ajax_customize_save', array (
				$this,
				'save' 
		) );
		add_action ( 'wp_ajax_customize_refresh_nonces', array (
				$this,
				'refresh_nonces' 
		) );
		...
		add_filter ( 'customize_render_partials_response', array (
				$this,
	 ...
	 * Returns either the standard message for UI
	 * or the Ajax message.
	 *
	 * @since 3.4.0
	 *       
	 * @param mixed $ajax_message
	 *        	Ajax return
	 * @param mixed $message
	 *        	UI message
	 */
...
<?php
	...
	public function start_previewing_theme() {
		...
		if (! $this->is_theme_active ()) {
			...
			add_filter ( 'pre_option_current_theme', array (
					$this,
					'current_theme' 
			) );
			
			// @link: https://core.trac.wordpress.org/ticket/20027
			add_filter ( 'pre_option_stylesheet', array (
					$this,
					'get_stylesheet' 
			) );
			add_filter ( 'pre_option_template', array (
					$this,
					'get_template' 
			) );
			
			// Handle custom theme roots.
			add_filter ( 'pre_option_stylesheet_root', array (
					$this,
					'get_stylesheet_root' 
			) );
			add_filter ( 'pre_option_template_root', array (
					$this,
					'get_template_root' 
			) );
		}
	...
	public function stop_previewing_theme() {
		...
		if (! $this->is_theme_active ()) {
			...
			remove_filter ( 'pre_option_current_theme', array (
					$this,
					'current_theme' 
			) );
			
			// @link: https://core.trac.wordpress.org/ticket/20027
			remove_filter ( 'pre_option_stylesheet', array (
					$this,
					'get_stylesheet' 
			) );
			remove_filter ( 'pre_option_template', array (
					$this,
					'get_template' 
			) );
			
			// Handle custom theme roots.
			remove_filter ( 'pre_option_stylesheet_root', array (
					$this,
					'get_stylesheet_root' 
			) );
			remove_filter ( 'pre_option_template_root', array (
					$this,
					'get_template_root' 
			) );
		}
	...
	function import_theme_starter_content($starter_content = array()) {
		...
		if (! empty ( $all_post_slugs )) {
			$existing_posts_query = new WP_Query ( array (
					...
					'post_status' => array_diff ( get_post_stati (), array (
							'auto-draft' 
					) ),
					'post_type' => 'any',
					'posts_per_page' => - 1 
		...
		if (! empty ( $attachments )) {
			...
			foreach ( $attachments as $symbol => $attachment ) {
				...
				if (! $attachment_id) {
					...
					$attachment_post_data = array_merge ( wp_array_slice_assoc ( $attachment, array (
							'post_title',
							'post_content',
							'post_excerpt' 
					) ), array (
	...
	public function _save_starter_content_changeset() {
		...
		$this->save_changeset_post ( array (
				'data' => array_fill_keys ( $this->pending_starter_content_settings_ids, array (
	 ...
	 * @param mixed $default
	 *        	Value returned $setting has no post value (added in 4.2.0)
	 ...
	 * preview, since it causes the jQuery Ajax to fail.
	 * Send 200 instead.
	 *
	 * @since 4.0.0
	 * @deprecated 4.7.0
...
form.customize-unpreviewable, form.customize-unpreviewable input, form.customize-unpreviewable select,
...
<?php
	...
	public function save() {
		...
		if (isset ( $_POST ['customize_changeset_status'] )) {
			...
			if (! get_post_status_object ( $changeset_status ) || ! in_array ( $changeset_status, array (
					'draft',
					'pending',
					'publish',
					'future' 
		...
		if (isset ( $response ['setting_validities'] )) {
			$response ['setting_validities'] = array_map ( array (
					$this,
					'prepare_setting_validity_for_js' 
		 ...
		 *        	Additional information passed back to the 'saved'
	...
	function save_changeset_post($args = array()) {
		...
		// Validate settings.
		$validated_values = array_merge ( array_fill_keys ( array_keys ( $args ['data'] ), null ), // Make sure existence/capability checks are done on value-less setting updates.
$post_values );
		...
		foreach ( $args ['data'] as $setting_id => $setting_params ) {
			...
			if (null === $setting_params) {
				...
				$data [$changeset_setting_id] = array_merge ( $merged_setting_params, array (
						'type' => $setting->type,
		...
		remove_filter ( 'wp_save_post_revision_post_has_changed', array (
				$this,
				'_filter_revision_post_has_changed' 
		) );
	 ...
	 * @access private
	 *        
	 * @param array $inactive_theme_mod_settings
	 ...
	 * @param array $args
	 *        	Setting arguments; passed to WP_Customize_Setting
	 *        	constructor.
	 ...
	 * @param array $args
	 *        	Optional. Panel arguments. Default empty array.
	 *        	
	 ...
	 * @param array $args
	 *        	Control arguments; passed to WP_Customize_Control
	 *        	constructor.
	 ...
	 *        	Name of a custom control which is a subclass of
...
<?php
	 ...
	 *            
	 * @param WP_Customize_Panel|WP_Customize_Section|WP_Customize_Control $a
	 *        	Object A.
	 * @param WP_Customize_Panel|WP_Customize_Section|WP_Customize_Control $b
	 *        	Object B.
	 * @return int
	 */
	...
	public function set_autofocus($autofocus) {
		$this->autofocus = array_filter ( wp_array_slice_assoc ( $autofocus, array (
				'panel',
				'section',
				'control' 
		) ), 'is_string' );
	...
	public function get_nonces() {
		 ...
		 *       
		 * @param array $nonces
		 *        	Array of refreshed nonces for save and
		 *        	preview actions.
		 * @param WP_Customize_Manager $this
		 *        	WP_Customize_Manager instance.
		 */
...
<?php
	...
	public function register_controls() {
		...
		if (! $this->is_theme_active ()) {
			$themes = wp_prepare_themes_for_js ( array (
					wp_get_theme ( $this->original_stylesheet ) 
			) );
		...
		$this->add_control ( new WP_Customize_Site_Icon_Control ( $this, 'site_icon', array (
				...
				__ ( 'The Site Icon is used as a browser and app icon for your site. Icons must be square, and at least %s pixels wide and tall.' ), '<strong>512</strong>' ),
				'section' => 'title_tagline',
				'priority' => 60,
				'height' => 512,
				'width' => 512 
		...
		if (current_theme_supports ( 'custom-header', 'video' )) {
			...
			if ($width && $height) {
				$control_description = sprintf(
					...
					__ ( 'Upload your video in %1$s format and minimize its file size for best results. Your theme recommends dimensions of %2$s pixels.' ), '<code>.mp4</code>', sprintf ( '<strong>%s &times; %s</strong>', $width, $height ) );
			} elseif ($width) {
				$control_description = sprintf(
					/* translators: 1: .mp4, 2: header width in pixels */
					__ ( 'Upload your video in %1$s format and minimize its file size for best results. Your theme recommends a width of %2$s pixels.' ), '<code>.mp4</code>', sprintf ( '<strong>%s</strong>', $width ) );
			} else {
				$control_description = sprintf(
					/* translators: 1: .mp4, 2: header height in pixels */
					__ ( 'Upload your video in %1$s format and minimize its file size for best results. Your theme recommends a height of %2$s pixels.' ), '<code>.mp4</code>', sprintf ( '<strong>%s</strong>', $height ) );
			}
		} else {
	 ...
	 * @since 4.7.0
	 *       
	 * @return s bool Whether there are published (or to be published) pages.
	 */
	...
	/**
	 * Add settings from the POST data that were not added with code, e.g.
	 * dynamically-created settings for Widgets
	 *
	 * @since 4.2.0
	 * @access public
	 *        
		...
		if ($video) {
			...
			if ('.mp4' !== substr ( $video, - 4 ) && '.mov' !== substr ( $video, - 4 )) { // Check for .mp4 or .mov format, which (assuming h.264 encoding) are the only cross-browser-supported formats.
				$validity->add ( 'invalid_file_type', sprintf(
					...
					__ ( 'Only %1$s or %2$s files may be used for header video. Please convert your video file and try again, or, upload your video to YouTube and link it with the option below.' ), '<code>.mp4</code>', '<code>.mov</code>' ) );
			}
		}
		return $validity;
	}
