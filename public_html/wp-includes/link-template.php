 ...
 *        	Optional. The type of URL being considered (e.g. single, category, etc)
...
function user_trailingslashit($string, $type_of_url = '') {
	 ...
	 *        	URL with or without a trailing slash.
	 * @param string $type_of_url
	 *        	The type of URL being considered. Accepts 'single', 'single_trackback',
 ...
 *        	Optional. Whether to keep post name or page name. Default false.
 *        	
 ...
 *        
 * @param int $id
 *        	Optional. Post ID. Default uses the global `$post`.
 * @param bool $leavename
...
function get_post_permalink($id = 0, $leavename = false, $sample = false) {
	...
	$draft_or_pending = get_post_status ( $id ) && in_array ( get_post_status ( $id ), array (
			'draft',
			'pending',
			'auto-draft',
			'future' 
...
function _get_page_link($post = false, $leavename = false, $sample = false) {
	...
	$draft_or_pending = in_array ( $post->post_status, array (
			'draft',
			'pending',
			'auto-draft' 
	) );
 ...
 * @param string $feed
 *        	Optional. Feed type. Default empty.
 */
function the_feed_link($anchor, $feed = '') {
	 ...
	 * @param string $feed
	 *        	The feed type, or an empty string for the
	 *        	default feed type.
	 */
 ...
 *        	Optional. Post ID. Default is the ID of the global `$post`.
 * @param string $feed
 *        	Optional. Feed format. Default empty.
 */
 ...
 *        	Optional. Taxonomy of `$term_id`. Default 'category'.
 * @param string $feed
 *        	Optional. Feed type. Default empty.
 ...
 *       
 * @param string $link
 *        	Optional. Anchor text. Default empty.
 * @param string $before
 ...
 *        	Term ID.
 * @param string $taxonomy
 *        	Optional. Taxonomy. Defaults to the taxonomy of the term identified
 *        	by `$term_id`.
 * @param string $object_type
 *        	Optional. The object type. Used to highlight the proper post type
 ...
 *        	Optional. Display after edit link. Default empty.
 * @param object $term
 *        	Optional. Term object. If null, the queried object will be inspected. Default null.
 * @param bool $echo
 ...
 *        
 * @param string $query
 *        	Optional. The query string to use. If empty the current query is used. Default empty.
 ...
 *        	Optional. Search query. Default empty.
 * @param string $feed
 *        	Optional. Feed type. Default empty.
 ...
 *       
 * @param int|WP_Post $post
 *        	Optional. Post ID or `WP_Post` object. Defaults to global `$post`.
 * @param array $query_args
 *        	Optional. Array of additional query args to be appended to the link.
 *        	Default empty array.
 * @param string $preview_link
 *        	Optional. Base preview link to be used if it should differ from the
 ...
 *       
 * @param string $text
 *        	Optional. Anchor text. If null, default is 'Edit This'. Default null.
 * @param string $before
 *        	Optional. Display before edit link. Default empty.
 * @param string $after
 *        	Optional. Display after edit link. Default empty.
	...
	if ($in_same_term || ! empty ( $excluded_terms )) {
		...
		if ($in_same_term) {
			...
			if (! is_object_in_taxonomy ( $post->post_type, $taxonomy ))
				return '';
			$term_array = wp_get_object_terms ( $post->ID, $taxonomy, array (
					'fields' => 'ids' 
			) );
 ...
 *        	Optional. Whether link should be in a same taxonomy term. Default false.
 * @param array|string $excluded_terms
 *        	Optional. Array or comma-separated list of excluded term IDs. Default true.
 * @param string $taxonomy
 *        	Optional. Taxonomy, if $in_same_term is true. Default 'category'.
 */
 ...
 *       
 * @param bool $in_same_term
 *        	Optional. Whether returned post should be in a same taxonomy term.
 *        	Default false.
 * @param array|string $excluded_terms
 *        	Optional. Array or comma-separated list of excluded term IDs.
 *        	Default empty.
 * @param bool $start
 *        	Optional. Whether to retrieve first or last post. Default true
 * @param string $taxonomy
 *        	Optional. Taxonomy, if $in_same_term is true. Default 'category'.
 ...
 *       
 * @param string $format
 *        	Optional. Link anchor format. Default '&laquo; %link'.
 * @param string $link
 ...
 *        	Optional. Whether link should be in a same taxonomy term. Default false.
 * @param array|string $excluded_terms
 *        	Optional. Array or comma-separated list of excluded terms IDs. Default empty.
 * @param bool $previous
 *        	Optional. Whether to display link to previous or next post. Default true.
 * @param string $taxonomy
 *        	Optional. Taxonomy, if $in_same_term is true. Default 'category'.
 ...
 *        
 * @param int $pagenum
 *        	Optional. Page ID. Default 1.
 * @param bool $escape
 *        	Optional. Whether to escape the URL for display, with esc_url(). Defaults to true.
 ...
 *       
 * @param string $sep
 *        	Optional. Separator for posts navigation links. Default empty.
 * @param string $prelabel
 *        	Optional. Label for previous pages. Default empty.
 ...
 *       
 * @param array $args
 *        	Optional. See get_the_post_navigation() for available arguments.
 *        	Default empty array.
 */
 ...
 *        	Navigational links.
 * @param string $class
 *        	Optional. Custom class for nav element. Default: 'posts-navigation'.
 * @param string $screen_reader_text
 *        	Optional. Screen reader text for nav element. Default: 'Posts navigation'.
 ...
 *        
 * @param string|array $args
 *        	Optional args. See paginate_links(). Default empty array.
 ...
 *       
 * @param array $args
 *        	See get_the_comments_navigation() for available arguments. Default empty array.
 */
...
function get_home_url($blog_id = null, $path = '', $scheme = null) {
	 ...
	 *        	Scheme to give the home URL context. Accepts 'http', 'https',
 ...
 *       
 * @param string $path
 *        	Optional. Path relative to the includes URL. Default empty.
 * @param string $scheme
 *        	Optional. Scheme to give the includes URL context. Accepts
...
function includes_url($path = '', $scheme = null) {
	 ...
	 * @param string $path
	 *        	Path relative to the URL to the wp-includes directory. Blank string
 ...
 *       
 * @param string $path
 *        	Optional. Path relative to the content URL. Default empty.
 ...
 *       
 * @param string $path
 *        	Optional. Extra path appended to the end of the URL, including
...
function network_site_url($path = '', $scheme = null) {
	 ...
	 * @param string $path
	 *        	Path relative to the network site URL. Blank string if
...
function network_admin_url($path = '', $scheme = 'admin') {
	 ...
	 * @param string $path
	 *        	Path relative to the network admin URL. Blank string if
	 *        	no path is specified.
	 */
...
function user_admin_url($path = '', $scheme = 'admin') {
	 ...
	 * @param string $path
	 *        	Path relative to the URL. Blank string if
	 *        	no path is specified.
	 */
 ...
 * @param string|null $scheme
 *        	Optional. Scheme to give $url. Currently 'http', 'https', 'login',
 ...
 * @param string $path
 *        	Optional path relative to the dashboard. Use only paths known to
 ...
 *       
 * @param int $id
 *        	Optional. A post or site id. Default is 0, which means the current post or site.
 * @param string $context
 *        	Optional. Whether the id is a 'site' id, 'post' id, or 'media' id. If 'post',
 *        	the post_type of the post is consulted. If 'query', the current query is consulted
 *        	to determine the id and context. Default 'post'.
 * @param bool $allow_slugs
 *        	Optional. Whether to allow post slugs in the shortlink. It is up to the plugin how
 ...
 *        	The Gravatar to retrieve a URL for. Accepts a user_id, gravatar md5 hash,
 ...
 *       
 * @param mixed $id_or_email
 *        	The Gravatar to retrieve. Accepts a user_id, gravatar md5 hash,
 *        	user email, WP_User object, WP_Post object, or WP_Comment object.
 * @param array $args
 *        	{
 *        	Optional. Arguments to return instead of the default arguments.
 ...
 *       
 * @param string $file
 *        	Optional. File to return the URL for in the template directory.
 * @return string The URL of the file.
 */
 ...
 *       
 * @param string $file
 *        	Optional. File to return the path for in the template directory.
 * @return string The path of the file.
 */
