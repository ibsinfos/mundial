...
function wp_authenticate_username_password($user, $username, $password) {
	 ...
	 *        	WP_User or WP_Error object if a previous
	...
	if (! wp_check_password ( $password, $user->user_pass, $user->ID )) {
		return new WP_Error ( 'incorrect_password', sprintf(
				...
				__ ( '<strong>ERROR</strong>: The password you entered for the username %s is incorrect.' ), '<strong>' . $username . '</strong>' ) . ' <a href="' . wp_lostpassword_url () . '">' . __ ( 'Lost your password?' ) . '</a>' );
	}
...
function wp_authenticate_email_password($user, $email, $password) {
	...
	if (! wp_check_password ( $password, $user->user_pass, $user->ID )) {
		return new WP_Error ( 'incorrect_password', sprintf(
				...
				__ ( '<strong>ERROR</strong>: The password you entered for the email address %s is incorrect.' ), '<strong>' . $email . '</strong>' ) . ' <a href="' . wp_lostpassword_url () . '">' . __ ( 'Lost your password?' ) . '</a>' );
	}
 ...
 *        	Array of user IDs.
 * @param string|array $post_type
 *        	Optional. Single post type or array of post types to check. Defaults to 'post'.
 * @param bool $public_only
 *        	Optional. Only return counts for public posts. Defaults to false.
 ...
 *
 * @param array $args
 *        	Optional. Arguments to retrieve users. See WP_User_Query::prepare_query().
 ...
 *        	User ID
 * @param bool $all
 *        	Whether to retrieve all sites, or only sites that are not
...
function get_blogs_of_user($user_id, $all = false) {
	 ...
	 * @since 4.6.0
	 *       
	 * @param null|array $sites
	 *        	An array of site objects of which the user is a member.
	 * @param int $user_id
	 *        	User ID.
	 * @param bool $all
	 *        	Whether the returned array should contain all sites, including
	 ...
	 *        	User ID.
	 * @param bool $all
	 *        	Whether the returned sites array should contain all sites, including
	 *        	those marked 'deleted', 'archived', or 'spam'. Default false.
	 */
 ...
 *        	Optional. User ID to set up global data.
 */
 ...
 *        	How to sanitize user fields. Looks for 'raw', 'edit', 'db', 'display',
 *        	'attribute' and 'js'.
 ...
 *        	The user's password.
 * @param string $email
 *        	Optional. The user's email. Default empty.
 * @return int|WP_Error The newly created user's ID or a WP_Error object if the user could not
 *         be created.
 */
 ...
 *        
 * @param WP_User $user
 *        	Optional. WP_User object. Default null.
 * @return array Array of contact methods and their labels.
 */
...
function register_new_user($user_login, $user_email) {
	 ...
	 *        	Contains any errors with submitted username and email,
	 ...
	 *       
	 * @param WP_Error $errors
	 *        	A WP_Error object containing any errors encountered
	 *        	during registration.
	 * @param string $sanitized_user_login
 ...
 * @param string $notify
 *        	Optional. Type of notification that should happen. Accepts 'admin'
