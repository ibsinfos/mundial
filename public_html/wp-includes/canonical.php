 ...
 *        	Optional. The URL that was requested, used to
...
function redirect_canonical($requested_url = null, $do_redirect = true) {
	...
	if (isset ( $_SERVER ['REQUEST_METHOD'] ) && ! in_array ( strtoupper ( $_SERVER ['REQUEST_METHOD'] ), array (
			'GET',
			'HEAD' 
	) )) {
		return;
	...
	if (is_feed () && ($id = get_query_var ( 'p' ))) {
		if ($redirect_url = get_post_comments_feed_link ( $id, get_query_var ( 'feed' ) )) {
			$redirect ['query'] = _remove_qs_args_if_not_in_url ( $redirect ['query'], array (
					'p',
					'page_id',
					'attachment_id',
					'pagename',
	...
	if (is_404 ()) {
		...
		if (is_attachment () && ! array_diff ( array_keys ( $wp->query_vars ), array (
				'attachment',
				'attachment_id' 
		) ) && ! $redirect_url) {
			...
			if ($redirect_url = get_permalink ( get_query_var ( 'p' ) ))
				$redirect ['query'] = remove_query_arg ( array (
						'p',
						'post_type' 
				), $redirect ['query'] );
...
function redirect_guess_404_permalink() {
	...
	if (get_query_var ( 'name' )) {
		...
		if (get_query_var ( 'post_type' ))
			...
			$where .= " AND post_type IN ('" . implode ( "', '", get_post_types ( array (
					'public' => true 
			) ) ) . "')";
