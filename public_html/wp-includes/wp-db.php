...
class wpdb {
	 ...
	 * @param string $charset
	 *        	Optional. The character set. Default null.
	 * @param string $collate
	 *        	Optional. The collation. Default null.
	 */
	 ...
	 *       
	 * @param array $modes
	 *        	Optional. A list of SQL modes to set.
	 */
		...
		if (! $success) {
			...
			if (! did_action ( 'template_redirect' )) {
				...
				$message .= '<p>' . sprintf(
					...
					__ ( 'If you don&#8217;t know how to set up a database you should <strong>contact your host</strong>. If all else fails you may find help at the <a href="%s">WordPress Support Forums</a>.' ), __ ( 'https://wordpress.org/support/' ) ) . "</p>\n";
	 ...
	 *        	The array of variables to substitute into the query's placeholders if being called with an array of arguments,
	 *        	or the first variable to substitute into the query's placeholders if being called with individual arguments.
	 * @param mixed $args,...
	 *        	further variables to substitute into the query's placeholders if being called wih individual arguments.
		...
		if (count ( $args ) !== $placeholders) {
			if (1 === $placeholders && $passed_as_array) {
				...
				_doing_it_wrong ( 'wpdb::prepare',
					...
					sprintf ( __ ( 'The query does not contain the correct number of placeholders (%1$d) for the number of arguments passed (%2$d).' ), $placeholders, count ( $args ) ), '4.8.3' );
			}
		}
	 ...
	 *        
	 * @param string $text
	 *        	The raw text to be escaped. The input typed by the user should have no
	 ...
	 *        	Optional. Allows the function to bail. Default true.
	 ...
	 *        	Optional. An array of formats to be mapped to each of the value in $data.
	 ...
	 * @param array|string $format
	 *        	Optional. An array of formats to be mapped to each of the values in $data.
	 *        	If string, that format will be used for all of the values in $data.
	 *        	A format is one of '%d', '%f', '%s' (integer, float, string).
	 *        	If omitted, all values in $data will be treated as strings unless otherwise specified in wpdb::$field_types.
	 * @param array|string $where_format
	 *        	Optional. An array of formats to be mapped to each of the values in $where.
...
;
	 ...
	 *        
	 * @param array $data
	 *        	Array of value arrays. Each value array has the keys
	...
	protected function strip_invalid_text_from_query($query) {
		...
		$data = $this->strip_invalid_text ( array (
				$data 
		) );
		if (is_wp_error ( $data )) {
			return $data;
