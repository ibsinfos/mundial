...
function get_the_author($deprecated = '') {
	 ...
	 * @since 2.9.0
	 *       
	 * @param string $authordata->display_name
...
function get_the_modified_author() {
	if ($last_id = get_post_meta ( get_post ()->ID, '_edit_last', true )) {
		 ...
		 * @since 2.8.0
		 *       
		 * @param string $last_user->display_name
		 *        	The author's display name.
		 */
...
function get_the_author_link() {
	if (get_the_author_meta ( 'url' )) {
		return sprintf ( '<a href="%1$s" title="%2$s" rel="author external">%3$s</a>', esc_url ( get_the_author_meta ( 'url' ) ),
			/* translators: %s: author's display name */
			esc_attr ( sprintf ( __ ( 'Visit %s&#8217;s website' ), get_the_author () ) ), get_the_author () );
	} else {
		return get_the_author ();
	}
}
...
function get_the_author_posts_link() {
	...
	$link = sprintf ( '<a href="%1$s" title="%2$s" rel="author">%3$s</a>', esc_url ( get_author_posts_url ( $authordata->ID, $authordata->user_nicename ) ),
		/* translators: %s: author's display name */
		esc_attr ( sprintf ( __ ( 'Posts by %s' ), get_the_author () ) ), get_the_author () );
...
function wp_list_authors($args = '') {
	...
	foreach ( $authors as $author_id ) {
		...
		$link = sprintf ( '<a href="%1$s" title="%2$s">%3$s</a>', get_author_posts_url ( $author->ID, $author->user_nicename ),
			/* translators: %s: author's display name */
			esc_attr ( sprintf ( __ ( 'Posts by %s' ), $author->display_name ) ), $name );
