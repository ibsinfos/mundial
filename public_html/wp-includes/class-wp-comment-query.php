...
class WP_Comment_Query {
	...
	public function parse_query($query = '') {
		 ...
		 *       
		 * @param
		 *        	WP_Comment_Query &$this The WP_Comment_Query instance (passed by reference).
		 */
		do_action_ref_array ( 'parse_comment_query', array (
				&$this 
		) );
	}
	...
	public function get_comments() {
		 ...
		 */
		do_action_ref_array ( 'pre_get_comments', array (
				&$this 
		) );
		 ...
		 *        	WP_Comment_Query &$this Current instance of WP_Comment_Query, passed by reference.
		 */
		$_comments = apply_filters_ref_array ( 'the_comments', array (
				$_comments,
				&$this 
		) );
	...
	protected function get_comment_ids() {
		...
		if (in_array ( $this->query_vars ['orderby'], array (
				'none',
				array (),
				false 
		), true )) {
		...
		if (strlen ( $this->query_vars ['search'] )) {
			$search_sql = $this->get_search_sql ( $this->query_vars ['search'], array (
					'comment_author',
		...
		$plucked = wp_array_slice_assoc ( $this->query_vars, array (
				'post_author',
				'post_name',
				'post_parent' 
		) );
		 ...
		 *        	WP_Comment_Query &$this Current instance of WP_Comment_Query, passed by reference.
		 */
		$clauses = apply_filters_ref_array ( 'comments_clauses', array (
				compact ( $pieces ),
				&$this 
		) );
	...
	private function set_found_comments() {
		...
		if ($this->query_vars ['number'] && ! $this->query_vars ['no_found_rows']) {
			 ...
			 * @since 4.4.0
			 *       
			 * @param string $found_comments_query
