...
final class WP_Customize_Widgets {
	...
	public function __construct($manager) {
		...
		add_filter ( 'is_active_sidebar', array (
				$this,
				'tally_sidebars_via_is_active_sidebar_calls' 
		), 10, 2 );
		add_filter ( 'dynamic_sidebar_has_widgets', array (
				$this,
	...
	public function customize_register() {
		...
		$sidebars_widgets = array_merge ( array (
				'wp_inactive_widgets' => array () 
		), array_fill_keys ( array_keys ( $wp_registered_sidebars ), array () ), wp_get_sidebars_widgets () );
	...
	public function enqueue_scripts() {
		...
		$move_widget_area_tpl = str_replace ( array (
				'{description}',
				'{btn}' 
		), array (
	...
	<div id="available-widgets">
		...
		<div id="available-widgets-list">
			<?php foreach ( $this->get_available_widgets() as $available_widget ): ?>
				<div
				id="widget-tpl-<?php echo esc_attr( $available_widget['id'] ) ?>"
...
<?php
	...
	public function get_setting_args($id, $overrides = array()) {
		...
		if (preg_match ( $this->setting_id_patterns ['sidebar_widgets'], $id, $matches )) {
			$args ['sanitize_callback'] = array (
					$this,
					'sanitize_sidebar_widgets' 
			);
			$args ['sanitize_js_callback'] = array (
					$this,
...
<?php
	...
	public function customize_dynamic_partial_args($partial_args, $partial_id) {
		...
		if (preg_match ( '/^widget\[(?P<widget_id>.+)\]$/', $partial_id, $matches )) {
			...
			$partial_args = array_merge ( $partial_args, array (
					...
					'settings' => array (
							$this->get_setting_id ( $matches ['widget_id'] ) 
					),
					'capability' => 'edit_theme_options' 
			) );
		}
	...
	public function filter_wp_kses_allowed_data_attributes($allowed_html) {
		foreach ( array_keys ( $this->before_widget_tags_seen ) as $tag_name ) {
			...
			$allowed_html [$tag_name] = array_merge ( $allowed_html [$tag_name], array_fill_keys ( array (
	...
	public function filter_sidebars_widgets_for_rendering_widget($sidebars_widgets) {
		$sidebars_widgets [$this->rendering_sidebar_id] = array (
	...
	public function capture_filter_pre_update_option($new_value, $option_name, $old_value) {
		...
		if (! isset ( $this->_captured_options [$option_name] )) {
			add_filter ( "pre_option_{$option_name}", array (
					$this,
	...
	protected function stop_capturing_option_updates() {
		...
		remove_filter ( 'pre_update_option', array (
				$this,
				'capture_filter_pre_update_option' 
		), 10 );
		
		foreach ( array_keys ( $this->_captured_options ) as $option_name ) {
			remove_filter ( "pre_option_{$option_name}", array (
					$this,
					'capture_filter_pre_get_option' 
			) );
		}
