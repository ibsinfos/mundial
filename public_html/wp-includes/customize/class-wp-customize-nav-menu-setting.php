...
class WP_Customize_Nav_Menu_Setting extends WP_Customize_Setting {
	 ...
	 *        	Bootstrap Customizer instance.
	 * @param string $id
	 *        	An specific ID of the setting. Can be a
	...
	public function preview() {
		...
		add_filter ( 'wp_get_nav_menus', array (
				$this,
				'filter_wp_get_nav_menus' 
		), 10, 2 );
		add_filter ( 'wp_get_nav_menu_object', array (
				$this,
				'filter_wp_get_nav_menu_object' 
		), 10, 2 );
		add_filter ( 'default_option_nav_menu_options', array (
				$this,
				'filter_nav_menu_options' 
		) );
		add_filter ( 'option_nav_menu_options', array (
				$this,
				'filter_nav_menu_options' 
		) );
	...
	public function filter_wp_get_nav_menus($menus, $args) {
		...
		if ($is_delete) {
			...
			array_splice ( $menus, $index, (- 1 === $index ? 0 : 1), array (
					$menu_obj 
			) );
		}
	...
	protected function update($value) {
		...
		if ($is_delete) {
			...
			$menu_data = wp_array_slice_assoc ( $value, array (
					'description',
					'parent' 
			) );
