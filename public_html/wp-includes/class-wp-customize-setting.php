...
;
		...
		if (! empty ( $this->id_data ['keys'] )) {
			...
			add_action ( "customize_post_value_set_{$this->id}", array (
					$this,
	...
	public function preview() {
		...
		if (! $needs_preview) {
			if (! has_action ( "customize_post_value_set_{$this->id}", array (
					$this,
					'preview' 
			) )) {
				add_action ( "customize_post_value_set_{$this->id}", array (
		...
		switch ($this->type) {
			case 'theme_mod' :
				if (! $is_multidimensional) {
					add_filter ( "theme_mod_{$id_base}", array (
							$this,
							'_preview_filter' 
					) );
				} else {
			...
			case 'option' :
				if (! $is_multidimensional) {
					add_filter ( "pre_option_{$id_base}", array (
							$this,
							'_preview_filter' 
					) );
				} else {
			...
			default :
				 ...
				 * @since 3.4.0
				 *       
				 * @param WP_Customize_Setting $this
