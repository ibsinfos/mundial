...
class WP_Network_Query {
	...
	public function parse_query($query = '') {
		 ...
		 */
		do_action_ref_array ( 'parse_network_query', array (
				&$this 
		) );
	}
	...
	public function get_networks() {
		 ...
		 *        	WP_Network_Query &$this Current instance of WP_Network_Query, passed by reference.
		 */
		$_networks = apply_filters_ref_array ( 'the_networks', array (
				$_networks,
				&$this 
		) );
	...
	protected function get_network_ids() {
		...
		if (strlen ( $this->query_vars ['search'] )) {
			$this->sql_clauses ['where'] ['search'] = $this->get_search_sql ( $this->query_vars ['search'], array (
					"$wpdb->site.domain",
					"$wpdb->site.path" 
			) );
		}
		 ...
		 *       
		 * @param array $pieces
		 *        	A compacted array of network query clauses.
		 * @param
		 *        	WP_Network_Query &$this Current instance of WP_Network_Query, passed by reference.
		 */
		$clauses = apply_filters_ref_array ( 'networks_clauses', array (
				compact ( $pieces ),
				&$this 
		) );
	...
	private function set_found_networks() {
		...
		if ($this->query_vars ['number'] && ! $this->query_vars ['no_found_rows']) {
			 ...
			 * @since 4.6.0
			 *       
			 * @param string $found_networks_query
