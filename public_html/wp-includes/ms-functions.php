 ...
 *        	Optional. A user to whom to reassign posts.
 * @return true|WP_Error
 */
...
function is_email_address_unsafe($user_email) {
	 ...
	 * @since 3.5.0
	 *       
	 * @param bool $is_email_address_unsafe
 ...
 *        	Optional. The user object to check against the new site name.
...
function wpmu_signup_blog_notification($domain, $path, $title, $user_login, $user_email, $key, $meta = array()) {
	...
	apply_filters ( 'wpmu_signup_blog_notification_subject',
			...
			_x ( '[%1$s] Activate %2$s', 'New site notification email subject' ), $domain, $path, $title, $user_login, $user_email, $key, $meta ), $from_name, esc_url ( 'http://' . $domain . $path ) );
...
function wpmu_signup_user_notification($user_login, $user_email, $key, $meta = array()) {
	...
	apply_filters ( 'wpmu_signup_user_notification_subject',
			...
			_x ( '[%1$s] Activate %2$s', 'New user notification email subject' ), $user_login, $user_email, $key, $meta ), $from_name, $user_login );
	wp_mail ( $user_email, wp_specialchars_decode ( $subject ), $message, $message_headers );
 ...
 * @param array $meta
 *        	Optional. Used to set initial site options.
 * @param int $site_id
 *        	Optional. Only relevant on multi-network installs.
...
function wpmu_create_blog($domain, $path, $title, $user_id, $meta = array(), $site_id = 1) {
	 ...
	 *        	Site path.
	 * @param int $site_id
	 *        	Site ID. Only relevant on multi-network installs.
	 * @param array $meta
 ...
 * @param int $site_id
 *        	Optional. Relevant only on multi-network installs.
 * @return int
 */
 ...
 * @param array $meta
 *        	Optional. Not used in the default function, but is passed along to hooks for customization.
 * @return bool
 */
 ...
 *        	Optional. Full path of a subdirectory to exclude from the total.
...
function maybe_add_existing_user_to_blog() {
	...
	wp_die ( sprintf ( __ ( 'You have been added to this site. Please visit the <a href="%s">homepage</a> or <a href="%s">log in</a> using your username and password.' ), home_url (), admin_url () ), __ ( 'WordPress &rsaquo; Success' ), array (
			'response' => 200 
	) );
}
 ...
 *        	Optional. Defaults to current user. WP_User object,
...
function get_subdirectory_reserved_names() {
	 ...
	 *       
	 * @param array $subdirectory_reserved_names
