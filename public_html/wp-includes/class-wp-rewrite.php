...
class WP_Rewrite {
	...
	/**
	 * Extra permalink structures, e.g.
	 * categories, added by add_permastruct().
	 *
	 * @since 2.1.0
	 * @access private
	 * @var array
	...
	/**
	 * Whether to write every mod_rewrite rule for WordPress into the .
	 * htaccess file.
	 *
	...
	/**
	 * Adds or updates existing rewrite tags (e.g.
	 * %postname%).
	 *
	 ...
	 *
	 * @param string $tag
	 *        	Name of the rewrite tag to remove.
	 */
	 ...
	 * @param int $ep_mask
	 *        	Optional. Endpoint mask defining what endpoints are added to the structure.
	 ...
	 *        	Optional. Whether the 'directories' making up the structure should be walked
	...
	public function generate_rewrite_rules($permalink_structure, $ep_mask = EP_NONE, $paged = true, $feed = true, $forcomments = false, $walk_dirs = true, $endpoints = true) {
		...
		$dirs = $walk_dirs ? explode ( '/', $structure ) : array (
				$structure 
		);
		$num_dirs = count ( $dirs );
		...
		for($j = 0; $j < $num_dirs; ++ $j) {
			...
			if ($num_toks) {
				...
				if ($post) {
					...
					// Get rid of brackets.
					$submatchbase = str_replace ( array (
							'(',
							')' 
					), '', $match );
	 ...
	 *     
	 * @param string $permalink_structure
	 *        	The permalink structure to generate rules.
	 * @param bool $walk_dirs
	 *        	Optional, default is false. Whether to create list of directories to walk over.
	 * @return array
	 */
	...
	public function rewrite_rules() {
		...
		$robots_rewrite = (empty ( $home_path ['path'] ) || '/' == $home_path ['path']) ? array (
		 ...
		 * @since 1.5.0
		 *       
		 * @param array $date_rewrite
		 *        	The rewrite rules for date archives.
		 */
		 ...
		 * @since 1.5.0
		 *       
		 * @param array $comments_rewrite
		 *        	The rewrite rules for the site-wide comments feeds.
		 */
		 ...
		 *        	The rewrite rules for the "page" post type.
		 */
		...
		foreach ( $this->extra_permastructs as $permastructname => $struct ) {
			...
			if ('post_tag' == $permastructname) {
				 ...
				 *            
				 * @param array $rules
				 *        	The rewrite rules generated for tags.
				 */
		 ...
		 * @since 1.5.0
		 *       
		 * @param WP_Rewrite $this
		 *        	Current WP_Rewrite instance, passed by reference.
		 */
		do_action_ref_array ( 'generate_rewrite_rules', array (
				&$this 
		) );
		 ...
		 *        	The compiled array of rewrite rules.
		 */
	...
	/**
	 * Retrieves mod_rewrite-formatted rewrite rules to write to .
	 * htaccess.
	 *
	...
	public function mod_rewrite_rules() {
		...
		/**
		 * Filters the list of rewrite rules formatted for output to an .
		 * htaccess file.
		 *
		 * @since 1.5.0
		 *       
	...
	<system.webServer>
		 ...
		 *       
		 * @param string $rules
		 *        	Rewrite rules formatted for IIS web.config.
		 */
	...
	public function add_rule($regex, $query, $after = 'bottom') {
		...
		if ($external) {
			...
			if ('bottom' == $after) {
				$this->extra_rules = array_merge ( $this->extra_rules, array (
						$regex => $query 
				) );
			} else {
				$this->extra_rules_top = array_merge ( $this->extra_rules_top, array (
						$regex => $query 
				) );
			}
		}
	 ...
	 *        	Optional. Name of the corresponding query variable. Pass `false` to
