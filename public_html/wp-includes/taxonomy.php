 ...
 *        
 * @param array $args
 *        	Optional. An array of `key => value` arguments to match against the taxonomy objects.
 *        	Default empty array.
 * @param string $output
 *        	Optional. The type of output to return in the array. Accepts either taxonomy 'names'
 *        	or 'objects'. Default 'names'.
 * @param string $operator
 *        	Optional. The logical operation to perform. Accepts 'and' or 'or'. 'or' means only
 ...
 * @global array $wp_taxonomies The registered taxonomies.
 *        
 * @param array|string|WP_Post $object
 ...
 *        	If integer, term data will be fetched from the database, or from the cache if
 ...
 *        	Argument array, when using the legacy function parameter format. If present, this
 ...
 * @param bool $single
 *        	Whether to return a single value. If false, an array of all values matching the
 ...
 *        	The taxonomy name to use.
 * @param string $context
 *        	Optional. Context in which to sanitize the term. Accepts 'edit', 'db',
 ...
 *        	Taxonomy Name.
 * @param string $context
 *        	Context in which to sanitize the term field. Accepts 'edit', 'db', 'display',
 *        	'attribute', or 'js'.
...
function wp_delete_term($term, $taxonomy, $args = array()) {
	...
	if (is_taxonomy_hierarchical ( $taxonomy )) {
		...
		$wpdb->update ( $wpdb->term_taxonomy, compact ( 'parent' ), array (
	...
	foreach ( $object_ids as $object_id ) {
		$terms = wp_get_object_terms ( $object_id, $taxonomy, array (
				'fields' => 'ids',
				'orderby' => 'none' 
		) );
	 ...
	 *        	Copy of the already-deleted term, in the form specified
...
function wp_get_object_terms($object_ids, $taxonomies, $args = array()) {
	 ...
	 * @param array $args
	 *        	An array of arguments for retrieving terms for the given
...
function wp_insert_term($term, $taxonomy, $args = array()) {
	...
	$wpdb->insert ( $wpdb->term_taxonomy, compact ( 'term_id', 'taxonomy', 'description', 'parent' ) + array (
			'count' => 0 
	) );
 ...
 * @param bool $append
 *        	Optional. If false will delete difference of terms. Default false.
...
function wp_set_object_terms($object_id, $terms, $taxonomy, $append = false) {
	...
	if (! $append)
		$old_tt_ids = wp_get_object_terms ( $object_id, $taxonomy, array (
				'fields' => 'tt_ids',
				'orderby' => 'none' 
		) );
	else
	...
	if (! $append && isset ( $t->sort ) && $t->sort) {
		$values = array ();
		$term_order = 0;
		$final_tt_ids = wp_get_object_terms ( $object_id, $taxonomy, array (
				'fields' => 'tt_ids' 
		) );
...
function wp_update_term($term_id, $taxonomy, $args = array()) {
	...
	$wpdb->update ( $wpdb->term_taxonomy, compact ( 'term_id', 'taxonomy', 'description', 'parent' ), array (
			'term_taxonomy_id' => $tt_id 
	) );
 ...
 * @param string $taxonomy
 *        	Optional. Can be empty and will assume `tt_ids`, else will use for context.
 *        	Default empty.
 ...
 * @param string $taxonomy
 *        	Optional. Update Term to this taxonomy in cache. Default empty.
 */
 ...
 * @param array $terms
 *        	The set of terms - either an array of term objects or term IDs - from which those that
 *        	are descendants of $term_id will be chosen.
 * @param string $taxonomy
 *        	The taxonomy which determines the hierarchy of the terms.
 * @param array $ancestors
 *        	Optional. Term ancestors that have already been identified. Passed by reference, to keep
 ...
 *        	ID of the term_taxonomy item to receive a new term, or the term_taxonomy object
 *        	(corresponding to a row from the term_taxonomy table).
 * @param bool $record
 *        	Whether to record data about the split term in the options table. The recording
 ...
 *        	Optional. Term term_id, name, slug or array of said. Default null.
...
function is_object_in_term($object_id, $taxonomy, $terms = null) {
	...
	if (false === $object_terms) {
		$object_terms = wp_get_object_terms ( $object_id, $taxonomy, array (
				'update_term_meta_cache' => false 
		) );
 ...
 *        	Optional. Type of resource $object_type is. Accepts 'post_type'
...
function wp_check_term_hierarchy_for_loops($parent, $term_id, $taxonomy) {
	...
	if (! $loop = wp_find_hierarchy_loop ( 'wp_get_term_taxonomy_parent_id', $term_id, $parent, array (
			$taxonomy 
	) ))
	...
	foreach ( array_keys ( $loop ) as $loop_member )
		wp_update_term ( $loop_member, $taxonomy, array (
				'parent' => 0 
		) );
