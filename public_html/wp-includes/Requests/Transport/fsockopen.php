...
class Requests_Transport_fsockopen implements Requests_Transport {
	...
	public function request($url, $headers = array(), $data = array(), $options = array()) {
		...
		$options ['hooks']->dispatch ( 'fsockopen.remote_socket', array (
				&$remote_socket 
		) );
		...
		$options ['hooks']->dispatch ( 'fsockopen.remote_host_path', array (
				&$path,
				$url 
		) );
		...
		$options ['hooks']->dispatch ( 'fsockopen.after_headers', array (
				&$out 
		) );
		...
		$options ['hooks']->dispatch ( 'fsockopen.before_send', array (
				&$out 
		) );
		
		fwrite ( $socket, $out );
		$options ['hooks']->dispatch ( 'fsockopen.after_send', array (
				$out 
		) );
		...
		if (! $options ['blocking']) {
			fclose ( $socket );
			$fake_headers = '';
			$options ['hooks']->dispatch ( 'fsockopen.after_request', array (
					&$fake_headers 
			) );
			return '';
		}
		...
		while ( ! feof ( $socket ) ) {
			...
			if ($doingbody) {
				$options ['hooks']->dispatch ( 'request.progress', array (
						$block,
						$size,
						$this->max_bytes 
				) );
