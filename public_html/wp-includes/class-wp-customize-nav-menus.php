...
final class WP_Customize_Nav_Menus {
	...
	public function __construct($manager) {
		...
		add_filter ( 'customize_refresh_nonces', array (
				$this,
				'filter_nonces' 
		) );
		add_action ( 'wp_ajax_load-available-menu-items-customizer', array (
				$this,
				'ajax_load_available_items' 
		) );
		add_action ( 'wp_ajax_search-available-menu-items-customizer', array (
				$this,
				'ajax_search_available_items' 
		) );
		add_action ( 'wp_ajax_customize-nav-menus-insert-auto-draft', array (
				$this,
		...
		add_filter ( 'customize_dynamic_partial_args', array (
				$this,
	 ...
	 *        
	 * @param string $type
	 *        	Optional. Accepts any custom object type and has built-in support for
	...
	public function search_available_items_query($args = array()) {
		...
		if (1 === $args ['pagenum'] && $nav_menus_created_posts_setting && count ( $nav_menus_created_posts_setting ) > 0) {
			$stub_post_query = new WP_Query ( array_merge ( $query, array (
		 ...
		 *        	The array of menu items.
		 * @param array $args
		 *        	Includes 'pagenum' and 's' (search) arguments.
		 */
	...
	public function customize_register() {
		...
		foreach ( $locations as $location => $description ) {
			...
			if ($setting) {
				...
				add_filter ( "customize_sanitize_{$setting_id}", array (
						$this,
						'intval_base10' 
				) );
			} else {
	...
	public function ajax_insert_auto_draft_post() {
		...
		$illegal_params = array_diff ( array_keys ( $params ), array (
				'post_type',
				'post_title' 
		) );
...
<div id="available-menu-items" class="accordion-container">
	...
	<div id="available-menu-items-search"
		class="accordion-section cannot-expand">
		...
		<button type="button" class="clear-results">
			<span class="screen-reader-text"><?php _e( 'Clear Results' ); ?></span>
		</button>
...
<?php
	 ...
	 * @access private
	 *        
	 * @param array $available_item_type
...
<div id="<?php echo esc_attr( $id ); ?>" class="accordion-section">
	<h4 class="accordion-section-title" role="presentation">
				...
				<span class="spinner"></span> <span class="no-items"><?php _e( 'No items' ); ?></span>
	...
	<div class="accordion-section-content customlinkdiv">
		...
		<p class="button-controls">
			<span class="add-to-menu"> <input type="submit"
				...
				name="add-custom-menu-item" id="custom-menu-item-submit"> <span
				class="spinner"></span>
			</span>
		</p>
	</div>
...
<?php
	...
	public function filter_wp_nav_menu_args($args) {
		...
		(! empty ( $args ['theme_location'] ) || (! empty ( $args ['menu'] ) && (is_numeric ( $args ['menu'] ) || is_object ( $args ['menu'] )))) && 
		// ...and if the nav menu would be rendered with a wrapper container element (upon which to attach data-* attributes).
		(! empty ( $args ['container'] ) || (isset ( $args ['items_wrap'] ) && '<' === substr ( $args ['items_wrap'], 0, 1 ))));
