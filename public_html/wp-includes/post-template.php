...
function get_the_content($more_link_text = null, $strip_teaser = false) {
	...
	if (null === $more_link_text) {
		$more_link_text = sprintf ( '<span aria-label="%1$s">%2$s</span>', sprintf(
				...
				__ ( 'Continue reading %s' ), the_title_attribute ( array (
				'echo' => false 
		) ) ), __ ( '(more&hellip;)' ) );
	}
	...
	if (count ( $content ) > 1) {
		if ($more) {
			...
			if (! empty ( $more_link_text ))
				 ...
				 * @since 2.8.0
				 *       
				 * @param string $more_link_element
 ...
 *        
 * @param array $match
 *        	Match array from preg_replace_callback.
 * @return string
 */
 ...
 *        	Optional. Post ID or post object. Defaults to the global `$post`.
 */
...
function get_body_class($class = '') {
	 ...
	 * @param array $class
	 *        	An array of additional classes added to the body.
	 */
...
function post_password_required($post = null) {
	 ...
	 *       
	 * @param bool $required
	 *        	Whether the user needs to supply a password. True if password has not been
...
function wp_dropdown_pages($args = '') {
	 ...
	 *       
	 * @param string $output
	 *        	HTML output for drop down list of pages.
	 * @param array $r
...
function wp_list_pages($args = '') {
	...
	if (! in_array ( $r ['item_spacing'], array (
			'preserve',
			'discard' 
	), true )) {
...
function wp_page_menu($args = array()) {
	...
	if (! in_array ( $args ['item_spacing'], array (
			'preserve',
			'discard' 
	) )) {
		// invalid value, fall back to default.
	 ...
	 *
	 * @param array $args
	 *        	An array of page menu arguments.
	 */
 ...
 *        	Optional. Post ID or post object.
 * @param bool $fullsize
 *        	Optional, default is false. Whether to use full size.
 * @param bool $deprecated
 *        	Deprecated. Not used.
 * @param bool $permalink
 *        	Optional, default is false. Whether to include permalink.
 */
 ...
 * @param bool $icon
 *        	Optional. Whether the attachment is an icon. Default false.
 * @param string|false $text
 *        	Optional. Link text to use. Activated by passing a string, false otherwise.
 *        	Default false.
 * @param array|string $attr
 *        	Optional. Array or string of attributes. Default empty.
...
function wp_get_attachment_link($id = 0, $size = 'thumbnail', $permalink = false, $icon = false, $text = false, $attr = '') {
	 ...
	 *        	Post ID.
	 * @param string|array $size
	 *        	Size of the image. Image size or array of width and height values (in that order).
	 *        	Default 'thumbnail'.
	 * @param bool $permalink
...
function wp_post_revision_title($revision, $link = true) {
	...
	if (! in_array ( $revision->post_type, array (
			'post',
			'page',
			'revision' 
	) ))
...
function wp_post_revision_title_expanded($revision, $link = true) {
	 ...
	 *        	The revision object.
	 * @param bool $link
	 *        	Whether to link to the revisions page, as passed into
