 ...
 *        	Comment type, either user-submitted comment,
 ...
 *        	The ID of the post.
 * @param array $args
 *        	Optional. See WP_Comment_Query::query() for information on accepted arguments.
 ...
 *        	Optional. Array or string of arguments. See WP_Comment_Query::parse_query()
 ...
 *        
 * @param int $post_id
 *        	Optional. Comment amount in post if > 0, else total comments blog wide.
...
function sanitize_comment_cookies() {
	...
	if (isset ( $_COOKIE ['comment_author_email_' . COOKIEHASH] )) {
		 ...
		 * @since 1.5.0
		 *       
		 * @param string $author_email_cookie
	...
	if (isset ( $_COOKIE ['comment_author_url_' . COOKIEHASH] )) {
		 ...
		 * @since 1.5.0
		 *       
		 * @param string $author_url_cookie
 ...
 *        	When true, a disallowed comment will result in the function
...
function wp_allow_comment($commentdata, $avoid_die = false) {
	 ...
	 * @param bool $avoid_die
	 *        	Whether to prevent executing wp_die()
...
function wp_delete_comment($comment_id, $force_delete = false) {
	...
	if (! $force_delete && EMPTY_TRASH_DAYS && ! in_array ( wp_get_comment_status ( $comment ), array (
			'trash',
			'spam' 
	) ))
 ...
 *        	}
 * @param bool $avoid_die
 *        	Should errors be returned as WP_Error objects instead of
...
function wp_set_comment_status($comment_id, $comment_status, $wp_error = false) {
	...
	if (! $wpdb->update ( $wpdb->comments, array (
	 ...
	 * @param int $comment_id
	 *        	Comment ID.
	 * @param string|bool $comment_status
 ...
 *        	Optional. Whether to process previously deferred
...
! is_local_attachment ( $link_test )) : // Also, let's never ping local attachments.
	 ...
	 *       
	 * @param
	 *        	array &$post_links An array of post links to be checked, passed by reference.
	 * @param
	 *        	array &$pung Whether a link has already been pinged, passed by reference.
	 * @param int $post_ID
	 *        	The post ID.
	 */
	do_action_ref_array ( 'pre_ping', array (
			&$post_links,
			&$pung,
			$post->ID 
	) );
	...
	foreach ( ( array ) $post_links as $pagelinkedto ) {
		...
		if ($pingback_server_url) {
			 ...
			 * @since 2.9.0
			 *       
			 * @param string $concat_useragent
			 *        	The user agent concatenated with ' -- WordPress/'
...
function _close_comments_for_old_posts($posts, $query) {
	 ...
	 */
	$post_types = apply_filters ( 'close_comments_for_post_types', array (
			'post' 
	) );
