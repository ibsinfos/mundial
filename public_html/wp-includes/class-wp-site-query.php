...
class WP_Site_Query {
	...
	public function parse_query($query = '') {
		 ...
		 */
		do_action_ref_array ( 'parse_site_query', array (
				&$this 
		) );
	}
	...
	public function get_sites() {
		 ...
		 */
		do_action_ref_array ( 'pre_get_sites', array (
				&$this 
		) );
		 ...
		 *        	WP_Site_Query &$this Current instance of WP_Site_Query, passed by reference.
		 */
		$_sites = apply_filters_ref_array ( 'the_sites', array (
				$_sites,
				&$this 
		) );
	...
	protected function get_site_ids() {
		...
		if (strlen ( $this->query_vars ['search'] )) {
			...
			if ($this->query_vars ['search_columns']) {
				$search_columns = array_intersect ( $this->query_vars ['search_columns'], array (
						'domain',
						'path' 
				) );
			}
		 ...
		 *       
		 * @param array $pieces
		 *        	A compacted array of site query clauses.
		 * @param
		 *        	WP_Site_Query &$this Current instance of WP_Site_Query, passed by reference.
		 */
		$clauses = apply_filters_ref_array ( 'sites_clauses', array (
				compact ( $pieces ),
				&$this 
		) );
	...
	private function set_found_sites() {
		...
		if ($this->query_vars ['number'] && ! $this->query_vars ['no_found_rows']) {
			 ...
			 * @since 4.6.0
			 *       
			 * @param string $found_sites_query
