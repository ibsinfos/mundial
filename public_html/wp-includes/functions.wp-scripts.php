...
function _wp_scripts_maybe_doing_it_wrong($function) {
	...
	_doing_it_wrong ( $function, sprintf(
		...
		__ ( 'Scripts and styles should not be registered or enqueued until the %1$s, %2$s, or %3$s hooks.' ), '<code>wp_enqueue_scripts</code>', '<code>admin_enqueue_scripts</code>', '<code>login_enqueue_scripts</code>' ), '3.3.0' );
}
 ...
 *        	Optional. Whether to add the inline script before the handle
...
function wp_add_inline_script($handle, $data, $position = 'after') {
	...
	if (false !== stripos ( $data, '</script>' )) {
		_doing_it_wrong ( __FUNCTION__, sprintf(
			...
			__ ( 'Do not pass %1$s tags to %2$s.' ), '<code>&lt;script&gt;</code>', '<code>wp_add_inline_script()</code>' ), '4.5.0' );
 ...
 *        	Optional. String specifying script version number, if it has one, which is added to the URL
	...
	if ((is_admin () && 'admin_enqueue_scripts' !== $current_filter) || ('wp-login.php' === $GLOBALS ['pagenow'] && 'login_enqueue_scripts' !== $current_filter)) {
		...
		if (in_array ( $handle, $no )) {
			$message = sprintf(
				...
				__ ( 'Do not deregister the %1$s script in the administration area. To target the front-end theme, use the %2$s hook.' ), "<code>$handle</code>", '<code>wp_enqueue_scripts</code>' );
