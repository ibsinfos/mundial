...
class WP_REST_Server {
	...
	public function check_authentication() {
		 ...
		 *       
		 * @param
		 *        	WP_Error|null|bool WP_Error if authentication error, null if authentication
	...
	public function serve_request($path = null) {
		 ...
		 * @since 4.4.0
		 *       
		 * @param bool $rest_send_nocache_headers
		 ...
		 *            
		 * @param bool $rest_enabled
		 *        	Whether the REST API is enabled. Default true.
		 */
		apply_filters_deprecated ( 'rest_enabled', array (
				true 
		), '4.7.0', 'rest_authentication_errors', __ ( 'The REST API can no longer be completely disabled, the rest_authentication_errors can be used to restrict access to the API, instead.' ) );
		...
		if (! $served) {
			...
			if ($json_error_message) {
				$json_error_obj = new WP_Error ( 'rest_encode_error', $json_error_message, array (
						'status' => 500 
				) );
	 ...
	 *        	Route arguments.
	 * @param bool $override
	 *        	Optional. Whether the route should be overridden if it already exists.
	 *        	Default false.
	 */
	...
	public function get_routes() {
		 ...
		 *        	The available endpoints. An array of matching regex patterns, each mapped
	...
	public function dispatch($request) {
		 ...
		 *       
		 * @param mixed $result
		 *        	Response to replace the requested version with. Can be anything
		...
		return $this->error_to_response ( new WP_Error ( 'rest_no_route', __ ( 'No route was found matching the URL and request method' ), array (
				'status' => 404 
		) ) );
	}
	...
	public function get_namespace_index($request) {
		...
		if (! isset ( $this->namespaces [$namespace] )) {
			return new WP_Error ( 'rest_invalid_namespace', __ ( 'The specified namespace could not be found.' ), array (
					'status' => 404 
			) );
		}
