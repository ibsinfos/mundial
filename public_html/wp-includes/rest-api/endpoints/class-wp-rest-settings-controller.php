...
class WP_REST_Settings_Controller extends WP_REST_Controller {
	...
	public function get_item($request) {
		...
		foreach ( $options as $name => $args ) {
			 ...
			 *       
			 * @param mixed $result
			 *        	Value to use for the requested setting. Can be a scalar
	...
	public function update_item($request) {
		...
		foreach ( $options as $name => $args ) {
			 ...
			 *       
			 * @param bool $result
			 *        	Whether to override the default behavior for updating the
			 *        	value of a setting.
			 * @param string $name
			 *        	Setting name (as shown in REST API responses).
			 * @param mixed $value
			...
			if (is_null ( $request [$name] )) {
				...
				if (! is_scalar ( get_option ( $args ['option_name'], false ) )) {
					return new WP_Error ( 'rest_invalid_stored_value', sprintf ( __ ( 'The %s property has an invalid stored value, and cannot be updated to null.' ), $name ), array (
							'status' => 500 
					) );
				}
