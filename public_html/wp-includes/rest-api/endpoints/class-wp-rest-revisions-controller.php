...
class WP_REST_Revisions_Controller extends WP_REST_Controller {
	...
	protected function get_parent($parent) {
		$error = new WP_Error ( 'rest_post_invalid_parent', __ ( 'Invalid post parent ID.' ), array (
				'status' => 404 
		) );
		if (( int ) $parent <= 0) {
			return $error;
	...
	public function get_items_permissions_check($request) {
		...
		if (! current_user_can ( $parent_post_type_obj->cap->edit_post, $parent->ID )) {
			return new WP_Error ( 'rest_cannot_read', __ ( 'Sorry, you are not allowed to view revisions of this post.' ), array (
	...
	protected function get_revision($id) {
		$error = new WP_Error ( 'rest_post_invalid_id', __ ( 'Invalid revision ID.' ), array (
				'status' => 404 
		) );
		if (( int ) $id <= 0) {
			return $error;
	...
	public function delete_item($request) {
		...
		if (! $force) {
			return new WP_Error ( 'rest_trash_not_supported', __ ( 'Revisions do not support trashing. Set force=true to delete.' ), array (
					'status' => 501 
			) );
		}
		 ...
		 *       
		 * @param (mixed) $result
		 *        	The revision object (if it was deleted or moved to the trash successfully)
