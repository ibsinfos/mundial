...
class WP_REST_Request implements ArrayAccess {
	...
	public function sanitize_params() {
		...
		if ($invalid_params) {
			return new WP_Error ( 'rest_invalid_param', sprintf ( __ ( 'Invalid parameter(s): %s' ), implode ( ', ', array_keys ( $invalid_params ) ) ), array (
					'status' => 400,
	...
	public function has_valid_params() {
		...
		if (! empty ( $required )) {
			return new WP_Error ( 'rest_missing_callback_param', sprintf ( __ ( 'Missing parameter(s): %s' ), implode ( ', ', $required ) ), array (
					'status' => 400,
					'params' => $required 
			) );
		}
	...
	public static function from_url($url) {
		 ...
		 * @since 4.5.0
		 *       
		 * @param WP_REST_Request|false $request
		 *        	Generated request object, or false if URL
		 *        	could not be parsed.
		 * @param string $url
