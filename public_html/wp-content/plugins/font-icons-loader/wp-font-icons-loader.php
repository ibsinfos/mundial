	...
	if (file_exists ( $fil_dir )) {
		...
		foreach ( $dirlist as $val ) {
			if (is_dir ( $fil_dir . $val ) && ! in_array ( $val, array (
				...
				$filelist = scandir ( $fil_dir . $val . '/' );
				foreach ( $filelist as $fval ) {
					if (is_file ( $fil_dir . $val . '/' . $fval ) && end ( explode ( '.', $fval ) ) == 'css') {
						$fonts_activate [$val] = $val . '/' . $fval;
						if (! in_array ( $fonts_activate [$val], $active_fonts ))
							$active_fonts [$val] = $val . '/' . $fval;
					}
				}
			}
		}
							...
							if (isset ( $matches [0] ) && isset ( $matches [1] )) {
								...
								foreach ( $classes as $styleKey => $styleValue ) {
									if (isset ( $values [$styleKey] )) {
										$typicons_icons [] = array (
												'fa ' . substr ( $styleValue, 1, - 1 ) => ucwords ( str_replace ( '-', ' ', $values [$styleKey] ) ) 
										);
									}
								}
							}
						...
						if (isset ( $matches ) && ! empty ( $matches )) {
							if (isset ( $matches [0] ) && isset ( $matches [1] )) {
								...
								foreach ( $classes as $styleKey => $styleValue ) {
									if (isset ( $values [$styleKey] )) {
										$typicons_icons [] = array (
												substr ( $styleValue, 1, - 1 ) => ucwords ( str_replace ( '-', ' ', $values [$styleKey] ) ) 
										);
									}
								}
							}
						...
						if (isset ( $matches ) && ! empty ( $matches )) {
							if (isset ( $matches [0] ) && isset ( $matches [1] )) {
								...
								foreach ( $classes as $styleKey => $styleValue ) {
									if (isset ( $values [$styleKey] )) {
										$typicons_icons [] = array (
												'icon ' . substr ( $styleValue, 1, - 1 ) => ucwords ( str_replace ( '-', ' ', $values [$styleKey] ) ) 
										);
									}
								}
							}
						...
						if (isset ( $matches ) && ! empty ( $matches )) {
							if (isset ( $matches [0] ) && isset ( $matches [1] )) {
								...
								foreach ( $classes as $styleKey => $styleValue ) {
									if (isset ( $values [$styleKey] )) {
										$typicons_icons [] = array (
												substr ( $styleValue, 1 ) => ucwords ( str_replace ( '-', ' ', $values [$styleKey] ) ) 
										);
									}
								}
							}
						...
						if (isset ( $matches ) && ! empty ( $matches )) {
							if (isset ( $matches [0] ) && isset ( $matches [1] )) {
								...
								foreach ( $classes as $styleKey => $styleValue ) {
									if (isset ( $values [$styleKey] )) {
										$typicons_icons [] = array (
												substr ( $styleValue, 1, - 7 ) => ucwords ( str_replace ( '-', ' ', $values [$styleKey] ) ) 
										);
									}
								}
							}
...
<div class="fil-container">
	...
	if (file_exists ( $fil_dir )) {
		...
		foreach ( $dirlist as $val ) {
			if (is_dir ( $fil_dir . $val ) && ! in_array ( $val, array (
				...
				$html = '';
				foreach ( $filelist as $fval ) {
					if (is_file ( $fil_dir . $val . '/' . $fval ) && end ( explode ( '.', $fval ) ) == 'html') {
						$html = $fval;
					}
				}
				foreach ( $filelist as $fval ) {
					if (is_file ( $fil_dir . $val . '/' . $fval ) && end ( explode ( '.', $fval ) ) == 'css') {
						...
						echo '<td>' . $val . '/' . $fval . '<input type="hidden" name="Filpath-' . $val . '" value="' . $val . '/' . $fval . '"></td>' . "\n";
						echo '<td><nobr>' . date ( "m.d.Y H:i:s", filemtime ( $fil_dir . $val ) ) . '</nobr></td>' . "\n";
						echo '<td><a href="' . plugin_dir_url ( __FILE__ ) . FOLDER . $val . '/' . $html . '" target="_blank">' . $html . '</a></td>' . "\n";
						echo '</tr>';
					}
				}
			}
	...
	?>
			...
			<input type="submit" name="Active" value="Activate"> <input
				type="submit" name="Deactive" value="Deactivate"> <input
...
function fonts_actions() {
	if (empty ( $_POST ['Active'] ) && empty ( $_POST ['Deactive'] ) && empty ( $_POST ['Delete'] ))
		return false;
