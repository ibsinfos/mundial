...
class Robo_Disable_Comments {
	...
	public function wp_load_hooks() {
		...
		if (is_admin ()) {
			...
			if ($this->options ['remove_everywhere']) {
				...
				add_action ( 'admin_print_footer_scripts-index.php', array (
						$this,
						'dashboard_js' 
				) );
	...
	public function filter_query() {
		if (is_comment_feed ()) {
			wp_die ( __ ( 'Comments are closed.' ), '', array (
					'response' => 403 
			) );
		}
	}
	...
	public function plugin_actions_links($links, $file) {
		...
		if ($file == 'disable-comments-rb/disable-comments-rb.php' && current_user_can ( 'manage_options' )) {
			array_unshift ( $links, sprintf ( '<a href="%s">%s</a>', esc_attr ( $this->settings_page_url () ), __ ( 'Settings' ) ) );
		}
	...
	public function settings_menu() {
		...
		add_submenu_page ( 'options-general.php', $title, $title, 'manage_options', 'rb_disable_comments_settings', array (
				$this,
				'options' 
		) );
	}
