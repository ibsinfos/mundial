...
abstract class AbstractSignatureMethodTest extends \PHPUnit_Framework_TestCase {
	...
	protected function getRequest() {
		return $this->getMockBuilder ( 'Abraham\TwitterOAuth\Request' )->disableOriginalConstructor ()->getMock ();
	}
	protected function getConsumer($key = null, $secret = null, $callbackUrl = null) {
		return $this->getMockBuilder ( 'Abraham\TwitterOAuth\Consumer' )->setConstructorArgs ( array (
				$key,
				$secret,
				$callbackUrl 
		) )->getMock ();
	...
	protected function getToken($key = null, $secret = null) {
		return $this->getMockBuilder ( 'Abraham\TwitterOAuth\Token' )->setConstructorArgs ( array (
				$key,
				$secret 
		) )->getMock ();
	}
