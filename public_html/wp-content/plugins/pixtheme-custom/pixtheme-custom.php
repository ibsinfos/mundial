	...
	class PixthemeCustom {
		public function __construct() {
			...
			add_action ( 'init', array (
					$this,
					'pixtheme_init_staticblocks' 
			) );
			add_action ( 'init', array (
					$this,
					'pixtheme_portfolio_register' 
			) );
			add_filter ( 'manage_edit-portfolio_columns', array (
					$this,
					'pixtheme_portfolio_edit_columns' 
			) );
			add_action ( 'manage_posts_custom_column', array (
					$this,
					'pixtheme_portfolio_custom_columns' 
			) );
			add_filter ( 'post_type_link', array (
					$this,
					'pixtheme_post_type_link_filter_function' 
			), 1, 3 );
			add_action ( 'admin_menu', array (
					$this,
					'pixtheme_register_portfolio_menu' 
			) );
			add_action ( 'wp_ajax_pixtheme_portfolio_update_post_order', array (
					$this,
					'pixtheme_portfolio_update_post_order' 
			) );
			add_shortcode ( 'share', array (
					$this,
					'pixtheme_pix_share_buttons' 
			) );
			add_action ( 'admin_enqueue_scripts', array (
					$this,
					'pixtheme_custom_admin_enqueue_scripts' 
			) );
		}
		...
		public function pixtheme_register_portfolio_menu() {
			add_submenu_page ( 'edit.php?post_type=portfolio', 'Order portfolio', 'Sort items', 'edit_pages', 'portfolio-order', array (
					$this,
...
<div class="wrap">
	<h2>Sort Items</h2>
				...
				<table id="sortable-table-portfolio"
		class="wp-list-table widefat fixed posts">
		<thead>
			<tr>
		...
		<tbody data-post-type="portfolio">
			<!--?php while( $products--->
				...
				<td class="column-order"><img title=""
					src="<?php echo esc_url(plugins_url( '/img/move-icon-vertical.png', __FILE__ )); ?>"
					alt="Move Icon" height="32" /></td>
...
<pre>
		...
		public function pixtheme_custom_admin_enqueue_scripts() {
			wp_register_script ( 'pixtheme_plugin_admin_script', plugins_url ( '/js/admin.js', __FILE__ ), array (
					'jquery' 
			), '1.0.0', true );
...
add_action ( 'wp_enqueue_scripts', 'pixtheme_custom_styles' );
function pixtheme_custom_styles() {
	wp_register_style ( 'pixtheme_style', plugins_url ( '/css/pix-theme.css', __FILE__ ), array (), '1.0.0', 'screen, all' );
	wp_enqueue_style ( 'pixtheme_style' );
}