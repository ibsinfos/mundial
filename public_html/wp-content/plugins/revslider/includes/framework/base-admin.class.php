...
class RevSliderBaseAdmin extends RevSliderBase {
	...
	private static $allowed_views = array (
			'master-view',
			'system/validation',
			'system/dialog-video',
			'system/dialog-update',
			'system/dialog-global-settings',
			'sliders',
			'slider',
			'slider_template',
			'slides',
	...
	public static function addAdminMenu() {
		...
		foreach ( self::$arrMenuPages as $menu ) {
			...
			$revslider_screens [] = add_menu_page ( $title, $title, $role, 'revslider', array (
					self::$t,
					$pageFunctionName 
			), 'dashicons-update' );
		}
		...
		foreach ( self::$arrSubMenuPages as $menu ) {
			...
			$revslider_screens [] = add_submenu_page ( 'revslider', $title, $title, $role, $pageSlug, array (
					self::$t,
					$pageFunctionName 
			) );
		}
	...
	protected static function addMenuPage($title, $pageFunctionName) {
		self::$arrMenuPages [] = array (
				"title" => $title,
				"pageFunction" => $pageFunctionName 
		);
	}
	...
	protected function addEvent_onActivate($eventFunc = "onActivate") {
		register_activation_hook ( RS_PLUGIN_FILE_PATH, array (
				self::$t,
				$eventFunc 
		) );
	}
	...
	protected static function addActionAjax($ajaxAction, $eventFunction) {
		add_action ( 'wp_ajax_revslider_' . $ajaxAction, array (
				'RevSliderAdmin',
				$eventFunction 
		) );
	}
