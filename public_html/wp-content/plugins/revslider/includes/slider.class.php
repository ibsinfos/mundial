...
class RevSliderSlider extends RevSliderElementsBase {
	...
	public function initByID($sliderID) {
		...
		try {
			$sliderData = $this->db->fetchSingle ( RevSliderGlobals::$table_sliders, $this->db->prepare ( "id = %s", array (
					$sliderID 
			) ) );
		} catch ( Exception $e ) {
	...
	public function initByAlias($alias) {
		try {
			$where = $this->db->prepare ( "alias = %s  AND `type` != 'template'", array (
					$alias 
			) );
	...
	public function isAliasExistsInDB($alias) {
		$where = $this->db->prepare ( "alias = %s ", array (
				$alias 
		) );
		if (! empty ( $this->id )) {
			
			$where .= $this->db->prepare ( " and id != %s  AND `type` != 'template'", array (
					$this->id 
			) );
		}
	...
	public function setHeroSlide($data) {
		...
		$this->updateParam ( array (
				'hero_active' => intval ( $new_slide_id ) 
		) );
	...
	private function createUpdateSliderFromOptions($options, $sliderID = null) {
		...
		if (empty ( $sliderID )) { // create slider
			...
			$arrData ['settings'] = json_encode ( array (
					'version' => 5.0 
			) );
			...
			$sliderID = $this->db->update ( RevSliderGlobals::$table_sliders, $arrData, array (
					"id" => $sliderID 
			) );
		}
	}
	...
	public function deleteSlider() {
		...
		// delete slider
		$this->db->delete ( RevSliderGlobals::$table_sliders, $this->db->prepare ( "id = %s", array (
				$this->id 
		) ) );
	...
	private function deleteAllSlides() {
		...
		$this->db->delete ( RevSliderGlobals::$table_slides, $this->db->prepare ( "slider_id = %s", array (
				$this->id 
		) ) );
	...
	public function deleteStaticSlide() {
		...
		$this->db->delete ( RevSliderGlobals::$table_static_slides, $this->db->prepare ( "slider_id = %s", array (
				$this->id 
		) ) );
	}
	...
	private function duplicateSlider($title = false, $prefix = false) {
		...
		// insert a new slider
		$sqlSelect = $this->db->prepare ( "select " . RevSliderGlobals::FIELDS_SLIDER . " from " . RevSliderGlobals::$table_sliders . " where id = %s", array (
				$this->id 
		) );
		...
		if ($title === false) {
			...
			$sqlTitle = $this->db->fetch ( RevSliderGlobals::$table_sliders, $this->db->prepare ( "alias = %s", array (
					$newSliderAlias 
			) ) );
			if (! empty ( $sqlTitle )) {
				$response = $this->db->fetch ( RevSliderGlobals::$table_sliders );
		...
		$this->db->update ( RevSliderGlobals::$table_sliders, $arrUpdate, array (
				"id" => $lastID 
		) );
		
		// duplicate Slides
		$slides = $this->db->fetch ( RevSliderGlobals::$table_slides, $this->db->prepare ( "slider_id = %s", array (
				$this->id 
		) ) );
		if (! empty ( $slides )) {
		...
		if ($static_id > 0) {
			$slides_static = $this->db->fetch ( RevSliderGlobals::$table_static_slides, $this->db->prepare ( "id = %s", array (
					$static_id 
			) ) );
			...
			foreach ( $slides as $slide ) {
				...
				if ($did_change === true) {
					...
					if ($slide ['id'] == $static_id) {
						$this->db->update ( RevSliderGlobals::$table_static_slides, $arrCreate, array (
								"id" => $static_id 
						) );
					} else {
						$this->db->update ( RevSliderGlobals::$table_slides, $arrCreate, array (
								"id" => $slide ['id'] 
						) );
					}
				}
		...
		if (strpos ( $cus_js, 'revapi' ) !== false) {
			if (preg_match_all ( '/revapi[0-9]*/', $cus_js, $results )) {
				...
				$c_slider->updateParam ( array (
						'custom_javascript' => $cus_js 
				) );
			}
		}
	...
	public function duplicateSlide($slideID) {
		...
		// do duplication
		$sqlSelect = $this->db->prepare ( "select " . RevSliderGlobals::FIELDS_SLIDE . " from " . RevSliderGlobals::$table_slides . " where id = %s", array (
				intval ( $slideID ) 
		) );
	...
	private function shiftOrder($fromOrder) {
		$where = $this->db->prepare ( " slider_id = %s and slide_order >= %s", array (
				$this->id,
				$fromOrder 
		) );
	...
	public function exportSlider($useDummy = false) {
		...
		if (! empty ( $cfw ) && count ( $cfw ) > 0) {
			foreach ( $cfw as $key => $slide ) {
				...
				if (isset ( $slide ['params'] ['image'] ) && $slide ['params'] ['image'] != '')
					$usedImages [$slide ['params'] ['image']] = true; // ['params']['image'] background url
				if (isset ( $slide ['params'] ['background_image'] ) && $slide ['params'] ['background_image'] != '')
					$usedImages [$slide ['params'] ['background_image']] = true; // ['params']['image'] background url
				if (isset ( $slide ['params'] ['slide_thumb'] ) && $slide ['params'] ['slide_thumb'] != '')
					$usedImages [$slide ['params'] ['slide_thumb']] = true; // ['params']['image'] background url
				...
				if (isset ( $slide ['layers'] ) && ! empty ( $slide ['layers'] ) && count ( $slide ['layers'] ) > 0) {
					foreach ( $slide ['layers'] as $lKey => $layer ) {
						...
						if (isset ( $layer ['bgimage_url'] ) && $layer ['bgimage_url'] != '')
							$usedImages [$layer ['bgimage_url']] = true; // image_url if background layer image
		...
		if (! $usepcl) {
			...
			$list = $pclzip->add ( array (
					array (
							PCLZIP_ATT_FILE_NAME => 'slider_export.txt',
			...
			if (! is_wp_error ( $unzipfile )) {
				...
				if ($is_template !== false) {
					if ($uid_check != $is_template) {
						return (array (
								"success" => false,
								"error" => __ ( 'Please select the correct zip file, checksum failed!', 'revslider' ) 
						));
					}
				...
				if (! empty ( $animations )) {
					foreach ( $animations as $key => $animation ) { // $animation['id'], $animation['handle'], $animation['params']
						$exist = $db->fetch ( RevSliderGlobals::$table_layer_anims, $db->prepare ( "handle = %s", array (
								$animation ['handle'] 
						) ) );
						if (! empty ( $exist )) { // update the animation, get the ID
							if ($updateAnim == "true") { // overwrite animation if exists
								...
								$db->update ( RevSliderGlobals::$table_layer_anims, $arrUpdate, array (
...
strpos ( $class, ">" ) !== false) { // .tp-caption>.imageclass or .tp-caption.imageclass>img or .tp-caption.imageclass .img
				...
				if (! empty ( $navigations )) {
					...
					foreach ( $navigations as $key => $navigation ) {
						$exist = $db->fetch ( RevSliderGlobals::$table_navigation, $db->prepare ( "handle = %s", array (
								$navigation ['handle'] 
						) ) );
						unset ( $navigation ['id'] );
						...
						if (! empty ( $exist )) { // create new navigation, get the ID
							if ($updateNavigation == "true") { // overwrite navigation if exists
								...
								$db->update ( RevSliderGlobals::$table_navigation, $navigation, array (
										'handle' => $rh 
								) );
							} else {
						...
						if ($link_slide != false && $link_slide !== 'nothing') { // link to slide/scrollunder is set, move it to actions
							...
							switch ($link_slide) {
								case 'link' :
									...
									$slide ['layers'] [$key] ['layer_action']->action = array (
											'a' => 'link' 
									);
									$slide ['layers'] [$key] ['layer_action']->link_type = array (
											'a' => 'a' 
									);
									$slide ['layers'] [$key] ['layer_action']->image_link = array (
											'a' => $link 
									);
									$slide ['layers'] [$key] ['layer_action']->link_open_in = array (
											'a' => $link_open_in 
									);
								...
								case 'scroll_under' :
									...
									$slide ['layers'] [$key] ['layer_action']->scrollunder_offset = array (
								...
								default : // its an ID, so its a slide ID
									...
									$slide ['layers'] [$key] ['layer_action']->jump_to_slide = array (
											'a' => $slider_map [$link_slide] 
									);
									break;
							}
							$slide ['layers'] [$key] ['layer_action']->tooltip_event = array (
									'a' => 'click' 
							);
						...
						if ($link_slide != false && $link_slide !== 'nothing') { // link to slide/scrollunder is set, move it to actions
							...
							switch ($link_slide) {
								case 'link' :
									...
									$layer ['layer_action']->action = array (
											'a' => 'link' 
									);
									$layer ['layer_action']->link_type = array (
											'a' => 'a' 
									);
									$layer ['layer_action']->image_link = array (
											'a' => $link 
									);
									$layer ['layer_action']->link_open_in = array (
											'a' => $link_open_in 
									);
								...
								case 'scroll_under' :
									...
									$layer ['layer_action']->scrollunder_offset = array (
											'a' => $scrollunder_offset 
									);
								...
								default : // its an ID, so its a slide ID
									...
									$layer ['layer_action']->jump_to_slide = array (
											'a' => $slider_map [$link_slide] 
									);
									break;
							}
							$layer ['layer_action']->tooltip_event = array (
									'a' => 'click' 
							);
			...
			if ($is_template !== false) { // duplicate the slider now, as we just imported the "template"
				if ($single_slide !== false) { // add now one Slide to the current Slider
					...
					if ($changed) {
						...
						return (array (
								"success" => false,
								"error" => __ ( 'could not find correct Slide to copy, please try again.', 'revslider' ),
								"sliderID" => $sliderID 
						));
					}
				} else {
					...
					$real_slider_id = $mslider->duplicateSliderFromData ( array (
							'sliderid' => $sliderID,
							'title' => $talias 
					) );
				}
	...
	public function updateParam($arrUpdate) {
		...
		$this->db->update ( RevSliderGlobals::$table_sliders, $arrUpdateDB, array (
				"id" => $this->id 
		) );
	}
	...
	public function getUsedFonts($full = false) {
		...
		if ($staticID !== false) {
			...
			if ($msl->getID () !== '') {
				$mslides = array_merge ( $mslides, array (
						$msl 
				) );
			}
		}
	...
	public function getSlidesFromPosts($publishedOnly = false, $gal_ids = array()) {
		...
		if (! empty ( $gal_ids ))
			$sourceType = 'specific_posts'; // change to specific posts, give the gal_ids to the list
		...
		switch ($sourceType) {
			...
			case "current_post" :
				global $post;
				$arrPosts = $this->getPostsFromSpecificList ( array (
						"",
						$post->ID 
				) );
				break;
	...
	public function getSlidesFromStream($publishedOnly = false) {
			...
			case "facebook" :
				...
				if (! empty ( $arrPosts )) {
					foreach ( $arrPosts as $k => $p ) {
						...
						if (in_array ( $p->status_type, array (
								"wall_post" 
						) ))
							unset ( $arrPosts [$k] );
					}
		...
		foreach ( $arrPosts as $postData ) {
			if (empty ( $postData ))
				continue; // ignore empty entries, like from instagram
	...
	public function getFirstSlideIdFromGallery() {
		...
		$arrSlides = array ();
		$arrSlideRecords = $this->db->fetch ( RevSliderGlobals::$table_slides, $this->db->prepare ( "slider_id = %s", array (
				$this->id 
		) ), "slide_order", '', ' LIMIT 0,1' );
	...
	public function getSlidesFromGallery($publishedOnly = false, $allwpml = false, $first = false) {
		...
		foreach ( $arrSlideRecords as $record ) {
			...
			if ($first)
				break; // we only want the first slide!
		}
	...
	public function getSlidesCountFromGallery($publishedOnly = false) {
		...
		$sqlSelect = $this->db->prepare ( "select `id` from " . RevSliderGlobals::$table_slides . " where `slider_id` = %s", array (
				$this->id 
		) );
