...
class RevSliderSlide extends RevSliderElementsBase {
	...
	public function setImageByID($imageID, $size = 'full') {
		$a = apply_filters ( 'revslider_slide_setImageByID', array (
				'imageID' => $imageID,
				'size' => $size 
		), $this );
	...
	public function initByStreamData($postData, $slideTemplate, $sliderID, $sourceType, $additions) {
		$a = apply_filters ( 'revslider_slide_initByStreamData', array (
				'postData' => $postData,
	...
	private function initByFacebook($sliderID, $additions) {
		...
		if (isset ( $this->params ['enable_link'] ) && $this->params ['enable_link'] == "true" && isset ( $this->params ['link_type'] ) && $this->params ['link_type'] == "regular") {
			...
			$this->params ["link"] = str_replace ( array (
					"%link%",
					'{{link}}' 
	...
	private function initByInstagram($sliderID) {
		...
		if (empty ( $link ))
			$link = 'https://www.instagram.com/p/' . RevSliderFunctions::getVal ( $this->postData, 'code' );
	...
	private function setLayersByStreamData($sliderID, $stream_type, $additions = array()) {
		$a = apply_filters ( 'revslider_slide_setLayersByStreamData_pre', array (
				'arrLayers' => $this->arrLayers,
				'params' => $this->params 
		), $sliderID, $stream_type, $additions, $this );
		...
		$a = apply_filters ( 'revslider_slide_setLayersByStreamData_post', array (
				'arrLayers' => $this->arrLayers,
				'params' => $this->params 
		), $sliderID, $stream_type, $additions, $this );
	...
	public function set_stream_data($text, $attr, $stream_type, $additions = array(), $is_action = false) {
		...
		$text = str_replace ( array (
				'%date_modified%',
				'{{date_modified}}' 
		), $attr ['date_modified'], $text );
		$text = str_replace ( array (
				'%author_name%',
				'{{author_name}}' 
		), $attr ['author_name'], $text );
		$text = str_replace ( array (
				'%num_comments%',
				'{{num_comments}}' 
		), $attr ['num_comments'], $text );
		$text = str_replace ( array (
				'%catlist%',
				'{{catlist}}' 
		), $attr ['catlist'], $text );
		...
		$text = str_replace ( array (
				'%retweet_count%',
				'{{retweet_count}}' 
		), $attr ['retweet_count'], $text );
		$text = str_replace ( array (
				'%favorite_count%',
				'{{favorite_count}}' 
		), $attr ['favorite_count'], $text );
		$text = str_replace ( array (
				'%views%',
				'{{views}}' 
		), $attr ['views'], $text );
		...
		foreach ( $arrMatches as $matched ) {
			foreach ( $matched as $match ) {
				...
				if (count ( $vals ) !== 2)
					continue; // not correct values
				$vals [1] = intval ( $vals [1] ); // get real number
				if ($vals [1] === 0 || $vals [1] < 0)
					continue; // needs to be at least 1
				...
				foreach ( $img_sizes as $img_handle => $img_name ) {
					...
					if ($additions ['fb_type'] == 'album') {
						$text = str_replace ( array (
								...
								'{{image_url_' . $img_handle . '}}' 
						), $attr ['img_urls'] [$img_handle] ['url'], $text );
						$text = str_replace ( array (
								'%image_' . $img_handle . '%',
								'{{image_' . $img_handle . '}}' 
						), $attr ['img_urls'] [$img_handle] ['tag'], $text );
					} else {
						$text = str_replace ( array (
								'%image_url_' . $img_handle . '%',
								'{{image_url_' . $img_handle . '}}' 
						), $attr ['img_urls'] ['url'], $text );
						$text = str_replace ( array (
								'%image_' . $img_handle . '%',
								'{{image_' . $img_handle . '}}' 
						), $attr ['img_urls'] ['tag'], $text );
					}
				}
				break;
			case 'youtube' :
	...
	public function return_stream_data($stream_type, $additions = array()) {
		...
		switch ($stream_type) {
			...
			case 'flickr' :
				...
				$attr ['img_urls'] = @array (
						'square' => array (
								'url' => RevSliderFunctions::getVal ( $this->postData, 'url_sq' ),
								'tag' => '<img src="' . RevSliderFunctions::getVal ( $this->postData, 'url_sq' ) . '" width="' . RevSliderFunctions::getVal ( $this->postData, 'width_sq' ) . '" height="' . RevSliderFunctions::getVal ( $this->postData, 'height_sq' ) . '" data-no-retina />' 
						),
						'large-square' => array (
								'url' => RevSliderFunctions::getVal ( $this->postData, 'url_q' ),
								'tag' => '<img src="' . RevSliderFunctions::getVal ( $this->postData, 'url_q' ) . '" width="' . RevSliderFunctions::getVal ( $this->postData, 'width_q' ) . '" height="' . RevSliderFunctions::getVal ( $this->postData, 'height_q' ) . '"  data-no-retina />' 
						),
						'thumbnail' => array (
								'url' => RevSliderFunctions::getVal ( $this->postData, 'url_t' ),
								'tag' => '<img src="' . RevSliderFunctions::getVal ( $this->postData, 'url_t' ) . '" width="' . RevSliderFunctions::getVal ( $this->postData, 'width_t' ) . '" height="' . RevSliderFunctions::getVal ( $this->postData, 'height_t' ) . '"  data-no-retina />' 
						),
						'small' => array (
								'url' => RevSliderFunctions::getVal ( $this->postData, 'url_s' ),
								'tag' => '<img src="' . RevSliderFunctions::getVal ( $this->postData, 'url_s' ) . '" width="' . RevSliderFunctions::getVal ( $this->postData, 'width_s' ) . '" height="' . RevSliderFunctions::getVal ( $this->postData, 'height_s' ) . '"  data-no-retina />' 
						),
						'small-320' => array (
								'url' => RevSliderFunctions::getVal ( $this->postData, 'url_n' ),
								'tag' => '<img src="' . RevSliderFunctions::getVal ( $this->postData, 'url_n' ) . '" width="' . RevSliderFunctions::getVal ( $this->postData, 'width_n' ) . '" height="' . RevSliderFunctions::getVal ( $this->postData, 'height_n' ) . '"  data-no-retina />' 
						),
						'medium' => array (
								'url' => RevSliderFunctions::getVal ( $this->postData, 'url_m' ),
								'tag' => '<img src="' . RevSliderFunctions::getVal ( $this->postData, 'url_m' ) . '" width="' . RevSliderFunctions::getVal ( $this->postData, 'width_m' ) . '" height="' . RevSliderFunctions::getVal ( $this->postData, 'height_m' ) . '"  data-no-retina />' 
						),
						'medium-640' => array (
								'url' => RevSliderFunctions::getVal ( $this->postData, 'url_z' ),
								'tag' => '<img src="' . RevSliderFunctions::getVal ( $this->postData, 'url_z' ) . '" width="' . RevSliderFunctions::getVal ( $this->postData, 'width_z' ) . '" height="' . RevSliderFunctions::getVal ( $this->postData, 'height_z' ) . '"  data-no-retina />' 
						),
						'medium-800' => array (
								'url' => RevSliderFunctions::getVal ( $this->postData, 'url_c' ),
								'tag' => '<img src="' . RevSliderFunctions::getVal ( $this->postData, 'url_c' ) . '" width="' . RevSliderFunctions::getVal ( $this->postData, 'width_c' ) . '" height="' . RevSliderFunctions::getVal ( $this->postData, 'height_c' ) . '"  data-no-retina />' 
						),
						'large' => array (
								'url' => RevSliderFunctions::getVal ( $this->postData, 'url_l' ),
								'tag' => '<img src="' . RevSliderFunctions::getVal ( $this->postData, 'url_l' ) . '" width="' . RevSliderFunctions::getVal ( $this->postData, 'width_l' ) . '" height="' . RevSliderFunctions::getVal ( $this->postData, 'height_l' ) . '"  data-no-retina />' 
						),
						'original' => array (
								'url' => RevSliderFunctions::getVal ( $this->postData, 'url_o' ),
								'tag' => '<img src="' . RevSliderFunctions::getVal ( $this->postData, 'url_o' ) . '" width="' . RevSliderFunctions::getVal ( $this->postData, 'width_o' ) . '" height="' . RevSliderFunctions::getVal ( $this->postData, 'height_o' ) . '"  data-no-retina />' 
						) 
				);
				break;
			case 'youtube' :
	...
	public function set_post_data($text, $attr, $post_id) {
		...
		foreach ( $img_sizes as $img_handle => $img_name ) {
			...
			$text = str_replace ( array (
					'%featured_image_url_' . $img_handle . '%',
					'{{featured_image_url_' . $img_handle . '}}' 
			), $url, $text );
			$text = str_replace ( array (
					'%featured_image_' . $img_handle . '%',
					'{{featured_image_' . $img_handle . '}}' 
			), $tag, $text );
		}
		...
		foreach ( $arrMatches as $matched ) {
			foreach ( $matched as $match ) {
				...
				$avatar = get_avatar_url ( $authorID, array (
						"size" => $vals [0] 
				) );
		...
		if (RevSliderWooCommerce::isWooCommerceExists ()) {
			...
			if ($product !== false) {
				...
				if (strpos ( $text, 'wc_add_to_cart_button' ) !== false) {
					...
					if ($ajax_cart_en) {
						wp_enqueue_script ( 'wc-add-to-cart', $frontend_script_path . 'add-to-cart' . $suffix . '.js', array (
								'jquery' 
						), WC_VERSION, true );
				...
				$text = str_replace ( array (
						'%wc_stock_quantity%',
						'{{wc_stock_quantity}}' 
				), $wc_stock_quantity, $text );
				$text = str_replace ( array (
						'%wc_rating_count%',
						'{{wc_rating_count}}' 
				), $wc_rating_count, $text );
				$text = str_replace ( array (
						'%wc_review_count%',
						'{{wc_review_count}}' 
				), $wc_review_count, $text );
				$text = str_replace ( array (
						'%wc_tags%',
						'{{wc_tags}}' 
				), $wc_tags, $text );
	...
	public function getDataByID($slideid) {
		...
		if (strpos ( $slideid, 'static_' ) !== false) {
			...
			$record = $this->db->fetch ( RevSliderGlobals::$table_static_slides, $this->db->prepare ( "slider_id = %s", array (
					$sliderID 
			) ) );
			if (! empty ( $record )) {
				$return = $record [0];
			...
			$record = $this->db->fetchSingle ( RevSliderGlobals::$table_slides, $this->db->prepare ( "id = %d", array (
					$slideid 
			) ) );
			$return = $record;
		}
	...
	public static function isSlideByID($slideid) {
		...
		try {
			if (strpos ( $slideid, 'static_' ) !== false) {
				...
				$record = $db->fetch ( RevSliderGlobals::$table_static_slides, $db->prepare ( "slider_id = %s", array (
						$sliderID 
				) ) );
				...
				$record = $db->fetchSingle ( RevSliderGlobals::$table_slides, $db->prepare ( "id = %s", array (
						$slideid 
				) ) );
	...
	public function initByStaticID($slideid) {
		RevSliderFunctions::validateNumeric ( $slideid, "Slide ID" );
		$record = $this->db->fetchSingle ( RevSliderGlobals::$table_static_slides, $this->db->prepare ( "id = %s", array (
				$slideid 
		) ) );
	...
	/**
	 * get parameter from params array.
	 * if no default, then the param is a must!
	 */
	...
	protected function updateParamsInDB($arrUpdate = array(), $static = false) {
		...
		if ($static === false) {
			$this->db->update ( RevSliderGlobals::$table_slides, $arrDBUpdate, array (
					"id" => $this->id 
			) );
		} else {
			
			$this->db->update ( RevSliderGlobals::$table_static_slides, $arrDBUpdate, array (
					"id" => $static 
			) );
		}
	}
	...
	public function updateSlideFromData($data) {
		...
		$this->db->update ( RevSliderGlobals::$table_slides, $arrUpdate, array (
				"id" => $this->id 
		) );
	...
	public function updateStaticSlideFromData($data) {
		...
		$this->db->update ( RevSliderGlobals::$table_static_slides, $arrUpdate, array (
				"id" => $this->id 
		) );
	...
	public function deleteSlide() {
		...
		$this->db->delete ( RevSliderGlobals::$table_slides, $this->db->prepare ( "id = %s", array (
				$this->id 
		) ) );
			...
			foreach ( $this->arrLayers as $key => $layer ) {
				...
				foreach ( $all_fonts as $f ) {
					if (strtolower ( str_replace ( array (
							...
							"'",
							' ' 
					), '', $f ['label'] ) ) == strtolower ( str_replace ( array (
							'"',
							"'",
							' ' 
					), '', $font ) ) && $f ['type'] == 'googlefont') {
				...
				foreach ( $translation as $trans ) {
					if (isset ( $layers [$l] [$trans] )) {
						if (! is_array ( $layers [$l] [$trans] )) {
							$layers [$l] [$trans] = array (
									'desktop' => $layers [$l] [$trans] 
							);
						}
					}
				}
