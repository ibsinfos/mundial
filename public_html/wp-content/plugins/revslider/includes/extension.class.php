...
class RevSliderExtension {
	...
	public function init_essential_grid_extensions() {
		if (! class_exists ( 'Essential_Grid' ))
			return false; // only add if Essential Grid is installed
		
		add_filter ( 'essgrid_set_ajax_source_order', array (
				$this,
				'add_slider_to_eg_ajax' 
		) );
		add_filter ( 'essgrid_handle_ajax_content', array (
				$this,
				'set_slider_values_to_eg_ajax' 
		), 10, 4 );
		add_action ( 'essgrid_add_meta_options', array (
				$this,
				'add_eg_additional_meta_field' 
		) );
		add_action ( 'essgrid_save_meta_options', array (
				$this,
				'save_eg_additional_meta_field' 
		), 10, 2 );
		...
		add_action ( 'admin_head', array (
				$this,
				'add_eg_additional_inline_javascript' 
		) );
	...
	public function add_slider_to_eg_ajax($media) {
		$media ['revslider'] = array (
				'name' => __ ( 'Slider Revolution', 'revslider' ),
				'type' => 'ccw' 
		);
...
<p>
	<select name="eg_sources_revslider" id="eg_sources_revslider">
		...
		if (! empty ( $shortcodes )) {
			...
			<?php selected($selected_slider[0], $id); ?>><?php echo $name; ?></option>
						<?php
			}
		}
		?>
