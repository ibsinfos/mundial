...
class RevSliderTinyBox {
		...
		if ((isset ( $_GET ['action'] ) && $_GET ['action'] == 'edit') || (isset ( $_GET ['vc_action'] ) && $_GET ['vc_action'] == 'vc_inline') || $pagenow == 'post-new.php') {
			...
			if (get_user_option ( 'rich_editing' ) == 'true') {
				add_filter ( 'mce_external_plugins', array (
						'RevSliderTinyBox',
						'add_tinymce_shortcode_editor_plugin' 
				) );
				add_filter ( 'mce_buttons', array (
						'RevSliderTinyBox',
						'add_tinymce_shortcode_editor_button' 
				) );
			}
			
			add_action ( 'in_admin_footer', array (
					'RevSliderTinyBox',
					'add_tiny_mce_shortcode_dialog' 
			) );
		} else {
			return;
		}
	...
	public static function add_to_VC() {
		...
		if (version_compare ( WPB_VC_VERSION, '4.4', '>=' )) { // use if 4.4 or newer
			vc_add_shortcode_param ( 'rev_slider_shortcode', array (
					'RevSliderTinyBox',
					'revslider_shortcode_settings_field' 
			) );
		} else { // use if older than 4.4
			add_shortcode_param ( 'rev_slider_shortcode', array (
					'RevSliderTinyBox',
					'revslider_shortcode_settings_field' 
			) );
		}
	...
	public static function revslider_shortcode_settings_field($settings, $value) {
		return '<div class="rev_slider_shortcode_block">' . '<input id="rs-vc-input-' . $settings ['param_name'] . '" name="' . $settings ['param_name'] . '" class="wpb_vc_param_value wpb-textinput ' . $settings ['param_name'] . ' ' . $settings ['type'] . '_field" type="text" value="' . $value . '" />' . '</div>';
	}
	...
	public static function add_tiny_mce_shortcode_dialog() {
		?>
<div id="revslider-tiny-mce-dialog" tabindex="-1" action="" title=""
	style="display: none;">
	<form id="revslider-tiny-mce-settings-form" action="">
		...
		?>
				...
				<span class="revslider-step-actions-wrapper"> <a
					href="javascript:void(0);" class="button-primary nonclickable"
					id="rs-modify-predefined-slider"><span
						class="dashicons dashicons-admin-generic"></span><?php _e('Quick Modify Slider', 'revslider'); ?></a>
					<a href="javascript:void(0);" class="button-primary nonclickable"
					id="rs-add-predefined-slider"><span
						class="dashicons dashicons-migrate"></span><?php _e('Add Selected Slider', 'revslider'); ?></a>
				</span> <span style="clear: both; width: 100%; display: block"></span>
			</div>
		</div>
		...
		<div id="revslider-tiny-dialog-step-1-5" style="display: none;">
			<p class="revslider-quicktitle"></p>
			<div id="revslider-tiny-settings-wrap"
				class="revslider-quick-inner-wrapper"
			...
			<div class="revslider-stepnavigator">
				<span class="revslider-currentstep"><strong><?php _e('STEP 2', 'revslider'); ?></strong><?php _e('Quick Modify / Add Slider', 'revslider'); ?></span>
				<span class="revslider-step-actions-wrapper"> <a
					href="javascript:void(0);" class="button-primary  rs-goto-step-1"><span
						class="dashicons dashicons-arrow-left-alt"></span><?php _e('Select Slider', 'revslider'); ?></a>
...
<?php
	...
	public static function add_step_1_markup() {
		...
		?>
<div class="revslider-quick-inner-wrapper"
	style="padding-right: 0px; padding-bottom: 0px;">
			...
			foreach ( $sliders as $slider ) {
				...
				if (! empty ( $slides )) {
					...
					foreach ( $slides as $slide ) {
						...
						if ($active_slide == - 1)
							$active_slide = - 99; // do this so that we are hero, only the first slide will be active if no hero slide was yet set
					}
				}
	...
	<ul id="rs-shortcode-select-wrapper">
		<li class="rs-slider-modify-li rs-slider-modify-new-slider"><a
			href="<?php echo RevSliderBaseAdmin::getViewUrl(RevSliderAdmin::VIEW_SLIDER); ?>"
			target="_blank" class="button-primary revgreen"
			id="rs-create-predefined-slider"><span
		...
		if (! empty ( $sliders_info )) {
			...
			data-slideralias="<?php echo esc_attr($values['slideralias']); ?>"><span
			class="mini-transparent mini-as-bg"></span>
			<div
				class="rs-slider-modify-container <?php echo $values['bg_extraClass']; ?>"
				<?php echo $values['bg_fullstyle']; ?>></div> <i
			class="slide-link-forward eg-icon-forward"></i> <span
