...
class RevSliderOutput {
	...
	private function filterOneSlide($slides) {
		...
		if (strpos ( $oneSlideID, 'static_' ) !== false) {
			...
			$tmp_slides = $wpdb->get_results ( $wpdb->prepare ( "SELECT * FROM " . RevSliderGlobals::$table_static_slides . " WHERE slider_id = %s", array (
					$sliderID 
			) ), ARRAY_A );
...
<?php
		...
		if (count ( $slides ) == 0)
			return false; // No Slides added yet
		...
		foreach ( $slides as $slide ) {
			...
			if (trim ( $urlSlideImage ) == '')
				$urlSlideImage = $urlImageTransparent; // go back to transparent if img is empty
			...
			if ($post_based_slider) { // check if we are post based or normal slider
				...
				$slide_description = ' data-description="' . str_replace ( array (
						"\'",
						'\"' 
				), array (
						"'",
				...
				$slide_description = ' data-description="' . str_replace ( array (
						...
						"'",
						'"' 
				), esc_attr ( $slide->getParam ( 'slide_description', '' ) ) ) . '"';
			}
			...
			$this->enabled_sizes = array (
					'desktop' => 'on',
					'notebook' => $enable_custom_size_notebook,
					'tablet' => $enable_custom_size_tablet,
					'mobile' => $enable_custom_size_iphone 
			);
	...
	public function putBackgroundVideo($slide) {
		...
		if ($video_added == false)
			return false; // return here if no valid video was found.
	...
	public function putLayer($layer, $row_group_uid = false, $static_slide = false, $special_type = false) {
		...
		$blur_write_all = ($blurfilter != 0 && $blurfilter != '0px' || $hover_blurfilter != 0 && $hover_blurfilter != '0px' || $anim_blurfilter_start != 0 && $anim_blurfilter_start != '0px' || $anim_blurfilter_end != 0 && $anim_blurfilter_end != '0px') ? true : false;
		
		$grayscale_write_all = ($grayscalefilter != 0 && $grayscalefilter != '0%' || $hover_grayscalefilter != 0 && $hover_grayscalefilter != '0%' || $anim_grayscalefilter_start != 0 && $anim_grayscalefilter_start != '0%' || $anim_grayscalefilter_end != 0 && $anim_grayscalefilter_end != '0%') ? true : false;
		
		$brightness_write_all = ($brightnessfilter != 100 && $brightnessfilter != '100%' || $hover_brightnessfilter != 100 && $hover_brightnessfilter != '100%' || $anim_brightnessfilter_start != 100 && $anim_brightnessfilter_start != '100%' || $anim_brightnessfilter_end != 100 && $anim_brightnessfilter_end != '100%') ? true : false;
		...
		switch ($type) {
			...
			case 'column' :
				...
				if ($urlImage !== '') {
					...
					$urlImage = $objlib->get_correct_size_url ( $urlImage, array (
							'SET',
							'SET' 
					), $imglibsize ); // check for size to be used
				...
				if ($this->adv_resp_sizes == true) {
					...
					if ($scaleX !== '') {
						if (! is_object ( $scaleX )) {
							...
							$myScaleX = RevSliderFunctions::normalize_device_settings ( $scaleX, $this->enabled_sizes, 'html-array', array (
									'NaNpx' => '',
									'auto' => '' 
							) );
							if ($myScaleX == "['','','','']")
					...
					if ($scaleY !== '') {
						if (! is_object ( $scaleY )) {
							...
							$myScaleY = RevSliderFunctions::normalize_device_settings ( $scaleY, $this->enabled_sizes, 'html-array', array (
									'NaNpx' => '',
									'auto' => '' 
							) );
							if ($myScaleY == "['','','','']")
				...
				foreach ( $margin_vals as $margin ) {
					if (! empty ( $margin )) {
						foreach ( $margin as $mkey => $mar ) {
							$rl_margin [$mkey] [] = str_replace ( array (
									'px',
									'%' 
							), '', $mar );
						}
			...
			foreach ( $padding_vals as $padding ) {
				if (! empty ( $padding )) {
					foreach ( $padding as $mkey => $mar ) {
						$rl_padding [$mkey] [] = str_replace ( array (
								'px',
								'%' 
						), '', $mar );
					}
		...
		if ($show_on_hover === 'show-on-hover' || is_array ( $show_on_hover ) && $show_on_hover [0] == 'show-on-hover')
			$show_on_hover = false; // fix for layers that occured in version 5.2.2 and 5.2.3
		...
		$st_idle = array ();
		$def ['o'] = array (
				RevSliderFunctions::getVal ( $def_val, 'opacity', '0' ),
				'0' 
		);
		$def ['sX'] = array (
				RevSliderFunctions::getVal ( $def_val, 'scalex', '1' ),
				'1' 
		);
		$def ['sY'] = array (
				RevSliderFunctions::getVal ( $def_val, 'scaley', '1' ),
				'1' 
		);
		$def ['skX'] = array (
				RevSliderFunctions::getVal ( $def_val, 'skewx', '0' ),
				'0' 
		);
		$def ['skY'] = array (
				RevSliderFunctions::getVal ( $def_val, 'skewy', '0' ),
				'0' 
		);
		$def ['rX'] = array (
				RevSliderFunctions::getVal ( $def_val, 'xrotate', '0' ),
				'0' 
		);
		$def ['rY'] = array (
				RevSliderFunctions::getVal ( $def_val, 'yrotate', '0' ),
				'0' 
		);
		$def ['rZ'] = array (
				RevSliderFunctions::getVal ( $layer, '2d_rotation', '0' ),
				'0' 
		);
		...
		$def ['tP'] = array (
				RevSliderFunctions::getVal ( $def_val, 'pers', '600' ),
				'600' 
		);
		$def ['z'] = array (
				RevSliderFunctions::getVal ( $def_val, 'z', '0' ),
				'0' 
		);
		
		$st_idle ['font-family'] = array (
				str_replace ( '"', "'", RevSliderFunctions::getVal ( $def_val, 'font-family', '' ) ),
				str_replace ( '"', "'", $dff ) 
		);
		// $st_idle['text-align'] = array(RevSliderFunctions::getVal($def_val, 'text-align', 'inherit'), 'ALWAYS'); //$dta
		$st_idle ['text-transform'] = array (
				RevSliderFunctions::getVal ( $def_val, 'text-transform', 'left' ),
				$dttr 
		);
		...
		$st_idle ['font-style'] = array (
				$font_style,
				$dfs 
		);
		$st_idle ['text-decoration'] = array (
				RevSliderFunctions::getVal ( $def_val, 'text-decoration', 'none' ),
				$dtd 
		);
		...
		if (strpos ( $calc_bg_color, "gradient" ) !== false || strpos ( $calc_dbc_color, "gradient" ) !== false)
			$st_idle ['background'] = array (
					TPColorpicker::convert ( $bg_color, $bg_trans ),
					TPColorpicker::convert ( $dbc, $dbt ) 
			); // 'ALWAYS'
		
		else
			$st_idle ['background-color'] = array (
					TPColorpicker::convert ( $bg_color, $bg_trans ),
					TPColorpicker::convert ( $dbc, $dbt ) 
			); // 'ALWAYS'
		...
		$dbot = intval ( $dbot );
		$st_idle ['border-color'] = array (
				TPColorpicker::convert ( $border_color, $border_trans ),
				TPColorpicker::convert ( $dboc, $dbot ) 
		); // 'ALWAYS'
		                                                                                                                              // }
		
		$st_idle ['border-style'] = array (
				RevSliderFunctions::getVal ( $def_val, 'border-style', 'none' ),
				$dbs 
		);
		...
		$st_idle ['border-width'] = array (
				$bdw,
				$dbw 
		);
		
		$my_border = RevSliderFunctions::getVal ( $def_val, 'border-width', array (
				'0px',
				'0px',
				'0px',
				'0px' 
		...
		$my_border = RevSliderFunctions::getVal ( $def_val, 'border-radius', array (
				'0px',
				'0px',
				'0px',
				'0px' 
		...
		if ($is_hover_active) {
			...
			$def_h ['o'] = array (
					RevSliderFunctions::getVal ( $def_val_h, 'opacity', '0' ),
					'0' 
			);
			$def_h ['sX'] = array (
					RevSliderFunctions::getVal ( $def_val_h, 'scalex', '1' ),
					'1' 
			);
			$def_h ['sY'] = array (
					RevSliderFunctions::getVal ( $def_val_h, 'scaley', '1' ),
					'1' 
			);
			$def_h ['skX'] = array (
					RevSliderFunctions::getVal ( $def_val_h, 'skewx', '0' ),
					'0' 
			);
			$def_h ['skY'] = array (
					RevSliderFunctions::getVal ( $def_val_h, 'skewy', '0' ),
					'0' 
			);
			$def_h ['rX'] = array (
					RevSliderFunctions::getVal ( $def_val_h, 'xrotate', '0' ),
					'0' 
			);
			$def_h ['rY'] = array (
					RevSliderFunctions::getVal ( $def_val_h, 'yrotate', '0' ),
					'0' 
			);
			$def_h ['rZ'] = array (
					RevSliderFunctions::getVal ( $def_val_h, '2d_rotation', '0' ),
					'inherit' 
			);
			$def_h ['z'] = array (
					RevSliderFunctions::getVal ( $def_val_h, 'z', '0' ),
					'0' 
			);
			...
			if ($font_color !== 'transparent') {
				...
				$st_hover ['c'] = array (
						TPColorpicker::convert ( $font_color, $font_trans ),
						'ALWAYS' 
				);
			} else {
				$st_hover ['c'] = array (
			...
			$st_hover ['td'] = array (
					RevSliderFunctions::getVal ( $def_val_h, 'text-decoration', 'none' ),
					'none' 
			);
			...
			$st_hover ['zi'] = array (
					RevSliderFunctions::getVal ( $def_val_h, 'zindex', 'auto' ),
					'auto' 
			);
			...
			$st_hover ['bs'] = array (
					RevSliderFunctions::getVal ( $def_val_h, 'border-style', 'none' ),
					'none' 
			);
			$my_border_width = RevSliderFunctions::getVal ( $def_val_h, 'border-width', array (
					'0px',
					'0px',
					'0px',
					'0px' 
			...
			if (! empty ( $my_border_width ) && is_array ( $my_border_width )) {
				...
				$brsthoverdw = (isset ( $st_hover ['border-width'] ) && $st_hover ['border-width'] !== '0px 0px 0px 0px') ? 'ALWAYS' : '0px 0px 0px 0px';
				$st_hover ['bw'] = array (
						$my_border_width . ' ' . $my_border_width . ' ' . $my_border_width . ' ' . $my_border_width,
						$brsthoverdw 
				);
			}
			
			$my_border = RevSliderFunctions::getVal ( $def_val_h, 'border-radius', array (
					'0px',
					'0px',
					'0px',
					'0px' 
		...
		if ($is_hover_active && ($ibw_h == false || $ibs_h == false) && ($ibw == false || $ibs == false) || $is_hover_active == false && ($ibw == false || $ibs == false)) {
	...
	public function set_placeholders($text) {
		...
		if (isset ( $post->ID ))
			$text = str_replace ( array (
					'%current_page_link%',
					'{{current_page_link}}' 
			), get_permalink ( $post->ID ), $text );
		$text = str_replace ( array (
				'%home_url%',
				'{{home_url}}' 
		), esc_url ( home_url ( '/' ) ), $text );
		if (isset ( $post->ID ))
		...
		foreach ( $zones as $zs => $zone ) { // go through all three zones
			...
			foreach ( $layers as $layer ) {
				if (RevSliderFunctions::getVal ( $layer, 'type', 'text' ) !== 'row')
					continue; // we only want to handle rows here to get the zones we need to create
				...
				if ($layer_zone !== $zone)
					continue; // wrong zones, so continue
	...
	public function putCreativeRows($layers, $current_zone) {
		...
		foreach ( $layers as $layer ) {
			if (RevSliderFunctions::getVal ( $layer, 'type', 'text' ) !== 'row')
				continue; // we only want to handle rows here of the current zone and add them as a rows
	...
	public function putCreativeColumn($layers, $unique_id) {
		...
		foreach ( $layers as $layer ) {
			...
			if (RevSliderFunctions::getVal ( $layer, 'type', 'text' ) !== 'column')
				continue; // we only want to handle columns here of the current row
			if (RevSliderFunctions::getVal ( $layer, 'p_uid', - 1 ) !== $unique_id)
				continue; // has the wrong row ID
	...
	public function putCreativeGroups() {
		...
		foreach ( $layers as $layer ) {
			if (RevSliderFunctions::getVal ( $layer, 'type', 'text' ) !== 'group')
				continue; // we only want to handle groups here to get the zones we need to create
...
<?php
		...
		if ($slider_type !== 'hero' && ($enable_arrows == 'on' || $enable_bullets == 'on' || $enable_tabs == 'on' || $enable_thumbnails == 'on' || $touch_enabled == 'on' || $keyboard_enabled == 'on' || $mousescroll_enabled != 'off')) {
			...
			if ($enable_arrows == 'on') {
				...
				echo '\',' . "\n";
				echo '					left: {' . "\n";
				echo (in_array ( $this->slider->getParam ( 'leftarrow_position', 'slider' ), array (
						'layergrid',
						'grid' 
				) )) ? '									container:"layergrid",' . "\n" : '';
				...
				echo '					},' . "\n";
				echo '					right: {' . "\n";
				echo (in_array ( $this->slider->getParam ( 'rightarrow_position', 'slider' ), array (
						'layergrid',
						'grid' 
				) )) ? '									container:"layergrid",' . "\n" : '';
			...
			if ($enable_bullets == 'on') {
				...
				echo (in_array ( $this->slider->getParam ( 'bullets_position', 'slider' ), array (
						'layergrid',
						'grid' 
				) )) ? '									container:"layergrid",' . "\n" : '';
			...
			if ($enable_thumbnails == 'on') {
				...
				if ($this->slider->getParam ( 'thumbnails_inner_outer', 'inner' ) == 'inner') {
					echo (in_array ( $this->slider->getParam ( 'thumbnails_position', 'slider' ), array (
							'layergrid',
							'grid' 
					) )) ? '									container:"layergrid",' . "\n" : '';
				}
			...
			if ($enable_tabs == 'on') {
				...
				if ($this->slider->getParam ( 'tabs_inner_outer', 'inner' ) == 'inner') {
					echo (in_array ( $this->slider->getParam ( 'tabs_position', 'slider' ), array (
							'layergrid',
							'grid' 
					) )) ? '									container:"layergrid",' . "\n" : '';
				}
...
>
		...
		if (! $markup_export) { // not needed for html markup export
			if (has_action ( 'wp_footer', array (
					$this,
					'add_inline_double_jquery_error' 
			) ) === false) {
