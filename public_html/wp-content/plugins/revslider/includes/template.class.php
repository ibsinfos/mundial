...
class RevSliderTemplate {
	...
	public function _download_template($uid) {
		...
		if (wp_mkdir_p ( $upload_dir ['basedir'] . $this->templates_path )) { // check here to not flood the server
				...
				if ($response = $request ['body']) {
					if ($response !== 'invalid') {
						...
						if ($ret !== false) {
							...
							$return = array (
									'error' => __ ( 'Can\'t write the file into the uploads folder of WordPress, please change permissions and try again!', 'revslider' ) 
							);
						}
					} else {
						$return = array (
								'error' => __ ( 'Purchase Code is invalid', 'revslider' ) 
						);
					}
				}
			...
			$return = array (
					'error' => __ ( 'Can\'t write into the uploads folder of WordPress, please change permissions and try again!', 'revslider' ) 
			);
		}
						...
						if (isset ( $cur ['slider'] ) && is_array ( $cur ['slider'] )) {
							foreach ( $cur ['slider'] as $ck => $c ) {
								if ($c ['uid'] == $n ['uid']) {
									...
									if (isset ( $c ['is_new'] ))
										$n ['is_new'] = true; // is_new will stay until update is done
		...
		if (! empty ( $templates ) && is_array ( $templates )) {
			...
			if (! empty ( $templates ['slider'] ) && is_array ( $templates ['slider'] )) {
				foreach ( $templates ['slider'] as $key => $temp ) {
					...
					if ($connection > 3)
						continue; // cant connect to server
	...
	/**
	 * Add default Template Slides that can't be deleted for example.
	 * Authors can add their own Slides here through Filter
	 * 
	 * @since : 5.0
	 */
	...
	public function getDefaultTemplateSliders() {
		 ...
		 * array('title' => 'PJ Slider 1', 'alias' => 'pjslider1', 'width' => 1400, 'height' => 868, 'zip' => 'exwebproduct.zip', 'uid' => 'bde6d50c2f73f8086708878cf227c82b', 'installed' => false, 'img' => RS_PLUGIN_URL .
		 * 'admin/assets/imports/exwebproduct.jpg'),
				...
				foreach ( $defaults as $key => $slider ) {
					foreach ( $sliders as $ikey => $installed ) {
						if ($installed ['alias'] == $slider ['alias']) {
							...
							$c_slides = $this->getThemePunchTemplateSlides ( array (
									$installed 
							) );
							if (empty ( $c_slides )) {
	...
	public function write_import_template_markup($template) {
		...
		?>
<div data-src="<?php echo $template['img']; ?>"
	class="template_slider_item_import"
	...
	<div class="template_preview_add_wrapper">
				...
				<a class="preview_template_slider"
			href="<?php echo esc_attr($template['preview']); ?>" target="_blank"><i
...
<div class="template_thumb_more">
	...
	<span class="ttm_space"></span> <span class="ttm_label_direct"><?php _e('Available Version', 'revslider'); ?></span>
	...
	<span class="ttm_space"></span>
		...
		if ($deny == '' && $allow_install == true) {
			if (isset ( $template ['package_parent'] ) && $template ['package_parent'] !== '') {
				...
				?>
					<div class="install_template_slider<?php echo $deny; ?>"
		data-zipname="<?php echo $template['zip']; ?>"
		data-uid="<?php echo $template['uid']; ?>"
		data-title="<?php echo esc_html($template['title']); ?>">
		<i class="eg-icon-download"></i><?php _e('Install Slider', 'revslider'); ?></div>							
					<?php
			}
			if (isset ( $template ['package'] ) && $template ['package'] !== '') {
				?>
					<span class="tp-clearfix" style="margin-bottom: 5px"></span>
	<div class="install_template_slider_package<?php echo $deny; ?>"
		data-zipname="<?php echo $template['zip']; ?>"
		data-uid="<?php echo $template['uid']; ?>"
		data-title="<?php echo esc_html($template['title']); ?>">
		<i class="eg-icon-download"></i><?php _e('Install Slider Pack', 'revslider'); ?></div>							
					<?php
			}
		} else {
			?>
				<div class="dontadd_template_slider_item">
		<i class="icon-not-registered"></i><?php _e('Requirements not met', 'revslider'); ?></div>
				<?php
		}
		?>
			<span class="tp-clearfix" style="margin-bottom: 5px"></span>
...
<div class="template_thumb_more">
	...
	<span class="ttm_space"></span> <span class="ttm_label_direct"><span
		class="ttm_label_half"><?php _e('Installed Vers.', 'revslider'); ?></span><span
		class="ttm_label_half"><?php _e('Available Vers.', 'revslider'); ?></span></span>
	<span class="ttm_label_half ttm_insalled"><span
		class="ttm_label_half ttm_available"><?php echo $template['version'];?></span>
		<span class="ttm_space"></span>	
			<?php
		if ($deny == '' && $allow_install == true) {
			?>
				<div class="install_template_slide<?php echo $deny; ?>"
			data-slidenumber="<?php echo $template['number']; ?>"
			data-zipname="<?php echo $template['zip']; ?>"
			data-uid="<?php echo $template['uid']; ?>"
			data-title="<?php echo esc_html($template['title']); ?>">
...
<div class="template_slide_single_element" style="display: inline-block">
	<div <?php echo $data_urlImageForView; ?>
		class="<?php echo ($slider_id !== false) ? 'template_slider_item' : 'template_item'; ?> <?php echo $bg_extraClass; ?>"
		<?php echo $bg_fullstyle; ?> data-gridwidth="<?php echo $width; ?>"
		data-gridheight="<?php echo $height; ?>"
		<?php if($slider_id !== false){ ?>
		data-sliderid="<?php echo $slider_id; ?>" <?php }else{ ?>
		data-slideid="<?php echo $slide_id; ?>" <?php } ?>></div>
	<div class="template_thumb_overview"></div>
	<div class="template_preview_add_wrapper">
				<?php if(isset($template['preview']) && $template['preview'] !== ''){ ?>
				<a class="preview_template_slider"
			...
			class="eg-icon-search"></i></a>
				<?php } ?>
				<span data-sliderid="<?php echo $slider_id; ?>"
			data-slideid="<?php echo $slide_id; ?>"
			class="show_more_template_slider <?php if (isset($template["user_template"])) echo 'add_user_template_slide_item'; ?>"><i
			class="eg-icon-plus"></i></span>
				<?php
...
if (isset ( $template ["user_template"] )) {
			?>
					<span data-sliderid="<?php echo $slider_id; ?>"
			data-slideid="<?php echo $slide_id; ?>"
			class="show_more_template_slider delete_user_template_slide_item"><i
			class="eg-icon-trash"></i></span>
					<?php
		}
		?>
				<span class="template_group_opener"><i class="fa-icon-folder"></i></span>
...
if (isset ( $template ['title'] )) {
	...
	<ul class="ttm_requirements">
		...
		class="ttm_label_half"><?php _e('Installed Vers.', 'revslider'); ?></span><span
		class="ttm_label_half"><?php _e('Available Vers.', 'revslider'); ?></span></span>
	<span class="ttm_label_half ttm_insalled"><?php echo isset($template['current_version']) ? $template['current_version'] : 'N/A';?></span><span
		class="ttm_label_half ttm_available"><?php echo $template['version'];?></span>	
					<?php
			}
			?>
			...
			if ($allow_install !== false) {
				...
				if ($slider_id !== false) {
					?>
						<div class="add_template_slider_item"
		data-sliderid="<?php echo $slider_id; ?>">
						<?php
				} else {
					?>
						<div class="add_template_slide_item"
			data-slideid="<?php echo $slide_id; ?>">
						<?php
				}
				?>
				...
				if ($slider_id !== false && isset ( $template ['package'] ) && $template ['package'] !== '' && $template ['package_full_installded']) {
					?>
						<div class="add_template_slider_item_package"
			data-uid="<?php echo $template['uid']; ?>"
			data-title="<?php echo esc_html($template['title']); ?>">
			<i class="eg-icon-plus"></i><?php echo __('Add Slider Pack', 'revslider'); ?></div>
						<?php
				}
				if ($slider_id !== false) {
					?>	
						<span class="tp-clearfix" style="margin-bottom: 5px"></span>
		<div class="install_template_slider"
			data-zipname="<?php echo $template['zip']; ?>"
			data-uid="<?php echo $template['uid']; ?>"
			data-title="<?php echo esc_html($template['title']); ?>">
			<i class="eg-icon-download"></i><?php _e('Re-Download Slider', 'revslider'); ?></div>
		<span class="tp-clearfix" style="margin-bottom: 5px"></span>
						<?php
					if (isset ( $template ['package'] ) && $template ['package'] !== '') {
						$txt = ($template ['package_full_installded']) ? __ ( 'Re-Download Slider Pack', 'revslider' ) : __ ( 'Install Slider Pack', 'revslider' );
						?>
							<div class="install_template_slider_package"
			data-zipname="<?php echo $template['zip']; ?>"
			data-uid="<?php echo $template['uid']; ?>"
			data-title="<?php echo esc_html($template['title']); ?>">
			<i class="eg-icon-download"></i><?php echo $txt; ?></div>
		<span class="tp-clearfix" style="margin-bottom: 5px"></span>
							<?php
					}
				}
	...
	public function create_html_slides($tp_template_slider, $all_slider, $templates) {
		...
		class="template_group_wrappers  <?php echo $slidercat.$slidercat_new.' '; if(isset($m_slider['filter'])){ echo implode(' ', $m_slider['filter']); } ?>">							
							<?php
					...
					<span class="template_installed"><?php _e("Installed", "revslider"); ?><i
				...
				<div class="template_thumb_title"><?php echo $title; ?></div>
					...
					if (! empty ( $c_slides )) {
						?>
						<div style="margin-bottom: 30px"
		class="template_group_wrappers not-imported-wrapper <?php echo $slidercat.$slidercat_new; if(isset($m_slider['filter'])){ echo implode(' ', $m_slider['filter']); } ?>">
		...
		if (! empty ( $all_slider )) {
			foreach ( $all_slider as $c_slider ) {
				...
				if (! empty ( $c_slides )) {
					?>
					<div
		class="template_group_wrappers temp_existing <?php //if(isset($c_slider['filter'])){ echo implode(' ', $c_slider['filter']); } ?>">
						<?php
			...
			foreach ( $sliders as $slider ) {
				if (isset ( $slider ['package'] ) && $slider ['package'] == $package) {
					if (isset ( $slider ['package_parent'] ) && $slider ['package_parent'] == 'true')
						continue; // dont install parent package
