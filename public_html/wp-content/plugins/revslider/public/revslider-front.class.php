...
class RevSliderFront extends RevSliderBaseFront {
	...
	public function __construct() {
		...
		add_filter ( 'punchfonts_modify_url', array (
				'RevSliderFront',
				'modify_punch_url' 
		) );
...
<?php
	...
	public static function add_admin_menu_nodes() {
		...
		self::_add_node ( '<span class="rs-label">Slider Revolution</span>', false, admin_url ( 'admin.php?page=revslider' ), array (
				'class' => 'revslider-menu' 
		), 'revslider' ); // <span class="wp-menu-image dashicons-before dashicons-update"></span>
		...
		if (! empty ( $sliders )) {
			foreach ( $sliders as $id => $slider ) {
				self::_add_node ( '<span class="rs-label" data-alias="' . esc_attr ( $slider ['alias'] ) . '">' . esc_html ( $slider ['title'] ) . '</span>', 'revslider', admin_url ( 'admin.php?page=revslider&view=slide&id=new&slider=' . intval ( $id ) ), array (
						'class' => 'revslider-sub-menu' 
				), esc_attr ( $slider ['alias'] ) ); // <span class="wp-menu-image dashicons-before dashicons-update"></span>
			}
		}
	}
