...
class RevSliderAdmin extends RevSliderBaseAdmin {
	...
	private function init() {
		...
		add_action ( 'admin_enqueue_scripts', array (
				'RevSliderAdmin',
				'enqueue_all_admin_scripts' 
		) );
		
		add_action ( 'wp_ajax_revslider_ajax_call_front', array (
				'RevSliderAdmin',
				'onFrontAjaxAction' 
		) );
		add_action ( 'wp_ajax_nopriv_revslider_ajax_call_front', array (
				'RevSliderAdmin',
				'onFrontAjaxAction' 
...
<?php
	...
	public static function enqueue_all_admin_scripts() {
		...
		wp_enqueue_script ( 'tp-color-picker-js', plugins_url ( '../public/assets/js/tp-color-picker.min.js', __FILE__ ), array (
				'jquery' 
		...
		foreach ( $post_types as $post_type ) {
			if ($post_type == $screen->id) {
				wp_enqueue_script ( 'revslider-tinymce-shortcode-script', RS_PLUGIN_URL . 'admin/assets/js/tinymce-shortcode-script.js', array (
						'jquery' 
				), RevSliderGlobals::SLIDER_REVISION );
			}
		}
...
<div class="updated below-h2 rs-update-notice-wrap" id="message">
	<a href="javascript:void(0);" style="float: right; padding-top: 9px;"
			...
			if (! empty ( $notices ) && is_array ( $notices )) {
				...
				foreach ( $notices as $notice ) {
					if ($notice->is_global !== true && ! in_array ( $screen->id, $revslider_screens ))
						continue; // check if global or just on plugin related pages
...
<div
	class="<?php echo $notice->color; ?> below-h2 rs-update-notice-wrap"
	id="message"
	style="clear: both; display: block; position: relative; margin: 35px 20px 25px 0px">
	<div style="display: table; width: 100%;">
		<div
			style="vertical-align: middle; display: table-cell; min-width: 100%; padding-right: 15px;"><?php echo $notice->text; ?></div><?php echo $text; ?></div>
</div>
...
<?php
	...
	private function addSliderMetaBox($postTypes = null) { // null = all, post = only posts
		try {
			self::addMetaBox ( "Revolution Slider Options", '', array (
					"RevSliderAdmin",
					"customPostFieldsOutput" 
			), $postTypes );
		} catch ( Exception $e ) {
		}
	}
...
<?php
	...
	public static function onAddScripts() {
		...
		wp_enqueue_script ( 'unite_layers', RS_PLUGIN_URL . 'admin/assets/js/edit_layers.js', array (
				'jquery-ui-mouse' 
		), RevSliderGlobals::SLIDER_REVISION );
		...
		add_filter ( 'revslider_mod_icon_sets', array (
				'RevSliderBase',
				'set_icon_sets' 
		) );
	...
	public static function toggle_favorite_by_id($id) {
		...
		$response = $wpdb->update ( $table_name, array (
			...
			switch ($action) {
				case 'add_new_preset' :
					...
					if ($result === true) {
						...
						self::ajaxResponseSuccess ( __ ( 'Preset created', 'revslider' ), array (
								'data' => $presets 
						) );
					} else {
				...
				case 'remove_preset' :
					...
					if ($result === true) {
						...
						self::ajaxResponseSuccess ( __ ( 'Preset deleted', 'revslider' ), array (
								'data' => $presets 
						) );
					} else {
						self::ajaxResponseError ( $result, false );
				...
				case "import_slide_online_template_slidersview" :
					...
					self::importSliderOnlineTemplateHandle ( $data, $viewBack, 'true', 'none', array (
							'slider_id' => $sliderid,
							'slide_id' => $slidenum 
					) );
					break;
				case "import_slide_template_slidersview" :
					...
					self::importSliderTemplateHandle ( $viewBack, $updateAnim, $updateStatic, array (
							'slider_id' => $sliderid,
							'slide_id' => $slidenum 
					) );
					break;
				...
				case "duplicate_slide_stay" :
					...
					if ($action == 'duplicate_slide_stay') {
						self::ajaxResponseSuccess ( __ ( "Slide duplicated", 'revslider' ), array (
								'id' => $return [1] 
						) );
					} else {
				...
				case "add_slide_to_template" :
					...
					self::ajaxResponseSuccess ( __ ( 'Slide added to Templates', 'revslider' ), array (
							'HTML' => $html 
					) );
					exit ();
					break;
				...
				case "insert_captions_css" :
					...
					if ($arrCaptions !== false) {
						...
						self::ajaxResponseSuccess ( __ ( "CSS saved", 'revslider' ), array (
				...
				case "rename_captions_css" :
					...
					self::ajaxResponseSuccess ( __ ( "Class name renamed", 'revslider' ), array (
							"arrCaptions" => $arrCaptions,
							'compressed_css' => $styles . $custom_css,
							'initstyles' => $arrCssStyles 
					) );
				...
				case "delete_captions_css" :
					...
					self::ajaxResponseSuccess ( __ ( "Style deleted!", 'revslider' ), array (
							"arrCaptions" => $arrCaptions,
							'compressed_css' => $styles . $custom_css,
							'initstyles' => $arrCssStyles 
					) );
				...
				case "create_navigation_preset" :
					...
					if ($return === true) {
						self::ajaxResponseSuccess ( __ ( 'Navigation preset saved/updated', 'revslider' ), array (
								'navs' => $nav->get_all_navigations () 
						) );
					} else {
						if ($return === false)
				...
				case "delete_navigation_preset" :
					...
					if ($return) {
						self::ajaxResponseSuccess ( __ ( 'Navigation preset deleted', 'revslider' ), array (
								'navs' => $nav->get_all_navigations () 
						) );
					} else {
						if ($return === false)
				...
				case "activate_purchase_code" :
					...
					if ($result === true) {
						...
						if ($result == 'exist') {
							self::ajaxResponseData ( array (
									'error' => $result,
									'msg' => __ ( 'Purchase Code already registered!', 'revslider' ) 
							) );
						}
						/*
				...
				case 'change_specific_navigation' :
					...
					self::ajaxResponseSuccess ( __ ( 'Navigation saved/updated', 'revslider' ), array (
							'navs' => $nav->get_all_navigations () 
					) );
				...
				case 'change_navigations' :
					...
					self::ajaxResponseSuccess ( __ ( 'Navigations updated', 'revslider' ), array (
							'navs' => $nav->get_all_navigations () 
					) );
					break;
				case 'delete_navigation' :
					...
					if (isset ( $data ) && intval ( $data ) > 0) {
						...
						if ($return !== true) {
							...
							self::ajaxResponseSuccess ( __ ( 'Navigation deleted', 'revslider' ), array (
									'navs' => $nav->get_all_navigations () 
							) );
						}
					}
				...
				case "get_facebook_photosets" :
					if (! empty ( $data ['url'] )) {
						...
						if (! empty ( $return )) {
							self::ajaxResponseSuccess ( __ ( 'Successfully fetched Facebook albums', 'revslider' ), array (
									'html' => implode ( ' ', $return ) 
							) );
						} else {
						...
						self::ajaxResponseSuccess ( __ ( 'Cleared Albums', 'revslider' ), array (
								'html' => implode ( ' ', $return ) 
						) );
					}
					break;
				case "get_flickr_photosets" :
					if (! empty ( $data ['url'] ) && ! empty ( $data ['key'] )) {
						...
						if (! empty ( $return )) {
							self::ajaxResponseSuccess ( __ ( 'Successfully fetched flickr photosets', 'revslider' ), array (
									"data" => array (
											'html' => implode ( ' ', $return ) 
									) 
							) );
						...
						if (empty ( $data ['url'] ) && empty ( $data ['key'] )) {
							self::ajaxResponseSuccess ( __ ( 'Cleared Photosets', 'revslider' ), array (
									'html' => implode ( ' ', $return ) 
							) );
				...
				case "get_youtube_playlists" :
					if (! empty ( $data ['id'] )) {
						...
						self::ajaxResponseSuccess ( __ ( 'Successfully fetched YouTube playlists', 'revslider' ), array (
								"data" => array (
										'html' => implode ( ' ', $return ) 
								) 
						) );
