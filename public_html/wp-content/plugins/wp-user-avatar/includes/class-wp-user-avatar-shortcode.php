...
class WP_User_Avatar_Shortcode {
	...
	public function __construct() {
		...
		add_shortcode ( 'avatar', array (
				$this,
				'wpua_shortcode' 
		) );
		add_shortcode ( 'avatar_upload', array (
				$this,
				'wpua_edit_shortcode' 
		) );
		// Add avatar and scripts to avatar_upload
		add_action ( 'wpua_show_profile', array (
				$wp_user_avatar,
	...
	private function wpua_edit_user($user_id = 0) {
		...
		$user = new stdClass ();
		$errors = new WP_Error ();
		do_action_ref_array ( 'wpua_update_errors', array (
				&$errors,
				$update,
				&$user 
		) );
	...
	private function wpua_edit_form($user) {
		ob_start ();
		?>
<form id="wpua-edit-<?php echo $user->ID; ?>" class="wpua-edit"
	action="" method="post" enctype="multipart/form-data">
      <?php do_action('wpua_show_profile', $user); ?>
      <input type="hidden" name="wpua_action" value="update" /> <input
