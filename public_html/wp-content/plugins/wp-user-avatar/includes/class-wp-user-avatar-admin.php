...
class WP_User_Avatar_Admin {
	...
	public function __construct() {
		...
		register_activation_hook ( WPUA_DIR . 'wp-user-avatar.php', array (
				$this,
				'wpua_options' 
		) );
		...
		add_action ( 'admin_init', array (
				$this,
				'wpua_register_settings' 
		) );
		// Default avatar
		add_filter ( 'default_avatar_select', array (
				$this,
				'wpua_add_default_avatar' 
		), 10 );
		add_filter ( 'whitelist_options', array (
				$this,
				'wpua_whitelist_options' 
		), 10 );
		...
		// Media states
		add_filter ( 'display_media_states', array (
				$this,
				'wpua_add_media_state' 
		), 10, 1 );
	}
	...
	public function wpua_admin() {
		add_menu_page ( __ ( 'WP User Avatar', 'wp-user-avatar' ), __ ( 'Avatars', 'wp-user-avatar' ), 'manage_options', 'wp-user-avatar', array (
				$this,
				'wpua_options_page' 
		), WPUA_URL . 'images/wpua-icon.png' );
		add_submenu_page ( 'wp-user-avatar', __ ( 'Settings', 'wp-user-avatar' ), __ ( 'Settings', 'wp-user-avatar' ), 'manage_options', 'wp-user-avatar', array (
				$this,
				'wpua_options_page' 
		) );
		$hook = add_submenu_page ( 'wp-user-avatar', __ ( 'Library', 'wp-user-avatar' ), __ ( 'Library', 'wp-user-avatar' ), 'manage_options', 'wp-user-avatar-library', array (
				$this,
				'wpua_media_page' 
		) );
		add_action ( "load-$hook", array (
				$this,
				'wpua_media_screen_option' 
		) );
		add_filter ( 'set-screen-option', array (
				$this,
				'wpua_set_media_screen_option' 
		), 10, 3 );
	}
	...
	public function wpua_add_default_avatar() {
		...
		remove_filter ( 'get_avatar', array (
				$wpua_functions,
				'wpua_get_avatar_filter' 
		) );
		...
		if (! empty ( $wpua_avatar_default ) && $wpua_functions->wpua_attachment_is_image ( $wpua_avatar_default )) {
			$avatar_thumb_src = $wpua_functions->wpua_get_attachment_image_src ( $wpua_avatar_default, array (
					32,
					32 
			) );
	...
	public function wpua_action_links($links, $file) {
		if (basename ( dirname ( $file ) ) == 'wp-user-avatar') {
			$links [] = '<a href="' . esc_url ( add_query_arg ( array (
					'page' => 'wp-user-avatar' 
			), admin_url ( 'admin.php' ) ) ) . '">' . __ ( 'Settings', 'wp-user-avatar' ) . '</a>';
		}
		return $links;
	}
	...
	public function wpua_add_column($columns) {
		return $columns + array (
				'wp-user-avatar' => __ ( 'Avatar', 'wp-user-avatar' ) 
		);
	}
	...
	public function wpua_show_column($value, $column_name, $user_id) {
		...
		$wpua_image = $wpua_functions->wpua_get_attachment_image ( $wpua, array (
				32,
				32 
		) );
