...
<p class="search-box">
	...
	<input type="hidden" id="page" name="page"
		value="wp-user-avatar-library" /> <input type="search"
		id="<?php echo $input_id ?>" name="s"
...
<?php
	...
	public function get_views() {
		...
		$type_links ['all'] = sprintf ( '<a href="%s">', esc_url ( add_query_arg ( array (
				'page' => 'wp-user-avatar-library' 
		), 'admin.php' ) ) ) . sprintf ( _nx ( 'All <span class="count">(%s)</span>', 'All <span class="count">(%s)</span>', $_total_posts, 'uploaded files' ), number_format_i18n ( $_total_posts ) ) . '</a>';
		return $type_links;
	}
...
<tr id='post-<?php echo $post->ID; ?>'
	...
	valign="top">
      <?php
		...
		for="cb-select-<?php the_ID(); ?>"><?php echo sprintf(__('Select %s','wp-user-avatar'), $att_title);?></label>
		<input type="checkbox" name="media[]"
		id="cb-select-<?php the_ID(); ?>" value="<?php the_ID(); ?>" />
                <?php } ?>
              </th>
            <?php
						break;
                ...
                <td <?php echo $attributes ?>><?php
						if ($thumb = $wpua_functions->wpua_get_attachment_image ( $post->ID, array (
								80,
								60 
						), true )) {
