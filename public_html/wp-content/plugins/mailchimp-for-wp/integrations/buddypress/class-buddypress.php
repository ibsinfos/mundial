...
class MC4WP_BuddyPress_Integration extends MC4WP_User_Integration {
	...
	public function add_hooks() {
		...
		if (is_multisite ()) {
			 ...
			 */
			add_filter ( 'bp_signup_usermeta', array (
					$this,
					'store_usermeta' 
			), 10, 1 );
			add_action ( 'bp_core_activated_user', array (
					$this,
					'subscribe_from_usermeta' 
			), 10, 3 );
		} else {
			add_action ( 'bp_core_signup_user', array (
					$this,
					'subscribe_from_form' 
			), 10, 4 );
		}
		 ...
		 * installs have a user moderation plugin (e.g.
		 ...
		 */
		add_action ( 'mc4wp_integration_buddypress_subscribe_user', array (
				$this,
				'subscribe_buddypress_user' 
		), 10, 1 );
	}
	...
	public function subscribe_from_form($user_id, $user_login, $user_password, $user_email) {
		 ...
		 *        	False does not subscribe the user.
		 * @param int $user_id
	 ...
	 *
	 * @param int $user_id
	 *        	The activated user ID
	 * @param string $key
	 *        	the activation key (not used)
	 * @param array $userdata
	 *        	An array containing the activated user data
	 * @return bool
	 */
