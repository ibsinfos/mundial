...
class MC4WP_Form {
	...
	public function validate() {
		...
		if ($valid) {
			...
			$validator->add_rule ( '_mc4wp_timestamp', 'range', 'spam', array (
					'max' => time () - 2 
			) );
		...
		/**
		 * Filters whether this form has errors.
		 * Runs only when a form is submitted.
	 ...
	 * Should be called before calling validate() method.
	 *
	...
	protected function parse_request_data(MC4WP_Request $request) {
		 ...
		 * @since 3.0
		 *       
		 * @param array $ignored_field_names
	...
	public function set_config(array $config) {
		...
		if (! in_array ( $this->config ['action'], array (
				'subscribe',
				'unsubscribe' 
		) )) {
		...
		if (! in_array ( $this->config ['email_type'], array (
				'html',
				'text' 
		) )) {
	...
	public function get_required_fields() {
		...
		$required_fields = array_diff ( $required_fields, array (
				'EMAIL' 
		) );
