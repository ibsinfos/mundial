...
class MC4WP_Forms_Admin {
	...
	public function add_hooks() {
		add_action ( 'register_shortcode_ui', array (
				$this,
				'register_shortcake_ui' 
		) );
		add_action ( 'mc4wp_save_form', array (
				$this,
				'update_form_stylesheets' 
		) );
		add_action ( 'mc4wp_admin_preview_form', array (
				$this,
				'prepare_form_preview' 
		) );
		add_action ( 'mc4wp_admin_edit_form', array (
				$this,
				'process_save_form' 
		) );
		add_action ( 'mc4wp_admin_add_form', array (
				$this,
				'process_add_form' 
		) );
		add_filter ( 'mc4wp_admin_menu_items', array (
				$this,
				'add_menu_item' 
		), 5 );
		
		add_action ( 'mc4wp_admin_show_forms_page-edit-form', array (
				$this,
				'show_edit_page' 
		) );
		add_action ( 'mc4wp_admin_show_forms_page-add-form', array (
				$this,
				'show_add_page' 
		) );
		
		add_action ( 'mc4wp_admin_enqueue_assets', array (
				$this,
				'enqueue_assets' 
		), 10, 2 );
	}
	...
	public function enqueue_assets($suffix, $page = '') {
		...
		wp_register_script ( 'mc4wp-forms-admin', MC4WP_PLUGIN_URL . 'assets/js/forms-admin' . $suffix . '.js', array (
				'mc4wp-admin' 
		), MC4WP_VERSION, true );
	...
	public function sanitize_form_data($data) {
		 ...
		 *
		 * @param array $data
		 *        	Sanitized array of form data.
		 * @param array $raw_data
		 *        	Raw array of form data.
		 *        	
		 * @since 3.0.8
		 * @ignore
		 *
