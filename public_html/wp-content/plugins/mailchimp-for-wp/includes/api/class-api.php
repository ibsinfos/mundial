...
class MC4WP_API {
	 ...
	 *        	Array of extra merge variables
	 * @param string $email_type
	 *        	The email type to send to this email address. Possible values are `html` and `text`.
	 * @param boolean $double_optin
	 *        	Should this email be confirmed via double opt-in?
	 ...
	 *        	Send a welcome e-mail, only if double_optin is false.
	 *        	
	...
	public function get_list_groupings($list_id) {
		$result = $this->call ( 'lists/interest-groupings', array (
				'id' => $list_id 
		) );
		if (is_array ( $result )) {
			return $result;
	 ...
	 *        	Array of ID's of the lists to fetch. (optional)
	 *        	
	 * @return bool
	 */
	...
	public function get_lists_for_email($email) {
		...
		$result = $this->call ( 'helper/lists-for-email', array (
				'email' => $email 
		) );
	...
	public function get_lists_with_merge_vars($list_ids) {
		$result = $this->call ( 'lists/merge-vars', array (
				'id' => $list_ids 
		) );
	...
	public function list_has_subscriber($list_id, $email) {
		$member_info = $this->get_subscriber_info ( $list_id, array (
				array (
						'email' => $email 
				) 
		) );
	...
	public function add_ecommerce_order(array $order_data) {
		$response = $this->call ( 'ecomm/order-add', array (
				'order' => $order_data 
		) );
