...
class MC4WP_Field_Guesser {
	...
	public function guessed() {
		...
		foreach ( $fields as $field => $value ) {
			...
			$sub_fields = is_array ( $value ) ? $value : array (
					$value 
			);
			...
			$simple_key = str_replace ( array (
					'-',
					'_',
					' ' 
			), '', $field );
			
			if (empty ( $guessed ['FNAME'] ) && $this->string_contains ( $simple_key, array (
					'FIRSTNAME',
					'FNAME',
					'GIVENNAME',
					'FORENAME' 
				...
				$guessed ['FNAME'] = $value;
			} elseif (empty ( $guessed ['LNAME'] ) && $this->string_contains ( $simple_key, array (
					'LASTNAME',
					'LNAME',
					'SURNAME',
					'FAMILYNAME' 
	...
	public function combine($methods) {
		...
		foreach ( $methods as $method ) {
			if (method_exists ( $this, $method )) {
				$combined = array_merge ( $combined, call_user_func ( array (
						$this,
						$method 
				) ) );
			}
