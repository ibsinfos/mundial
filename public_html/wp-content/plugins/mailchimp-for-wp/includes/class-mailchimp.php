...
class MC4WP_MailChimp {
	 ...
	 *        	The list id to subscribe to
	 * @param string $email_address
	 *        	The email address to subscribe
	 * @param array $args        	
	 * @param boolean $update_existing
	 *        	Update information if this email is already on list?
	 * @param boolean $replace_interests
	 *        	Replace interest groupings, only if update_existing is true.
	 *        	
	 * @return object
	 */
	...
	public function list_unsubscribe($list_id, $email_address) {
		...
		try {
			$this->api->update_list_member ( $list_id, $email_address, array (
					'status' => 'unsubscribed' 
			) );
		} catch ( MC4WP_API_Resource_Not_Found_Exception $e ) {
	 ...
	 *
	 * @param boolean $force
	 *        	Whether to force a result by hitting MailChimp API
	 * @return array
	 */
	...
	private function fetch_list($list_id) {
		try {
			$list_data = $this->api->get_list ( $list_id, array (
					'fields' => 'id,name,stats,web_id,campaign_defaults.from_name,campaign_defaults.from_email' 
			) );
			...
			if ($list_data->stats->merge_field_count > 0) {
				$field_data = $this->api->get_list_merge_fields ( $list->id, array (
						'count' => 100,
			...
			$interest_categories_data = $this->api->get_list_interest_categories ( $list->id, array (
					'count' => 100,
			...
			foreach ( $interest_categories_data as $interest_category_data ) {
				...
				$interests_data = $this->api->get_list_interest_category_interests ( $list->id, $interest_category->id, array (
						'count' => 100,
	 ...
	 *
	 * @param bool $force
	 *        	Force result by hitting MailChimp API
	 * @return array
	 */
	...
	public function fetch_list_ids() {
		try {
			$lists_data = $this->api->get_lists ( array (
					'count' => 200,
					'fields' => 'lists.id' 
			) );
		} catch ( MC4WP_API_Exception $e ) {
	 ...
	 * @param string $list_id        	
	 * @param bool $force
	 *        	Whether to force a result by hitting remote API
	 * @return MC4WP_MailChimp_List
	 */
	...
	public function get_subscriber_counts() {
		...
		try {
			$lists = $this->api->get_lists ( array (
					'count' => 100,
					'fields' => 'lists.id,lists.stats' 
			) );
		} catch ( MC4WP_API_Exception $e ) {
			return array ();
		}
		...
		/**
		 * Filters the cache time for MailChimp lists configuration, in seconds.
	 ...
	 *        	Array of list ID's, or single string.
