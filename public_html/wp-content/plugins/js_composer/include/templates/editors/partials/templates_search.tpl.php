<?php
if (! defined ( 'ABSPATH' )) {
	die ( '-1' );
}

?>
<div class="vc_ui-panel-header-actions">
	<div class="vc_ui-search-box">
		<div class="vc_ui-search-box-input">
			<input type="search" id="vc_templates_name_filter"
				data-vc-templates-name-filter
				placeholder="<?php echo __( 'Search template by name', 'js_composer' ) ?>">
			<label for="vc_templates_name_filter"> <i
				class="vc-composer-icon vc-c-icon-search"></i>
			</label>
		</div>
	</div>
</div>
