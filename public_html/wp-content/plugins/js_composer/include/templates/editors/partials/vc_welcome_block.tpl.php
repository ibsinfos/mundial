...
<div id="vc_no-content-helper" class="vc_welcome vc_ui-font-open-sans">
	...
	if (vc_user_access ()->part ( 'shortcodes' )->checkStateAny ( true, 'custom', null )->get () && vc_user_access_check_shortcode_all ( 'vc_row' ) && vc_user_access_check_shortcode_all ( 'vc_column' )) :
		?>
			...
			data-vc-element="add-element-action" href="#"> <i
			class="vc_ui-icon-pixel vc_ui-icon-pixel-control-plus"></i>
...
if (vc_user_access ()->part ( 'shortcodes' )->can ( 'vc_column_text_all' )->get ()) :
		?>
			...
			title="<?php _e( 'Add text block', 'js_composer' ) ?>"> <i
			class="vc_ui-icon-pixel vc_ui-icon-pixel-control-edit"></i>
...
if ($templates_total_count > 0 && vc_user_access ()->part ( 'templates' )->can ()->get ()) :
		?>
			...
			class="vc_general vc_ui-button vc_ui-button-shape-rounded vc_ui-button-warning"
			href="#"> <i
			class="vc_ui-icon-pixel vc_ui-icon-pixel-control-template"></i>
	...
	if (vc_user_access ()->part ( 'shortcodes' )->checkStateAny ( true, 'custom', null )->get () && vc_user_access_check_shortcode_all ( 'vc_row' ) && vc_user_access_check_shortcode_all ( 'vc_column' )) :
		...
		<a id="vc_not-empty-add-element"
			class="vc_add-element-not-empty-button"
			title="<?php _e( 'Add Element', 'js_composer' ) ?>"
			data-vc-element="add-element-action"> <i
			class="vc-composer-icon vc-c-icon-add"></i>
		</a>
	</div>
		<?php endif; ?>
