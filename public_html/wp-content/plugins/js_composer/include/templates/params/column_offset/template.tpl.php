...
<div class="vc_column-offset" data-column-offset="true">
	<?php if ( '1' === vc_settings()->get( 'not_responsive_css' ) ) :  ?>
		<div
		class="wpb_alert wpb_content_element vc_alert_rounded wpb_alert-warning">
	...
	<?php echo esc_attr( $settings['type'] ) ?> '_field"
	...
	<table class="vc_table vc_column-offset-table">
		...
		<?php foreach ( $sizes as $key => $size ) :  ?>
			...
			<td class="vc_screen-size vc_screen-size-<?php echo $key ?>"><span
				title="<?php echo $size ?>"><i
					class="vc-composer-icon vc-c-icon-layout-<?php echo $key ?>"></i></span>
			</td>
			<td>
			...
			<td><label> <input type="checkbox"
					name="vc_hidden-<?php echo $key ?>" value="yes"
					<?php echo in_array( 'vc_hidden-' . $key, $data ) ? ' checked="true"' : ''?>
...
<script type="text/javascript">
	window.VcI8nColumnOffsetParam = <?php
