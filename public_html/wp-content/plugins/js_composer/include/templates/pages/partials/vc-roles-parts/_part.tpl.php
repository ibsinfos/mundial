	...
	<td>
		<fieldset>
			...
			<select name="<?php echo esc_attr( $params_prefix . '[_state]' ) ?>"
				...
				<?php endif; ?> class="vc_ui-settings-roles-dropdown">
...
<?php if ( ! empty( $capabilities ) ) : ?>
	<?php if ( isset( $use_table ) && true === $use_table ) : ?>
		...
		?>
<tr
	data-vc-role-related-part="<?php echo esc_attr( $part . '-' . $role ) ?>"
...
vc_include_template ( 'editors/partials/add_element_tabs.tpl.php', array (
					...
					<tr>
						<th><?php echo esc_html( $item_header_name ) ?></th>
							<?php foreach ( $cap_types as $type ) : ?>
								<th class="column-date"><label> <input type="checkbox"
								name="all"
				...
				<tfoot
					<?php echo isset( $global_set ) ? ' style="display: none;"' : '' ?>>
					<tr>
						<th><?php echo esc_html( $item_header_name ) ?></th>
							<?php foreach ( $cap_types as $type ) : ?>
								<th class="column-date"><label> <input type="checkbox"
						...
						<td title="<?php echo esc_attr( $cap['base'] ) ?>">
							...
							<div class="vc_wp-form-checkbox">
								<label> <input type="checkbox"
									...
									data-vc-cap="<?php echo esc_attr( $type[0] ) ?>" value="1"
									<?php echo ! isset( $global_set ) && $controller->can( $cap['base'] . '_' . $type[0], false )->get() ? ' checked' : '' ?>>
		...
		<fieldset>
			...
			<div class="vc_wp-form-row">
					...
					<label> <input type="checkbox"
						...
						value="1" class="vc_roles-settings-checkbox"
						<?php
			// hard coded yes :)
			if ('administrator' === $role && 'settings' === $part && ('vc-roles-tab' === $cap [0] || 'vc-updater-tab' === $cap [0])) {
				...
				?>
						...
						data-vc-roles="serialize" data-vc-roles="checkbox"
						<?php
