	...
	<table class="vc_general vc_wp-form-table fixed">
		...
		<?php foreach ( $post_types as $post_type ) : ?>
			...
			<tr>
				...
				<td><select
					name="<?php echo $field_key ?>[<?php echo $post_type[0] ?>]">
						<option value=""><?php _e( 'None', 'js_composer' ) ?></option>
						<?php foreach ( $templates as $templates_category ) : ?>
							<optgroup
							label="<?php echo esc_attr( $templates_category['category_name'] ) ?>">
					...
					?>
								...
								<?php echo $selected ?>><?php echo esc_html( $template['name'] ) ?></option>
								<?php endforeach; ?>
							</optgroup>
						<?php endforeach; ?>
					</select></td>
