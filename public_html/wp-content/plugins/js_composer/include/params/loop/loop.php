...
;
	...
	protected function parse_tax_query($value) {
		...
		$terms = get_terms ( VcLoopSettings::getTaxonomies (), array (
				'include' => array_map ( 'abs', $terms ) 
		) );
		foreach ( $terms as $t ) {
...
class VcLoopSettings {
	...
	public function parse_order_by($value) {
		return $this->parseDropDown ( $value, array (
				...
				array (
						'modified',
						__ ( 'Modified', 'js_composer' ) 
				),
				array (
						'rand',
						__ ( 'Random', 'js_composer' ) 
				),
				array (
						'comment_count',
						__ ( 'Comment count', 'js_composer' ) 
				),
				array (
						'menu_order',
						__ ( 'Menu order', 'js_composer' ) 
				) 
		) );
	}
	...
	public function parse_order($value) {
		return $this->parseDropDown ( $value, array (
				array (
						'ASC',
						__ ( 'Ascending', 'js_composer' ) 
				),
				array (
						'DESC',
						__ ( 'Descending', 'js_composer' ) 
				) 
		) );
	}
	...
	public function parse_tax_query($value) {
		...
		$list = get_terms ( self::getTaxonomies (), array (
				'include' => array_map ( 'abs', $list ) 
		) );
		foreach ( $list as $obj ) {
			$options [] = array (
