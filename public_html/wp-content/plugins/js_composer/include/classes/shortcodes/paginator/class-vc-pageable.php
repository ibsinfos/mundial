...
abstract class WPBakeryShortCode_Vc_Pageable extends WPBakeryShortCode {
	...
	protected function contentLoadMore($grid_style, $settings, $content) {
		if (! isset ( $settings ['btn_data'] ) && isset ( $settings ['button_style'] ) && isset ( $settings ['button_size'] ) && isset ( $settings ['button_color'] )) {
			...
			$button3 = new WPBakeryShortCode_VC_Btn ( array (
					'base' => 'vc_btn' 
			) );
			
			return '<div class="vc_pageable-slide-wrapper vc_clearfix" data-vc-grid-content="true">' . $content . '</div>' . '<div class="vc_pageable-load-more-btn" data-vc-grid-load-more-btn="true">' . apply_filters ( 'vc_gitem_template_attribute_' . 'vc_btn', '', array (
					'post' => new stdClass (),
	...
	protected function contentLazy($grid_style, $settings, $content) {
		return '<div class="vc_pageable-slide-wrapper vc_clearfix" data-vc-grid-content="true">' . $content . '</div><div data-lazy-loading-btn="true" style="display: none;"><a href="' . get_permalink ( $settings ['page_id'] ) . '"></a></div>';
	}
