	...
	abstract class WPBakeryShortCode extends WPBakeryVisualComposerAbstract {
		...
		protected function registerJs($param) {
			if (is_array ( $param ) && ! empty ( $param )) {
				...
				wp_register_script ( $name, $param, array (
						'jquery' 
				), WPB_VC_VERSION, true );
			}
		}
		...
		protected function registerCss($param) {
			if (is_array ( $param ) && ! empty ( $param )) {
				...
				wp_register_style ( $name, $param, array (
						'js_composer' 
				), WPB_VC_VERSION );
			}
		}
		...
		public function getControlsList() {
			...
			if ($allAccess) {
				...
				if ($editAccess) {
					...
					return in_array ( 'add', $controls ) ? array (
							'add' 
					) : array ();
				}
			}
		 ...
		 * @param
		 *        	$param
		 * @param $value vc_filter:
		 *        	vc_wpbakeryshortcode_single_param_html_holder_value - hook to override param value (param type and etc is available in args)
		 *        	
		 * @return string
		 */
