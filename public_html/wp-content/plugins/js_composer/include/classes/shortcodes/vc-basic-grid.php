...
class WPBakeryShortCode_VC_Basic_Grid extends WPBakeryShortCode_Vc_Pageable {
	...
	public function getId($atts, $content) {
		...
		$id_to_save = json_encode ( array (
				'failed_to_get_id' => esc_attr ( $id_value ) 
		) );
	...
	public function buildQuery($atts) {
		...
		if ('ids' !== $atts ['post_type']) {
			...
			if (! empty ( $atts ['taxonomies'] )) {
				$vc_taxonomies_types = get_taxonomies ( array (
						'public' => true 
				) );
	...
	public function buildItems() {
		...
		foreach ( $post_data as $post ) {
			$post->filter_terms = wp_get_object_terms ( $post->ID, $this->atts ['filter_source'], array (
					'fields' => 'ids' 
			) );
