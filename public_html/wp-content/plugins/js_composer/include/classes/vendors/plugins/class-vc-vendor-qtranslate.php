...
class Vc_Vendor_Qtranslate implements Vc_Vendor_Interface {
	...
	public function enqueueJsBackend() {
		if ($this->isValidPostType () || apply_filters ( 'vc_vendor_qtranslate_enqueue_js_backend', false )) {
			...
			wp_enqueue_script ( 'vc_vendor_qtranslate_backend', vc_asset_url ( 'js/vendors/qtranslate_backend.js' ), array (
					'vc-backend-min-js' 
			), '1.0', true );
		}
	}
	...
	public function enqueueJsFrontend() {
		if ($this->isValidPostType ()) {
			...
			wp_enqueue_script ( 'vc_vendor_qtranslate_frontend', vc_asset_url ( 'js/vendors/qtranslate_frontend.js' ), array (
	...
	public function generateSelect() {
		...
		if (is_array ( $this->languages ) && ! empty ( $this->languages )) {
			...
			foreach ( $this->languages as $lang ) {
				$output .= '<option value="' . $lang . '" link="' . add_query_arg ( array (
						'qlang' => $lang 
				), $inline_url ) . '">' . qtrans_getLanguageName ( $lang ) . '</option>';
			}
			$output .= '</select>';
		}
	...
	public function generateSelectFrontend() {
		...
		if (is_array ( $this->languages ) && ! empty ( $this->languages )) {
			...
			$inline_url = vc_frontend_editor ()->getInlineUrl ();
			foreach ( $this->languages as $lang ) {
				$output .= '<option value="' . add_query_arg ( array (
						'qlang' => $lang 
				), $inline_url ) . '"' . ($q_lang == $lang ? ' selected' : '') . ' > ' . qtrans_getLanguageName ( $lang ) . '</option > ';
			}
			$output .= '</select > ';
		}
