	...
	class WP_CustomCode_Pro {
		...
		public function __construct() {
			...
			add_action ( 'plugins_loaded', array (
					$this,
					'load_plugin_languages' 
			) );
			add_action ( 'init', array (
					$this,
					'_init' 
			) );
			add_action ( 'wp_ajax_wcjp_ajax_call', array (
					$this,
					'wcjp_ajax_call' 
			) );
			add_action ( 'wp_ajax_nopriv_wcjp_ajax_call', array (
					$this,
					'wcjp_ajax_call' 
			) );
			add_action ( 'wp_head', array (
					$this,
					'wce_inline_code_header_footer' 
			), 500 );
			add_action ( 'wp_footer', array (
					$this,
					'wce_inline_code_header_footer' 
			), 500 );
			add_shortcode ( 'wce_code', array (
					$this,
					'wce_editor_inline_code' 
			) );
		}
		/**
		 * Call PHP Script.
...
EOT;
		...
		function processor() {
			...
			try {
				...
				echo wcjp_Template::show_message ( array (
						'error' => $e->getMessage () 
				) );
			}
		}
		...
		function create_menu() {
			...
			$pagehook1 = add_menu_page ( __ ( 'CSS-JS-PHP', WCJP_TEXT_DOMAIN ), __ ( 'CSS-JS-PHP', WCJP_TEXT_DOMAIN ), 'wcjp_admin_overview', WCJP_SLUG, array (
					$this,
					'processor' 
			) );
			...
			add_action ( 'load-' . $pagehook1, array (
					$this,
					'wcjp_backend_scripts' 
			) );
		}
		...
		function load_modules_menu() {
				...
				foreach ( $modules as $module ) {
					...
					if (method_exists ( $object, 'navigation' )) {
						...
						foreach ( $object->navigation () as $nav => $title ) {
							...
							$pagehooks [] = add_submenu_page ( WCJP_SLUG, $title, $title, $nav, $nav, array (
									$this,
									'processor' 
							) );
						}
			...
			if (is_array ( $pagehooks )) {
				...
				foreach ( $pagehooks as $key => $pagehook ) {
					add_action ( 'load-' . $pagehooks [$key], array (
							$this,
							'wcjp_backend_scripts' 
					) );
				}
