...
if (! class_exists ( 'FlipperCode_HTML_Markup' )) {
	...
	class FlipperCode_HTML_Markup {
		...
		public function __construct() {
			$this->allowed_attributes = array_fill_keys ( array (
					'min',
					'max',
					'choose_button',
					'remove_button',
		 ...
		 * @param string $manage_pagetitle
		 *        	Call to Action Title.
		 * @param string $manage_pagename
		 *        	Call to Action Page Slug.
		 */
...
';
		...
		public function get_header() {
			$output = '<div class="container">
						...
						<div class="wpgmp-overview">' . $this->get_form_messages ();
		 ...
		 *
		 * @param int $column
		 *        	Set columns occupied by element.
		 */
		...
		public static function field_category_selector($name, $atts) {
			...
			if (isset ( $atts ['data_type'] ) && ! empty ( $atts ['data_type'] ) && $atts ['data_type'] != '') {
				...
				switch ($data_type [0]) {
					case 'taxonomy' :
						$terms = get_terms ( $data_type [1], array (
								'hide_empty' => 0 
						) );
					...
					case 'users' :
						$users = isset ( $data_type [1] ) ? get_users ( array (
								'role' => $data_type [1] 
						) ) : get_users ( array () );
