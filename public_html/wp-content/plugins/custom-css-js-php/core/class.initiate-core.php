...
if (! class_exists ( 'FlipperCode_Initialise_Core' )) {
	class FlipperCode_Initialise_Core {
		...
		public function _register_flippercode_globals() {
			...
			add_action ( 'wp_ajax_hide_promotional_products', array (
					$this,
					'hide_promotional_products' 
			) );
			add_action ( 'wp_ajax_check_products_updates', array (
					$this,
					'check_products_updates' 
			) );
			add_action ( 'wp_ajax_verify_envanto_purchase', array (
					$this,
					'verify_envanto_purchase' 
			) );
			add_action ( 'wp_ajax_submit_user_suggestion', array (
					$this,
					'submit_user_suggestion' 
			) );
			add_action ( 'admin_enqueue_scripts', array (
					$this,
					'load_products_common_resources' 
			) );
		}
		public function _register_product_pointers() {
			foreach ( $this->productInfo as $productprefix ) {
				add_action ( 'admin_enqueue_scripts', array (
						$this,
						'load_product_pointers' 
				) );
				add_filter ( 'flippercode-product-pointer-toplevel_page_' . $productprefix . '_view_overview', array (
						$this,
						'register_product_pointer' 
				) );
			}
		}
		function register_product_pointer($p) {
			$p [$this->productPrefix] = array (
					...
					'options' => array (
							'content' => sprintf ( '<h3> %s </h3> <p> %s </p>', __ ( $this->welcomeWindowTitle, 'plugindomain' ), __ ( $this->welcomeWindowContent, 'plugindomain' ) ),
							'position' => array (
									'edge' => 'top',
									'align' => 'left' 
							) 
		...
		function load_product_pointers($hook_suffix) {
			...
			wp_enqueue_script ( 'fc-product-pointer', $this->corePath . 'js/productsintro.js', array (
					'wp-pointer' 
			) );
		...
		function submit_user_suggestion() {
			...
			if (isset ( $_POST ['action'] ) && $_POST ['action'] == 'submit_user_suggestion' && isset ( $_POST ['uss'] ) && wp_verify_nonce ( $_POST ['uss'], 'user-suggestion-submitted' )) {
				$data = $_POST;
				$current_user = wp_get_current_user ();
				...
				$bodyargs = array (
						'wunpu_action' => 'submit-suggestion',
						'username' => $username,
						'sitename' => $sitename,
				...
				if (is_wp_error ( $response )) {
					...
					$result = array (
							'status' => '1',
							'submission_saved' => $response ['body'] 
					);
					echo $response ['body'];
				}
			} else {
			...
			if (isset ( $_POST ['action'] ) and $_POST ['action'] == 'verify_envanto_purchase' and isset ( $_POST ['pvn'] ) && wp_verify_nonce ( $_POST ['pvn'], 'purchase-verification-request' )) {
				...
				if (is_wp_error ( $response )) {
					...
					if ($response ['response'] ['code'] == '200') {
						...
						$result = array (
								'status' => '1',
								'purchase_verified' => $valid_purchase ['status'] 
						);
						...
						$result = array (
								'status' => '0',
								'purchase_verified' => $valid_purchase ['status'],
								'error' => 'Sorry! Server cannot be reached right now.' 
						);
					}
				}
