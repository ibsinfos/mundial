...
<div class="container">
	...
	<fieldset>
					<form method="post"
						action="<?php echo admin_url( 'admin.php?page='.$this->admin_listing_page_name ); ?>">
	<?php
	...
	<?php wp_nonce_field( 'wpgmp-nonce','_wpnonce',true,true ); ?>
		...
		<a href="http://www.flippercode.com/forums" target="_blank"
			title="Ask Question" class="helpdask-bootom">Helpdesk ?</a>
	</div>
</div>
<?php
		...
		function get_sortable_columns() {
			if (empty ( $this->sortable )) {
				...
				$sortable_columns [$this->primary_col] = array (
						$this->primary_col,
						false 
				);
			} else {
				...
				foreach ( $this->sortable as $sortable ) {
					$sortable_columns [$sortable] = array (
							$sortable,
							false 
					);
				}
...
<div class="alignleft actions">
	Filter By: <select name="filter_by" style="float: none;">
		<option value="">Select Filter Criteria</option>
		<option value="today" <?php selected( $filter_by_post, 'today' ); ?>>Today</option>
		<option value="yesterday"
			<?php selected( $filter_by_post, 'yesterday' ); ?>>Yesterday</option>
		<option value="this_week"
			<?php selected( $filter_by_post, 'this_week' ); ?>>This Week</option>
		<option value="this_month"
			<?php selected( $filter_by_post, 'this_month' ); ?>>This Month</option>
		<option value="last_3_months"
			<?php selected( $filter_by_post, 'last_3_months' ); ?>>Last 3 Months</option>
		<option value="last_6_months"
			<?php selected( $filter_by_post, 'last_6_months' ); ?>>Last 6 Months</option>
		<option value="last_year"
			<?php selected( $filter_by_post, 'last_year' ); ?>>Last Year</option>
		...
		<?php
				...
				?>
		<div id="custom_filter" style="display:<?php echo $display; ?>;">
		From <input type="text" class="wpgmp_datepicker" name="from_date"
			id="from_date"
			value="<?php echo sanitize_text_field( $_POST['from_date'] );  ?>">
		To <input type="text" class="wpgmp_datepicker" name="to_date"
			id="to_date"
			value="<?php echo sanitize_text_field( $_POST['to_date'] );  ?>">
	</div>
