...
if ($akismet_user && in_array ( $akismet_user->status, array (
		'active',
		'active-dunning',
		'no-sub',
		'missing',
...
<p><?php esc_html_e('Akismet eliminates spam from your site. Register below to get started.', 'akismet'); ?></p>
	<div class="activate-highlight activate-option">
		...
		<form name="akismet_activate" id="akismet_activate"
			action="https://akismet.com/get/" method="post" class="right"
			target="_blank">
			<input type="hidden" name="passback_url"
				value="<?php echo esc_url( Akismet_Admin::get_page_url() ); ?>" /> <input
				type="hidden" name="blog"
				value="<?php echo esc_url( get_option( 'home' ) ); ?>" /> <input
				type="hidden" name="auto-connect"
				value="<?php echo esc_attr( $akismet_user->ID ); ?>" /> <input
				type="hidden" name="redirect" value="plugin-signup" /> <input
	...
	<div class="activate-highlight activate-option">
		...
		<form name="akismet_activate" id="akismet_activate"
			...
			<input type="hidden" name="passback_url"
				...
				value="<?php echo esc_attr( $akismet_user->ID ); ?>" /> <input
				type="hidden" name="redirect" value="upgrade" /> <input
				type="submit" class="button button-primary"
	...
	<div class="activate-highlight activate-option">
		...
		<form name="akismet_use_wpcom_key"
			...
			<input type="hidden" name="key"
				value="<?php echo esc_attr( $akismet_user->api_key );?>" /> <input
	...
	<div class="activate-highlight secondary activate-option">
		...
		<form action="<?php echo esc_url( Akismet_Admin::get_page_url() ); ?>"
			...
			<input id="key" name="key" type="text" size="15" value=""
				class="regular-text code"> <input type="hidden" name="action"
				value="enter-key">
		<?php wp_nonce_field( Akismet_Admin::NONCE )?>
	...
	<div class="activate-highlight secondary activate-option">
		...
		<form action="<?php echo esc_url( Akismet_Admin::get_page_url() ); ?>"
			...
			<input id="key" name="key" type="text" size="15"
				value="<?php echo esc_attr( Akismet::get_api_key() ); ?>"
				class="regular-text code"> <input type="hidden" name="action"
				value="enter-key">
		<?php wp_nonce_field( Akismet_Admin::NONCE ); ?>
		<input type="submit" name="submit" id="submit"
