...
class WPCF7_Contact_Form_List_Table extends WP_List_Table {
	...
	function column_title($item) {
		...
		if (wpcf7_validate_configuration () && current_user_can ( 'wpcf7_edit_contact_form', $item->id () )) {
			...
			if ($count_errors = $config_validator->count_errors ()) {
				$error_notice = sprintf ( _n ( '%s configuration error detected', '%s configuration errors detected', $count_errors, 'contact-form-7' ), number_format_i18n ( $count_errors ) );
				$output .= sprintf ( '<div class="config-error"><span class="dashicons dashicons-warning"></span> %s</div>', $error_notice );
			}
		}
		
		$actions = array (
				'edit' => sprintf ( '<a href="%1$s">%2$s</a>', esc_url ( $edit_link ), esc_html ( __ ( 'Edit', 'contact-form-7' ) ) ) 
		);
		
		if (current_user_can ( 'wpcf7_edit_contact_form', $item->id () )) {
			$copy_link = wp_nonce_url ( add_query_arg ( array (
					'action' => 'copy' 
			), $url ), 'wpcf7-copy-contact-form_' . absint ( $item->id () ) );
			
			$actions = array_merge ( $actions, array (
					'copy' => sprintf ( '<a href="%1$s">%2$s</a>', esc_url ( $copy_link ), esc_html ( __ ( 'Duplicate', 'contact-form-7' ) ) ) 
			) );
		}
	...
	function column_shortcode($item) {
		...
		foreach ( $shortcodes as $shortcode ) {
			$output .= "\n" . '<span class="shortcode"><input type="text"' . ' onfocus="this.select();" readonly="readonly"' . ' value="' . esc_attr ( $shortcode ) . '"' . ' class="large-text code" /></span>';
		}
