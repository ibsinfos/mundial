...
function wpcf7_admin_menu() {
	...
	add_menu_page ( __ ( 'Contact Form 7', 'contact-form-7' ), __ ( 'Contact', 'contact-form-7' ), 'wpcf7_read_contact_forms', 'wpcf7', 'wpcf7_admin_management_page', 'dashicons-email', $_wp_last_object_menu );
	...
	if ($integration->service_exists ()) {
		$integration = add_submenu_page ( 'wpcf7', __ ( 'Integration with Other Services', 'contact-form-7' ), __ ( 'Integration', 'contact-form-7' ), 'wpcf7_manage_integration', 'wpcf7-integration', 'wpcf7_admin_integration_page' );
...
function wpcf7_admin_enqueue_scripts($hook_suffix) {
	...
	wp_enqueue_script ( 'wpcf7-admin', wpcf7_plugin_url ( 'admin/js/scripts.js' ), array (
			'jquery',
			'jquery-ui-tabs' 
	), WPCF7_VERSION, true );
	...
	if (($post = wpcf7_get_current_contact_form ()) && current_user_can ( 'wpcf7_edit_contact_form', $post->id () ) && wpcf7_validate_configuration ()) {
		...
		$args ['configValidator'] ['errors'] = $config_validator->collect_error_messages ();
	}
	...
	wp_enqueue_script ( 'wpcf7-admin-taggenerator', wpcf7_plugin_url ( 'admin/js/tag-generator.js' ), array (
			'jquery',
			'thickbox',
			'wpcf7-admin' 
	), WPCF7_VERSION, true );
...
function wpcf7_admin_management_page() {
	...
	if ('validate' == wpcf7_current_action () && wpcf7_validate_configuration () && current_user_can ( 'wpcf7_edit_contact_forms' )) {
...
<?php
	if (current_user_can ( 'wpcf7_edit_contact_forms' )) {
		echo sprintf ( '<a href="%1$s" class="add-new-h2">%2$s</a>', esc_url ( menu_page_url ( 'wpcf7-new', false ) ), esc_html ( __ ( 'Add New', 'contact-form-7' ) ) );
	}
...
function wpcf7_admin_updated_message() {
	...
	if ('failed' == $_REQUEST ['message']) {
		$updated_message = __ ( "There was an error saving the contact form.", 'contact-form-7' );
	...
	if ('validated' == $_REQUEST ['message']) {
		...
		$count_invalid = isset ( $bulk_validate ['count_invalid'] ) ? absint ( $bulk_validate ['count_invalid'] ) : 0;
		
		if ($count_invalid) {
			$updated_message = sprintf ( _n ( "Configuration validation completed. An invalid contact form was found.", "Configuration validation completed. %s invalid contact forms were found.", $count_invalid, 'contact-form-7' ), number_format_i18n ( $count_invalid ) );
...
function wpcf7_plugin_action_links($links, $file) {
	...
	$settings_link = '<a href="' . menu_page_url ( 'wpcf7', false ) . '">' . esc_html ( __ ( 'Settings', 'contact-form-7' ) ) . '</a>';
