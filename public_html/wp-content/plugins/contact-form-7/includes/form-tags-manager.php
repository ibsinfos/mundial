...
class WPCF7_FormTagsManager {
	...
	public function tag_type_supports($tag, $feature) {
		...
		if (isset ( $this->tag_types [$tag] ['features'] )) {
			return ( bool ) array_intersect ( array_keys ( array_filter ( $this->tag_types [$tag] ['features'] ) ), $feature );
		}
	...
	public function normalize($content) {
		...
		$content = preg_replace_callback ( '/' . $this->tag_regex () . '/s', array (
				$this,
				'normalize_callback' 
		), $content );
	...
	private function normalize_callback($m) {
		...
		$result = $m [1] . '[' . $tag . ($attr ? ' ' . $attr : '') . ($m [4] ? ' ' . $m [4] : '') . ']' . ($content ? $content . '[/' . $tag . ']' : '') . $m [6];
	...
	public function scan($content, $replace = false) {
		...
		if ($replace) {
			...
			preg_replace_callback ( '/' . $this->tag_regex () . '/s', array (
					$this,
					'scan_callback' 
			), $content );
	...
	public function filter($input, $cond) {
		...
		$cond ['feature'] = is_string ( $cond ['feature'] ) ? trim ( $cond ['feature'] ) : '';
		...
		foreach ( $tags as $tag ) {
			...
			if ($cond ['feature'] && ! $this->tag_type_supports ( $tag->type, $cond ['feature'] )) {
				continue;
			}
	...
	private function scan_callback($m, $replace = false) {
		...
		if (is_array ( $attr )) {
			if (is_array ( $attr ['options'] )) {
				if ($this->tag_type_supports ( $tag, 'name-attr' ) && ! empty ( $attr ['options'] )) {
