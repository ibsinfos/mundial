...
class WPCF7_ConfigValidator {
	...
	public static function get_doc_link($error_code = '') {
		$url = __ ( 'https://contactform7.com/configuration-errors/', 'contact-form-7' );
	...
	public function count_errors($args = '') {
		...
		foreach ( $this->errors as $key => $errors ) {
			...
			if ($args ['section'] && $key != $args ['section'] && preg_replace ( '/\..*$/', '', $key, 1 ) != $args ['section']) {
				continue;
			}
		...
		foreach ( $this->errors as $section => $errors ) {
			...
			foreach ( $errors as $error ) {
				if (empty ( $error ['args'] ['message'] )) {
					...
					$message = $this->build_message ( $error ['args'] ['message'], $error ['args'] ['params'] );
				}
	...
	public function save() {
		...
		if ($this->errors) {
			update_post_meta ( $this->contact_form->id (), '_config_errors', $this->errors );
		}
	}
	public function restore() {
		$config_errors = get_post_meta ( $this->contact_form->id (), '_config_errors', true );
	...
	public function replace_mail_tags_with_minimum_input($matches) {
		...
		$form_tags = $this->contact_form->scan_form_tags ( array (
				'name' => $tagname 
		) );
	...
	public function detect_multiple_controls_in_label($section, $content) {
		...
		if (preg_match_all ( $pattern, $content, $matches )) {
			...
			foreach ( $matches [1] as $insidelabel ) {
				...
				foreach ( $tags as $tag ) {
					$is_multiple_controls_container = wpcf7_form_tag_supports ( $tag->type, 'multiple-controls-container' );
					...
					if (1 < $fields_count) {
						return $this->add_error ( $section, self::error_multiple_controls_in_label, array (
	...
	public function detect_unavailable_names($section, $content) {
		...
		if ($ng_names) {
			...
			return $this->add_error ( $section, self::error_unavailable_names, array (
					'message' => __ ( "Unavailable names (%names%) are used for form controls.", 'contact-form-7' ),
					'params' => array (
							'names' => implode ( ', ', $ng_names ) 
					),
	...
	public function validate_mail($template = 'mail') {
		...
		$callback = array (
				$this,
				'replace_mail_tags_with_minimum_input' 
		);
		
		$subject = $components ['subject'];
		$subject = new WPCF7_MailTaggedText ( $subject, array (
				'callback' => $callback 
		) );
		...
		$sender = new WPCF7_MailTaggedText ( $sender, array (
				'callback' => $callback 
		) );
		$sender = $sender->replace_tags ();
		$sender = wpcf7_strip_newline ( $sender );
		
		if (! $this->detect_invalid_mailbox_syntax ( sprintf ( '%s.sender', $template ), $sender ) && ! wpcf7_is_email_in_site_domain ( $sender )) {
			$this->add_error ( sprintf ( '%s.sender', $template ), self::error_email_not_in_site_domain, array (
					'link' => self::get_doc_link ( 'email_not_in_site_domain' ) 
			) );
		}
		
		$recipient = $components ['recipient'];
		$recipient = new WPCF7_MailTaggedText ( $recipient, array (
				'callback' => $callback 
		) );
		...
		$additional_headers = new WPCF7_MailTaggedText ( $additional_headers, array (
				'callback' => $callback 
		) );
		$additional_headers = $additional_headers->replace_tags ();
		$additional_headers = explode ( "\n", $additional_headers );
		$mailbox_header_types = array (
				'reply-to',
				'cc',
				'bcc' 
		);
		...
		foreach ( $additional_headers as $header ) {
			...
			if (! preg_match ( '/^([0-9A-Za-z-]+):(.+)$/', $header, $matches )) {
				...
				if (in_array ( strtolower ( $header_name ), $mailbox_header_types )) {
					$this->detect_invalid_mailbox_syntax ( sprintf ( '%s.additional_headers', $template ), $header_value, array (
		...
		if ($invalid_mail_header_exists) {
			$this->add_error ( sprintf ( '%s.additional_headers', $template ), self::error_invalid_mail_header, array (
					'link' => self::get_doc_link ( 'invalid_mail_header' ) 
			) );
		}
		
		$body = $components ['body'];
		$body = new WPCF7_MailTaggedText ( $body, array (
				'callback' => $callback 
		) );
		...
		if ('' !== $components ['attachments']) {
			foreach ( explode ( "\n", $components ['attachments'] ) as $line ) {
				...
				$this->detect_file_not_found ( sprintf ( '%s.attachments', $template ), $line );
			}
		}
	}
	public function detect_invalid_mailbox_syntax($section, $content, $args = '') {
		...
		if (! wpcf7_is_mailbox_list ( $content )) {
			return $this->add_error ( $section, self::error_invalid_mailbox_syntax, $args );
		}
	...
	public function detect_maybe_empty($section, $content) {
		if ('' === $content) {
			return $this->add_error ( $section, self::error_maybe_empty, array (
	...
	public function detect_file_not_found($section, $content) {
		...
		if (! @is_readable ( $path ) || ! @is_file ( $path )) {
			return $this->add_error ( $section, self::error_file_not_found, array (
	...
	public function detect_html_in_message($section, $content) {
		...
		if ($stripped != $content) {
			return $this->add_error ( $section, self::error_html_in_message, array (
