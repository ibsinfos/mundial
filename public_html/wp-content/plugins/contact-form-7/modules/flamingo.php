...
function wpcf7_flamingo_submit($contactform, $result) {
	...
	$cases = ( array ) apply_filters ( 'wpcf7_flamingo_submit_if', array (
			'spam',
			'mail_sent',
			'mail_failed' 
	) );
	...
	$fields_senseless = $contactform->scan_form_tags ( array (
	...
	$result += array (
			'flamingo_contact_id' => empty ( $flamingo_contact ) ? 0 : absint ( $flamingo_contact->id ),
			'flamingo_inbound_id' => empty ( $flamingo_inbound ) ? 0 : absint ( $flamingo_inbound->id ) 
	);
...
function wpcf7_flamingo_add_channel($slug, $name = '') {
	...
	if (! $parent) {
		$parent = wp_insert_term ( __ ( 'Contact Form 7', 'contact-form-7' ), Flamingo_Inbound_Message::channel_taxonomy, array (
				'slug' => 'contact-form-7' 
		) );
	...
	if (! $channel) {
		$channel = wp_insert_term ( $name, Flamingo_Inbound_Message::channel_taxonomy, array (
				'slug' => $slug,
				'parent' => $parent 
		) );
...
function wpcf7_flamingo_serial_number($output, $name, $html) {
	...
	if ($channel_id) {
		return 1 + ( int ) Flamingo_Inbound_Message::count ( array (
