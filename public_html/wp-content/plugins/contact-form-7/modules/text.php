...
function wpcf7_add_form_tag_text() {
	wpcf7_add_form_tag ( array (
			...
			'tel',
			'tel*' 
	), 'wpcf7_text_form_tag_handler', array (
			'name-attr' => true 
	) );
}
function wpcf7_text_form_tag_handler($tag) {
	...
	if (in_array ( $tag->basetype, array (
			'email',
			'url',
			'tel' 
	) )) {
	...
	<fieldset>
		...
		<table class="form-table">
			<tbody>
				...
				<tr>
					<th scope="row"><label
						for="<?php echo esc_attr( $args['content'] . '-values' ); ?>"><?php echo esc_html( __( 'Default value', 'contact-form-7' ) ); ?></label></th>
...
<?php if ( 'text' == $type ) : ?>
		<label> <input type="checkbox" name="akismet:author" class="option" />
			<?php echo esc_html( __( "This field requires author's name", 'contact-form-7' ) ); ?>
		</label>
<?php elseif ( 'email' == $type ) : ?>
		<label> <input type="checkbox" name="akismet:author_email"
								class="option" />
			<?php echo esc_html( __( "This field requires author's email address", 'contact-form-7' ) ); ?>
		</label>
<?php elseif ( 'url' == $type ) : ?>
		<label> <input type="checkbox" name="akismet:author_url"
								class="option" />
