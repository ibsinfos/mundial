...
if ($is_front || $is_blog) {
	...
	foreach ( $sidebars as $s ) :
		?>
<span data-sidebar="<?php echo esc_attr( $s ); ?>"
...
endforeach
	...
	foreach ( $sidebars as $s ) {
		...
		if ($replaced) {
			$content .= sprintf ( '<dt data-sidebar="%s" data-replaced="%s" class="cs-key %s">', esc_attr ( $s ), isset ( $selected [$s] ) ? esc_attr ( $selected [$s] ) : '', esc_attr ( $class, 'custom-sidebars' ) );
