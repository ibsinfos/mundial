...
<?php if ( ! empty( $import ) ) : ?>
	...
	<h3 class="title"><?php _e( 'Custom Sidebars', 'custom-sidebars' ); ?></h3>
		...
		<?php _e( 'Mark the sidebars that you want to import.', 'custom-sidebars' ); ?>
			...
			<p>
				<label for="import-widgets"> <input type="checkbox"
					id="import-widgets" name="import_widgets" />
				...
				<thead>
					<tr>
						...
						<th scope="col" id="description"
							class="manage-column column-description"><?php _e( 'Description', 'custom-sidebars' ); ?></th>
						<th scope="col" id="note" class="manage-column column-note"><?php _e( 'Note', 'custom-sidebars' ); ?></th>
						<th scope="col" id="widgets" class="manage-column column-widgets"
							style="display: none"><?php _e( 'Widgets', 'custom-sidebars' ); ?></th>
					</tr>
				</thead>
				<tbody>
<?php
	foreach ( $import ['sidebars'] as $sidebar ) {
        ...
        <tr class="<?php echo esc_attr( $alternate ); ?>">
						<th scope="row" class="check-column"><input type="checkbox"
							name="import_sb_<?php echo esc_attr( $sidebar['id'] ); ?>"
							id="<?php echo esc_attr( $id ); ?>" /></th>
						<td class="name column-name"><label
							for="<?php echo esc_attr( $id ); ?>"><?php echo esc_html( $sidebar['name'] ); ?></label></td>
...
<?php
	...
	foreach ( $theme_sidebars as $sidebar ) {
		if (isset ( $import ['widgets'] [$sidebar ['id']] )) {
			...
			?>
            <tr
						class="import-widgets <?php echo esc_attr( $alternate ); ?>"
						style="display: none">
						<th scope="row" class="check-column"><input type="checkbox"
							name="import_sb_<?php echo esc_attr( $sidebar['id'] ); ?>"
							id="<?php echo esc_attr( $id ); ?>" /></th>
...
<?php
	...
	?>
			...
			<p>
				<label for="import-config"> <input type="checkbox"
					id="import-config" name="import_plugin_config" />
			...
			<input type="hidden" name="import_data"
				value="<?php echo esc_attr( base64_encode( json_encode( (object) $import ) ) ); ?>" />
			<p class="buttons">
				<button type="button" class="btn-cancel button-link">Cancel</button>
				<button class="button-primary btn-import">
					<i class="dashicons dashicons-migrate"></i> Import selected items
				</button>
			</p>
		</form>
