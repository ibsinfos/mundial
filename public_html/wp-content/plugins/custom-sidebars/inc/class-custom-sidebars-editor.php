...
class CustomSidebarsEditor extends CustomSidebars {
	...
	private function __construct() {
		...
		add_action ( 'cs_ajax_request', array (
				$this,
				'handle_ajax' 
		) );
		...
		if ($user_can_save) {
			...
			foreach ( $posttypes as $pt ) {
				add_filter ( 'manage_' . $pt->name . '_posts_columns', array (
						$this,
						'post_columns' 
				) );
				
				add_action ( 'manage_' . $pt->name . '_posts_custom_column', array (
						$this,
						'post_column_content' 
				), 10, 2 );
			}
		...
		add_filter ( 'screen_settings', array (
				$this,
				'add_capabilities_select_box' 
		), 10, 2 );
		add_action ( 'wp_ajax_custom_sidebars_metabox_roles', array (
				$this,
				'update_custom_sidebars_metabox_roles' 
		) );
	}
	...
	public function handle_ajax($action) {
		...
		if (! current_user_can ( self::$cap_required )) {
			$req = self::req_err ( $req, __ ( 'You do not have permission for this', 'custom-sidebars' ) );
		} else {
			switch ($action) {
	 ...
	 * object.
	 * When $req->id is empty a new sidebar will be created. Otherwise
	 ...
	 * @param array $data
	 *        	Sidebar data to save (typically this is $_POST).
	...
	private function save_item($req, $data) {
		 ...
		 * check nonce
		 */
		if (! isset ( $data ['_wpnonce'] ) || ! wp_verify_nonce ( $data ['_wpnonce'], 'custom-sidebars-edit-sidebar' )) {
			return self::req_err ( $req, __ ( 'You have no permission to do this operation.', 'custom-sidebars' ) );
		}
		...
		if (empty ( $sb_name )) {
			return self::req_err ( $req, __ ( 'Sidebar-name cannot be empty', 'custom-sidebars' ) );
		}
		...
		if (empty ( $sb_id )) {
			...
			if (! $sidebar) {
				return self::req_err ( $req, __ ( 'The sidebar does not exist', 'custom-sidebars' ) );
			}
		}
		...
		if ('insert' == $action) {
			...
			if (! $found) {
				return self::req_err ( $req, __ ( 'The sidebar was not found', 'custom-sidebars' ) );
			}
		}
	...
	private function delete_item($req, $data) {
		 ...
		 * check nonce
		 */
		if (! isset ( $data ['_wpnonce'] ) || ! wp_verify_nonce ( $data ['_wpnonce'], 'custom-sidebars-delete-sidebar' )) {
			return self::req_err ( $req, __ ( 'You have no permission to do this operation.', 'custom-sidebars' ) );
		}
		$sidebars = self::get_custom_sidebars ();
	...
	private function get_location_data($req) {
		...
		foreach ( $raw_taxonomies ['custom'] as $t ) {
			$taxonomy = $t->name;
			if (isset ( $defaults ['taxonomies_archive'] ) && isset ( $defaults ['taxonomies_archive'] [$taxonomy] )) {
	...
	private function set_location_data($req) {
		...
		foreach ( $sidebars as $sb_id ) {
			...
			foreach ( $raw_posttype as $item ) {
				...
				} elseif (isset ( $options ['post_type_single'] [$pt] [$sb_id] ) && $options ['post_type_single'] [$pt] [$sb_id] == $req->id) {
				...
				} elseif (isset ( $options ['post_type_archive'] [$pt] [$sb_id] ) && $options ['post_type_archive'] [$pt] [$sb_id] == $req->id) {
			...
			foreach ( $raw_cat as $item ) {
				...
				} elseif (isset ( $options ['category_single'] [$cat] [$sb_id] ) && $options ['category_single'] [$cat] [$sb_id] == $req->id) {
				...
				} elseif (isset ( $options ['category_archive'] [$cat] [$sb_id] ) && $options ['category_archive'] [$cat] [$sb_id] == $req->id) {
			...
			foreach ( $special_arc as $key ) {
				...
				} elseif (isset ( $options [$key] [$sb_id] ) && $options [$key] [$sb_id] == $req->id) {
			...
			foreach ( $raw_authors as $user ) {
				$key = $user->ID;
				if (is_array ( @$data ['arc-aut'] [$sb_id] ) && in_array ( $key, $data ['arc-aut'] [$sb_id] )) {
					$options ['author_archive'] [$key] [$sb_id] = $req->id;
				} elseif (isset ( $options ['author_archive'] [$key] [$sb_id] ) && $options ['author_archive'] [$key] [$sb_id] == $req->id) {
			...
			foreach ( $raw_taxonomies ['custom'] as $taxonomy ) {
				$key = '_taxonomy_' . $taxonomy;
				if (isset ( $data ['arc'] [$sb_id] ) && is_array ( $data ['arc'] [$sb_id] ) && in_array ( $key, $data ['arc'] [$sb_id] )) {
					$options ['taxonomies_archive'] [$taxonomy] [$sb_id] = $req->id;
				} elseif (isset ( $options ['taxonomies_archive'] [$taxonomy] [$sb_id] ) && $options ['taxonomies_archive'] [$taxonomy] [$sb_id] == $req->id) {
			 ...
			 * @since 3.0.7
			 */
			if (isset ( $data ['arc'] [$sb_id] ) && is_array ( $data ['arc'] [$sb_id] ) && in_array ( '_category', $data ['arc'] [$sb_id] )) {
				$options ['category_archive'] [$sb_id] = $req->id;
			} elseif (isset ( $options ['category_archive'] [$sb_id] ) && $options ['category_archive'] [$sb_id] == $req->id) {
	...
	public function add_meta_box() {
		 ...
		 */
		if (defined ( 'CUSTOM_SIDEBAR_DISABLE_METABOXES' ) && CUSTOM_SIDEBAR_DISABLE_METABOXES == true) {
			return false;
		}
		...
		if ($pt_obj->publicly_queryable || $pt_obj->public) {
			add_meta_box ( 'customsidebars-mb', __ ( 'Sidebars', 'custom-sidebars' ), array (
					$this,
					'print_metabox_editor' 
			), $post_type, 'side' );
		}
	}
	 ...
	 * @param string $type
	 *        	Which form to display. 'metabox/quick-edit/col-sidebars'.
	 */
	 ...
	 * @param string $field        	
	 * @param string $value        	
	 * @param bool $update_string
	 *        	If false then the translation will only be
	...
	public function post_columns($columns) {
		...
		$columns = array_merge ( array_slice ( $columns, 0, $pos ), $insert, array_slice ( $columns, $pos ) );
	 ...
	 *        	Column-Key defined in post_columns above.
	 * @param int $post_id
	 *        	Post-ID
	 */
	 ...
	 *        	Post-type that is currently edited.
	 */
	...
	public function post_quick_edit_js() {
		...
		if (! self::supported_post_type ( $current_screen->post_type )) {
			return false;
		}
		...
		if ('widgets' == $screen->base && current_user_can ( 'manage_options' )) {
			...
			foreach ( $roles as $role => $data ) {
				if (isset ( $data ['capabilities'] [self::$cap_required] ) && $data ['capabilities'] [self::$cap_required]) {
					...
					$screen_settings .= sprintf ( '<label><input type="checkbox" name="cs-roles[]" value="%s" %s /> %s</label>', esc_attr ( $role ), checked ( $checked, true, false ), esc_html ( $data ['name'] ) );
				}
			}
