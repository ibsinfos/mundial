...
class CustomSidebarsReplacer extends CustomSidebars {
	...
	private function __construct() {
		...
		add_action ( 'register_sidebar', array (
				$this,
				'translate_sidebar' 
		) );
	...
	public function register_custom_sidebars() {
		...
		foreach ( $sb as $sidebar ) {
			 ...
			 *       
			 * @param array $sidebar
			 *        	Options used by WordPress to display
			 *        	the sidebar.
			 */
	...
	public function replace_sidebars() {
		 ...
		 *        	Array of the default sidebars for the page.
		 */
		...
		foreach ( $replacements as $sb_id => $replace_info ) {
			...
			if ($check) {
				...
				if (sizeof ( $original_widgets [$replacement] ) == 0) {
					...
					$_wp_sidebars_widgets [$sb_id] = array (
							'csemptywidget' 
					);
				} else {
					 ...
					 * after_title, .
					 * ..) then we need to strip-slashes for this
	 ...
	 *
	 * @param array $options
	 *        	Plugin options with the replacement rules.
	...
	public function determine_replacements($options) {
		...
		if (is_single ()) {
			...
			if (0 != $current_category && $replacements_todo > 0) {
				foreach ( $sidebars as $sb_id ) {
					if (isset ( $options ['category_archive'] ) && isset ( $options ['category_archive'] [$current_category] ) && isset ( $options ['category_archive'] [$current_category] [$sb_id] )) {
						$replacements [$sb_id] = array (
			...
			foreach ( $sidebars as $sb_id ) {
				if (isset ( $options ['taxonomies_archive'] ) && isset ( $options ['taxonomies_archive'] [$taxonomy] ) && isset ( $options ['taxonomies_archive'] [$taxonomy] [$sb_id] )) {
					$replacements [$sb_id] = array (
		 ...
		 *        	List of the final/replaced sidebars.
		 */
	 ...
	 * @since 1.0.0
	 * @param string $sb_id
	 *        	The original sidebar (the one that is replaced).
	 * @param string $replacement
	 *        	ID of the custom sidebar that should be used.
	 * @param string $method
	 *        	Info where the replacement setting is saved.
	 * @param int|string $extra_index
	 *        	Depends on $method - can be either one:
	...
	public function is_valid_replacement($sb_id, $replacement, $method, $extra_index) {
		...
		if ('particular' == $method) {
			...
			if (isset ( $options [$method] )) {
				if (- 1 != $extra_index && isset ( $options [$method] [$extra_index] ) && isset ( $options [$method] [$extra_index] [$sb_id] )) {
					unset ( $options [$method] [$extra_index] [$sb_id] );
					self::set_options ( $options );
				}
				
				if (1 == $extra_index && isset ( $options [$method] ) && isset ( $options [$method] [$sb_id] )) {
	...
	/**
	 * Returns an empty dummy-widget.
	 * This dummy widget is used when a custom
	...
	public function has_wrapper_code($sidebar) {
		return (strlen ( trim ( $sidebar ['before_widget'] ) ) || strlen ( trim ( $sidebar ['after_widget'] ) ) || strlen ( trim ( $sidebar ['before_title'] ) ) || strlen ( trim ( $sidebar ['after_title'] ) ));
	}
	 ...
	 * @since 2.0.9.7
	 * @param array $sidebar
	 *        	The sidebar that was registered
	 */
