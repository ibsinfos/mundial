<?php
add_action ( 'cs_init', array (
		'CustomSidebarsVisibility',
		'instance' 
) );
...
class CustomSidebarsVisibility extends CustomSidebars {
	...
	private function __construct() {
		if (is_admin ()) {
			...
			add_action ( 'cs_ajax_request_get', array (
					$this,
					'handle_ajax' 
			) );
		} else {
	...
	public function admin_widget_button($widget, $return, $instance) {
		...
		if (false === $Loaded) {
			...
			if (! self::$filtered_tax_list) {
				foreach ( $tax_list as $index => $tax_item ) {
					$tags = get_terms ( $tax_item->name, array (
							'hide_empty' => false 
					) );
					if (empty ( $tags )) {
		...
		?>
<div
	class="csb-visibility csb-visibility-<?php echo esc_attr( $widget->id ); ?>"
	...
	data-number="<?php echo esc_attr( absint( @$widget->number ) ); ?>">
		...
		<?php if ( ! isset( $_POST['csb-visibility-button'] ) ) : ?>
			<a href="#" class="button csb-visibility-button"><span
		class="dashicons dashicons-visibility"></span> <?php _e( 'Visibility', 'custom-sidebars' ); ?></a>
		<?php else : ?>
			<script>jQuery(function() { jQuery('.csb-visibility-<?php echo esc_js( $widget->id ); ?>').closest('.widget').trigger('csb:update'); }); </script>
		<?php endif; ?>

		<div class="csb-visibility-inner" <?php if ( ! $is_visible ) : ?>
		style="display: none" <?php endif; ?>>
		<input type="hidden" name="csb_visible" class="csb-visible-flag"
			value="<?php echo esc_attr( $is_visible ); ?>" />

		<div class="csb-option-row csb-action">
			<label for="<?php echo esc_attr( $widget->id ); ?>-action"
				class="lbl-show-if toggle-action" <?php if ( ! $action_show ) : ?>
				style="display: none" <?php endif; ?>><?php _e( '<b>Show</b> widget if:', 'custom-sidebars' ); ?></label>
			<label for="<?php echo esc_attr( $widget->id ); ?>-action"
				class="lbl-hide-if toggle-action" <?php if ( $action_show ) : ?>
				style="display: none" <?php endif; ?>><?php _e( '<b>Hide</b> widget if:', 'custom-sidebars' ); ?></label>
			<input type="hidden"
				id="<?php echo esc_attr( $widget->id ); ?>-action"
				name="csb_visibility[action]"
				value="<?php echo esc_attr( $data['action'] ); ?>" /> <i
				class="dashicons dashicons-plus choose-filters show-on-hover action"></i>
			<ul class="dropdown" style="display: none">
				<li class="csb-group"><?php _e( 'Filters', 'custom-sidebars' ); ?></li>
				<li class="add-filter" data-for=".csb-date" style="display: none">
					<?php _e( 'Date', 'custom-sidebars' ); ?>
				</li>
				<li class="add-filter" data-for=".csb-guest"
					<?php if ( ! empty( $cond['guest'] ) ) : ?> style="display: none"
					<?php endif; ?>>
					<?php _e( 'Guests', 'custom-sidebars' ); ?>
				</li>
				<li class="add-filter" data-for=".csb-roles"
					<?php if ( ! empty( $cond['roles'] ) ) : ?> style="display: none"
					<?php endif; ?>>
					<?php _e( 'Roles', 'custom-sidebars' ); ?>
				</li>
				<?php if ( false != $membership_levels ) : ?>
					<li class="add-filter" data-for=".csb-membership">
						<?php _e( 'Membership', 'custom-sidebars' ); ?>
					</li>
				<?php endif; ?>
				<?php if ( false != $membership2_items ) : ?>
					<li class="add-filter" data-for=".csb-membership2">
						<?php _e( 'Membership2', 'custom-sidebars' ); ?>
					</li>
				<?php endif; ?>
				<li class="add-filter" data-for=".csb-pagetypes"
					<?php if ( ! empty( $cond['pagetypes'] ) ) : ?>
					style="display: none" <?php endif; ?>>
					<?php _e( 'Special pages', 'custom-sidebars' ); ?>
				</li>
				<li class="add-filter" data-for=".csb-posttypes"
					<?php if ( ! empty( $cond['posttypes'] ) ) : ?>
					style="display: none" <?php endif; ?>>
...
foreach ( $tax_list as $tax_item ) :
			...
			?>
					...
					<?php if ( ! empty( $cond[ $row_id ] ) ) : ?> style="display: none"
					<?php endif; ?>>
		...
		<div class="csb-option-row csb-always"
			<?php if ( ! $data['always'] ) : ?> style="display: none"
			<?php endif; ?>>
...
/* DATE */ /*
		...
		<?php /* GUEST */ ?>
		<div class="csb-option-row csb-guest"
			<?php if ( empty( $cond['guest'] ) ) : ?> style="display: none"
			<?php endif; ?>>
			<label for="<?php echo esc_attr( $widget->id ); ?>-guest1"
				style="padding-top: 10px; margin-bottom: 0"> <input
				id="<?php echo esc_attr( $widget->id ); ?>-guest1" type="radio"
				name="<?php echo esc_attr( $block_name ); ?>[guest][]" value="guest"
				<?php checked( in_array( 'guest', $cond['guest'] ) ); ?> /> <span
				class="csb-and" style="display: none"><?php _e( 'AND', 'custom-sidebars' ); ?></span>
				<?php _e( 'User is not logged-in (Guest)', 'custom-sidebars' ); ?><br />
			</label> <label for="<?php echo esc_attr( $widget->id ); ?>-guest2"
				style="border: 0; margin-bottom: 0"> <input
				id="<?php echo esc_attr( $widget->id ); ?>-guest2" type="radio"
				name="<?php echo esc_attr( $block_name ); ?>[guest][]"
				value="member"
				<?php checked( in_array( 'member', $cond['guest'] ) ); ?> /> <span
				class="csb-and" style="display: none"><?php _e( 'AND', 'custom-sidebars' ); ?></span>
				<?php _e( 'User is logged-in (Member)', 'custom-sidebars' ); ?>
			</label> <i
				class="dashicons dashicons-trash clear-filter show-on-hover action"></i>
		</div>
		...
		<div class="csb-option-row csb-roles"
			<?php if ( empty( $cond['roles'] ) ) : ?> style="display: none"
			<?php endif; ?>>
			<label for="<?php echo esc_attr( $widget->id ); ?>-roles"> <span
				class="csb-and" style="display: none"><?php _e( 'AND', 'custom-sidebars' ); ?></span>
				<?php _e( 'User has role', 'custom-sidebars' ); ?>
			</label> <i
				class="dashicons dashicons-trash clear-filter show-on-hover action"></i>
			<select id="<?php echo esc_attr( $widget->id ); ?>-roles"
				name="<?php echo esc_attr( $block_name ); ?>[roles][]"
				multiple="multiple">
		...
		<div class="csb-option-row csb-membership"
			<?php if ( empty( $cond['membership'] ) ) : ?> style="display: none"
			<?php endif; ?>>
			<label for="<?php echo esc_attr( $widget->id ); ?>-membership"> <span
				class="csb-and" style="display: none"><?php _e( 'AND', 'custom-sidebars' ); ?></span>
				<?php _e( 'User has Membership Level', 'custom-sidebars' ); ?>
			</label> <i
				class="dashicons dashicons-trash clear-filter show-on-hover action"></i>
			<select id="<?php echo esc_attr( $widget->id ); ?>-membership"
				name="<?php echo esc_attr( $block_name ); ?>[membership][]"
				multiple="multiple">
		...
		<div class="csb-option-row csb-membership2"
			<?php if ( empty( $cond['membership2'] ) ) : ?> style="display: none"
			<?php endif; ?>>
			<label for="<?php echo esc_attr( $widget->id ); ?>-membership2"> <span
				class="csb-and" style="display: none"><?php _e( 'AND', 'custom-sidebars' ); ?></span>
				<?php _e( 'User has Membership', 'custom-sidebars' ); ?>
			</label> <i
				class="dashicons dashicons-trash clear-filter show-on-hover action"></i>
			<select id="<?php echo esc_attr( $widget->id ); ?>-membership2"
				name="<?php echo esc_attr( $block_name ); ?>[membership2][]"
				multiple="multiple">
		...
		<?php /* PAGE TYPES */ ?>
		<div class="csb-option-row csb-pagetypes"
			<?php if ( empty( $cond['pagetypes'] ) ) : ?> style="display: none"
			<?php endif; ?>>
			<label for="<?php echo esc_attr( $widget->id ); ?>-pagetypes"> <span
				class="csb-and" style="display: none"><?php _e( 'AND', 'custom-sidebars' ); ?></span>
				<?php _e( 'On these special pages', 'custom-sidebars' ); ?>
			</label> <i
				class="dashicons dashicons-trash clear-filter show-on-hover action"></i>
			<select id="<?php echo esc_attr( $widget->id ); ?>-pagetypes"
				name="<?php echo esc_attr( $block_name ); ?>[pagetypes][]"
				multiple="multiple">
		...
		<div class="csb-option-row csb-posttypes"
			<?php if ( empty( $cond['posttypes'] ) ) : ?> style="display: none"
			<?php endif; ?>>

			<label for="<?php echo esc_attr( $widget->id ); ?>-posttypes"> <span
				class="csb-and" style="display: none"><?php _e( 'AND', 'custom-sidebars' ); ?></span>
...
foreach ( $type_list as $type_item ) :
			...
			?>
				...
				<?php if ( ! $is_selected ) : ?> style="display: none"
				<?php endif; ?>>

				<label
					for="<?php echo esc_attr( $widget->id ); ?>-<?php echo esc_attr( $row_id ); ?>">
					<input type="checkbox"
					...
					data-lbl-single="<?php echo esc_attr( $lbl_single ); ?>" /> <span
					class="lbl">
							<?php echo esc_html( empty( $cond[ $row_id ] ) ? $lbl_all : $lbl_single ); ?>
						</span>
				</label>
				<div class="detail" <?php if ( empty( $cond[ $row_id ] ) ) : ?>
					style="display: none" <?php endif; ?>>
...
<?php
			foreach ( $tags as $one ) {
				printf ( '<option value="%d" selected="selected">%s</option>', esc_attr ( $one->term_id ), esc_html ( $one->name ) );
			}
			?>
</select>
		</div>
...
<?php
	...
	public function maybe_display_widget($instance) {
		...
		if (null === $Type_list) {
			...
			$Type_list = get_post_types ( array (
					'public' => true 
			), 'objects' );
		}
	...
	/**
	 * Ajax handler.
	 * If the action is processed the request is closed via die()
	 *
	 * @since 2.0.9.7
	 * @param string $action        	
	 */
	...
	protected function ajax_data_terms($term_name, $search) {
		$data = array ();
		$tags = get_terms ( $term_name, array (
				'hide_empty' => false,
				'search' => $search 
		) );
