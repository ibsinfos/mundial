...
add_action ( 'cs_init', array (
		'CustomSidebarsCheckupNotification',
		'instance' 
) );
...
class CustomSidebarsCheckupNotification extends CustomSidebars {
	...
	private function __construct() {
		...
		add_action ( 'admin_head-widgets.php', array (
				$this,
				'init_admin_head_in_widgets' 
		) );
		add_action ( 'wp_ajax_custom_sidebars_checkup_notification_dismiss', array (
				$this,
				'dismiss' 
		) );
	}
...
<div class="notice is-dismissible custom-sidebars-wp-checkup">
	...
	<form method="get" action="https://premium.wpmudev.org/wp-checkup/">
		<input type="hidden" name="external-url" value="1" /> <input
			type="text" name="the-url"
			value="<?php echo esc_url( get_option( 'home' ) ); ?>" /> <input
			type="submit"
			value="<?php esc_attr_e( 'Scan', 'custom-sidebars' ); ?>" /> <input
			type="hidden" name="utm_source" value="custom_sidebar_ad" /> <input
			type="hidden" name="utm_campaign" value="custom_sidebar_plugin" /> <input
			type="hidden" name="utm_medium" value="Custom Sidebars Plugin" />
	</form>
...
<script type="text/html"
	id="tmpl-custom-sidebars-<?php echo $template_name; ?>">
<?php
		$this->$method ();
		?>
</script>
	...
	<div class="cs-inner">
		...
		<form method="get" action="https://premium.wpmudev.org/wp-checkup/">
			<input type="hidden" name="external-url" value="1" /> <input
				...
				value="<?php echo esc_url( get_option( 'home' ) ); ?>" /><input
				type="submit"
				value="<?php esc_attr_e( 'Go', 'custom-sidebars' ); ?>" /> <input
				type="hidden" name="utm_source" value="custom_sidebar_ad" /> <input
				type="hidden" name="utm_campaign" value="custom_sidebar_plugin" /> <input
				type="hidden" name="utm_medium" value="Custom Sidebars Plugin" />
		</form>
...
<div class="custom-sidebars-box custom-sidebars-upfront">
	<div class="cs-inner">
		...
		<p>
			<a class="button" href="<?php echo esc_url( $url ); ?>"><?php esc_html_e( 'get Upfront free', 'custom-sidebars' ); ?></a>
		</p>
	</div>
</div>
<?php
