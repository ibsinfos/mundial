...
<div class="wp-filter">
		<ul class="filter-links"><?php
	foreach ( $filters as $key => $label ) {
		printf ( '<li><a href="#" class="filter %3$s" data-filter="%1$s">%2$s</a></li>', $key, $label, $current );
		$current = '';
	}
	?></ul>
	</div>
...
foreach ( $items as $item ) :
		...
		<div class="list-card <?php echo esc_attr( $item_class ); ?>">
				<div class="list-card-top">
					<span class="badge-container"> <span class="badge-active">
					...
					<div class="name">
						<h4
							class="<?php if ( $item->details ) : ?>toggle-details<?php endif; ?> is-no-detail">
		...
		?>
				...
				</span> <span class="toggle-details toggle-link is-detail">
