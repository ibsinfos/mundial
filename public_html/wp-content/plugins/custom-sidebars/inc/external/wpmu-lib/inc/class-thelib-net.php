...
class TheLib_Net extends TheLib {
	 ...
	 *        	Optional. Define URL protocol ('http', 'https')
	...
	public function current_url($protocol = null) {
		...
		if (! isset ( $Url [$protocol] )) {
			...
			if ((! $is_ssl && '80' != $_SERVER ['SERVER_PORT']) || ($is_ssl && '443' != $_SERVER ['SERVER_PORT'])) {
	 ...
	 *       
	 * @param string $url
	 *        	An URL that can be absolute or relative.
