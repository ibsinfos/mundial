...
class TheLib_Debug extends TheLib {
	...
	public function __construct() {
		...
		add_action ( 'wdev_debug_log', array (
				$this,
				'log' 
		), 10, 99 );
		
		add_action ( 'wdev_debug_log_trace', array (
				$this,
				'log_trace' 
		) );
		
		add_action ( 'wdev_debug_dump', array (
				$this,
				'dump' 
		), 10, 99 );
		
		add_action ( 'wdev_debug_trace', array (
				$this,
				'trace' 
		) );
	}
	 ...
	 * False means no debug output is made.
	 *
	 * @since 2.0.0
	 *        @api
	 *       
	 ...
	 *       
	 * @param
	 *        	mixed <dynamic> Each param will be dumped
	 */
...
<div class="wdev-debug">
	<span class="wdev-debug-label"
		onclick="toggleBlock('<?php echo esc_attr( $block_id ); ?>')">
	 ...
	 *       
	 * @param bool $output
	 *        	Optional. If false then the trace will be returned
	...
	public function trace($output = true) {
		...
		if (! $plain_text) {
			$block_id = 'wdev-debug-' . md5 ( rand () );
			$trace_str .= sprintf ( '<span class="wdev-trace-toggle" onclick="toggleBlock(\'%1$s-trace\')">
					<b>Back-Trace</b>
				</span>
		...
		for($i = 0; $i < $trace_num; $i += 1) {
			...
			if (0 === strpos ( $item ['class'], 'TheLib_' )) {
				continue;
			}
			...
			if ($plain_text) {
				...
				$trace_str .= sprintf ( "\r\n  %s. \t %s \t by %s", str_pad ( $line, 2, ' ', STR_PAD_LEFT ), $file . ': ' . str_pad ( $line_item ['line'], 5, ' ', STR_PAD_LEFT ), $item ['class'] . $item ['type'] . $item ['function'] . '(' . strip_tags ( $args ) . ')' );
			} else {
				$trace_str .= sprintf ( "<tr onclick='_m(this)'><td class='trc-num'>%s</td><td class='trc-loc'>%s</td><td class='trc-arg'>%s</td></tr>\r\n", $line, $line_item ['file'] . ': ' . $line_item ['line'], $item ['class'] . $item ['type'] . $item ['function'] . '(' . $args . ')' );
			}
		}
	 ...
	 *
	 * @param any $input
	 *        	The variable/object/value to dump.
	 * @param int $default_depth
	 *        	Deeper items will be collapsed
	 * @param int $level
		...
		if (in_array ( $type, array (
			...
			foreach ( $dump_data as $key => $value ) {
				if (! $populated) {
					...
					$this->_dump_line ( $item_key, $type_label, '', $level, $title_args );
				...
				$this->_dump_var ( $value, $key, $default_depth, $new_level, $args );
			...
			if (! $populated) {
				$this->_dump_line ( $item_key, $type_label, '', $level, $args );
			}
		} else {
			$this->_dump_line ( $item_key, $type_label, $type_data, $level, $args );
		}
	}
	...
	protected function _dump_line($key, $type, $value, $level, $args = array()) {
		...
		if (! empty ( $args ['toggle'] )) {
			$args ['containers'] = array_diff ( $args ['containers'], array (
					$args ['toggle'] 
			) );
			$args ['collapsed'] = array_diff ( $args ['collapsed'], array (
					$args ['toggle'] 
			) );
...
.wdev-debug .trc-param:last-child:after {
	...
	/**
	 * Returns an HTML element that displays a colored label.
	 * By default the
	 ...
	 *       
	 * @param string $label
	 *        	Optional. The label to display. Default is a
	 *        	random MD5 string.
	 * @param array $styles
	 *        	Optional. Array of CSS styles to apply.
	 * @return object {
	 *         Marker details
	 *        
	 *         $html
	...
	public function marker_html($label = null, $styles = array()) {
		...
		$styles = wp_parse_args ( $styles, $def_styles );
		...
		$marker = sprintf ( '<div style="%1$s">%2$s</div>', esc_attr ( $style ), $label );
