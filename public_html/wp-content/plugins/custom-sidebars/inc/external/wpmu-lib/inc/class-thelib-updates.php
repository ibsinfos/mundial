...
class TheLib_Updates extends TheLib {
	...
	/**
	 * The plugin name.
	 * Used to log data in the uploads directory.
	 * 
	 * @see write_to_file()
	 *
	 * @since 2.0.0
	 ...
	 * @param mixed $args
	 *        	Optional. All params that come after $command will be
	...
	/**
	 * Sets the plugin name (i.e.
	 * the sub-folder for the write_to_file function)
	 *
	 * @since 2.0.0
	 *        @api
	 *       
	 * @param string $name
	 *        	The plugin name, must be a valid folder name.
	 */
	 ...
	 *
	 * @param string $name
	 *        	Snapshot name, used as file-name.
	 * @param string $ext
	 *        	File extension.
	 * @param string $data
	...
	private function write_to_file($file, $ext, $data, $silent_fail = false) {
		...
		if (false !== $upload ['error']) {
			return $this->write_file_failed ( $silent_fail, 1, $upload ['error'] );
		}
		...
		if (! is_dir ( $target )) {
			return $this->write_file_failed ( $silent_fail, 2, 'Could not create sub-directory ' . $target );
		}
		...
		if (! file_exists ( $filename )) {
			return $this->write_file_failed ( $silent_fail, 3, 'Could not create file ' . $filename );
		}
	 ...
	 *       
	 * @param string $ext
	 *        	File extension to include. Empty means all files.
	 ...
	 *        	}
	 * @param bool $silent_fail
	 *        	If set to false then failure will be silent.
	...
	private function write_file_failed($silent_fail, $err_code, $error = '') {
		...
		$msg = sprintf ( '<b>Abborting update of %s!</b> ' . 'Could not create a restore-point [%s]<br />%s', ucwords ( $this->plugin ), $err_code, $error );
	...
	private function snapshot_collect($data_list) {
		...
		// Options.
		$dump->options = array ();
		if (isset ( $data_list->options ) && is_array ( $data_list->options )) {
		...
		$dump->posts = array ();
		$dump->postmeta = array ();
		if (isset ( $data_list->posts ) && is_array ( $data_list->posts )) {
	 ...
	 *        	Exact filename of the snapshot, including ext.
		...
		if (! empty ( $data ['posts'] ) && is_array ( $data ['posts'] )) {
			foreach ( $data ['posts'] as $posttype => $items ) {
				...
				foreach ( $items as $id => $post ) {
					self::$core->array->equip ( $post, 'post_author', 'post_date', 'post_date_gmt', 'post_content', 'post_title', 'post_excerpt', 'post_status', 'comment_status', 'ping_status', 'post_password', 'post_name', 'to_ping', 'pinged', 'post_modified', 'post_modified_gmt', 'post_content_filtered', 'post_parent', 'guid', 'menu_order', 'post_type', 'post_mime_type', 'comment_count' );
					$sql_idlist [] = $wpdb->prepare ( '%s', $id );
					$sql_values [] = $wpdb->prepare ( '(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)', $id, $post ['post_author'], $post ['post_date'], $post ['post_date_gmt'], $post ['post_content'], $post ['post_title'], $post ['post_excerpt'], $post ['post_status'], $post ['comment_status'], $post ['ping_status'], $post ['post_password'], $post ['post_name'], $post ['to_ping'], $post ['pinged'], $post ['post_modified'], $post ['post_modified_gmt'], $post ['post_content_filtered'], $post ['post_parent'], $post ['guid'], $post ['menu_order'], $post ['post_type'], $post ['post_mime_type'], $post ['comment_count'] );
				}
