...
class TheLib_Ui extends TheLib {
	 ...
	 *        	The module to load.
	 * @param string $onpage
	 *        	A page hook; files will only be loaded on this page.
	 */
	 ...
	 *       
	 * @param string $url
	 *        	Full URL to the javascript file.
	 * @param string $onpage
	 *        	A page hook; files will only be loaded on this page.
	 * @param int $priority
	 *        	Loading order. The higher the number, the later it is loaded.
	 */
	 ...
	 *
	 * @param string $url
	 *        	Full URL to the javascript/css file.
	 * @param string $type
	 *        	'css' or 'js'
	 * @param string $onpage
	 *        	A page hook; files will only be loaded on this page.
	...
	public function _add_js_or_css() {
		...
		foreach ( $scripts as $script ) {
			...
			if ('css' == $type) {
				...
				$handles = array_values ( array_map ( array (
						$this,
						'_get_script_handle' 
	...
	public function _enqueue_script_callback() {
		...
		foreach ( $items as $item ) {
			...
			if ('all' == $onpage || $hook == $onpage) {
				...
				if (! empty ( $url )) {
					wp_register_script ( $alias, $url, array (
							'jquery' 
					), false, true );
				}
	...
	public function _print_script_data() {
		...
		if (is_array ( $data )) {
			...
			foreach ( $collected as $var => $value ) {
				printf ( '%1$s = %2$s;', $var, json_encode ( $value ) );
			}
			echo '</script>';
		}
	}
	...
	public function _print_script_code() {
		...
		if (is_array ( $data )) {
			foreach ( $data as $item ) {
				printf ( '<script>try { %1$s } catch( err ){ window.console.log(err.message); }</script>', $item );
			}
		}
	}
	 ...
	 *        	Text to display.
	 * @param string $class
	 *        	Message-type [updated|error]
	 * @param string $screen
	 *        	Limit message to this screen-ID
	 * @param string $id
	 *        	Message ID. Prevents adding duplicate messages.
	 */
	public function admin_message($text, $class = '', $screen = '', $id = '') {
		...
		foreach ( $items as $key => $data ) {
			if ($data ['text'] == $text && $data ['class'] == $class && $data ['screen'] == $screen) {
	...
	public function _admin_notice_callback() {
		...
		foreach ( $items as $item ) {
			...
			if (empty ( $screen ) || $screen_id == $screen) {
				printf ( '<div class="%1$s notice notice-%1$s is-dismissible %3$s"><p>%2$s</p><button type="button" class="notice-dismiss"><span class="screen-reader-text">%4$s</span></button></div>', esc_attr ( $class ), $text, esc_attr ( $id ), __ ( 'Dismiss this notice.' ) );
			}
		}
	}
