...
class TheLib_Html extends TheLib {
	 ...
	 *        @api
	 *       
	 * @param array|string $pointer_id
	 *        	Internal ID of the pointer, make sure it is unique!
	 *        	Optionally this param can contain all params in array notation.
	 * @param string $html_el
	 *        	HTML element to point to (e.g. '#menu-appearance')
	 * @param string $title
	 *        	The title of the pointer.
	 * @param string $body
	 *        	Text of the pointer.
	 *        	
	 ...
	 * This decides if the pointer will be displayed.
	 *
	 * @since 1.0.2
	 * @internal
	 *
	 ...
	 *        	}
	 * @param bool $return
	 *        	Optional. If true the element is returned by
	...
	public function element($field_args, $return = false) {
		...
		if (! empty ( $ajax_data )) {
			if (! empty ( $ajax_data ['action'] ) && (empty ( $ajax_data ['_wpnonce'] ) || true === $ajax_data ['_wpnonce'])) {
		...
		switch ($type) {
			case self::INPUT_TYPE_HIDDEN :
				$this->element_hidden ( $id, $name, $value, $class );
				break;
			...
			case self::INPUT_TYPE_FILE :
				$this->element_input ( $labels, $type, $class, $id, $name, $value, $read_only . $max_attr . $attr_placeholder . $ajax_data . $data_attr, $wrapper_class );
				break;
			
			case self::INPUT_TYPE_DATEPICKER :
				$this->element_datepicker ( $labels, $class, $id, $name, $value, $max_attr . $attr_placeholder . $ajax_data . $data_attr, $wrapper_class );
				break;
			
			case self::INPUT_TYPE_TEXT_AREA :
				$this->element_textarea ( $labels, $class, $id, $name, $value, $read_only . $attr_placeholder . $ajax_data . $data_attr, $wrapper_class );
				break;
			
			case self::INPUT_TYPE_SELECT :
				$this->element_select ( $labels, $class, $id, $name, $value, $multiple . $read_only . $attr_data_placeholder . $ajax_data . $data_attr, $field_options, $wrapper_class );
				break;
			
			case self::INPUT_TYPE_RADIO :
				$this->element_radio ( $labels, $class, $id, $name, $value, $ajax_data, $field_options, $wrapper_class );
				break;
			
			case self::INPUT_TYPE_CHECKBOX :
				$this->element_checkbox ( $labels, $class, $id, $name, $value, $ajax_data . $data_attr, $field_options, $config );
				break;
			
			case self::INPUT_TYPE_WP_EDITOR :
				$this->element_wp_editor ( $labels, $id, $value, $field_options );
				break;
			...
			case self::INPUT_TYPE_SUBMIT :
				...
				$this->element_button ( $labels, $type, $class, $id, $name, $value, $button_value, $ajax_data . $data_attr );
				break;
			
			case self::INPUT_TYPE_IMAGE :
				$this->element_image ( $labels, $class, $id, $name, $value, $alt, $ajax_data . $data_attr );
				break;
			
			case self::INPUT_TYPE_RADIO_SLIDER :
				$this->element_radioslider ( $labels, $class, $id, $name, $value, $url, $read_only, $ajax_data . $data_attr, $field_options, $wrapper_class );
				break;
			
			case self::INPUT_TYPE_TAG_SELECT :
				$this->element_tagselect ( $labels, $class, $id, $name, $value, $field_options, $multiple . $read_only . $attr_data_placeholder . $data_attr, $ajax_data, $empty_text, $button_text, $title_selected, $wrapper_class );
				break;
			
			case self::INPUT_TYPE_WP_PAGES :
				$this->element_wp_pages ( $labels, $class, $id, $name, $value, $multiple . $read_only . $attr_data_placeholder . $ajax_data . $data_attr, $field_options, $wrapper_class );
				break;
			
			case self::TYPE_HTML_LINK :
				$this->element_link ( $labels, $class, $id, $value, $url, $ajax_data . $data_attr, $target );
				break;
			
			case self::TYPE_HTML_SEPARATOR :
				$this->element_separator ( ($value !== 'vertical' ? 'horizontal' : 'vertical') );
				break;
			
			case self::TYPE_HTML_TEXT :
				$this->element_wrapper ( $labels, $class, $id, $value, 'span', $wrapper_class );
				break;
			
			case self::TYPE_HTML_TABLE :
				$this->element_table ( $labels, $class, $id, $value, $field_options, $wrapper_class );
				break;
		}
	...
	private function element_hidden($id, $name, $value, $class) {
		printf ( '<input class="wpmui-field-input wpmui-hidden %4$s" type="hidden" id="%1$s" name="%2$s" value="%3$s" />', esc_attr ( $id ), esc_attr ( $name ), esc_attr ( $value ), esc_attr ( $class ) );
	}
	...
	private function element_input($labels, $type, $class, $id, $name, $value, $attr, $wrapper_class) {
		...
		printf ( '<input class="wpmui-field-input wpmui-%1$s %2$s wpmui-input-%4$s" type="%1$s" id="%3$s" name="%4$s" value="%5$s" %6$s />', esc_attr ( $type ), esc_attr ( $class ), esc_attr ( $id ), esc_attr ( $name ), esc_attr ( $value ), $attr );
	...
	private function element_datepicker($labels, $class, $id, $name, $value, $attr, $wrapper_class) {
		...
		printf ( '<span class="wpmui-field-input"><input class="wpmui-datepicker %1$s" type="text" id="%2$s" name="%3$s" value="%4$s" %5$s /><i class="wpmui-icon wpmui-fa wpmui-fa-calendar"></i></span>', esc_attr ( $class ), esc_attr ( $id ), esc_attr ( $name ), esc_attr ( $value ), $attr );
	...
	private function element_textarea($labels, $class, $id, $name, $value, $attr, $wrapper_class) {
		...
		printf ( '<textarea class="wpmui-field-input wpmui-textarea %1$s" type="text" id="%2$s" name="%3$s" %5$s>%4$s</textarea>', esc_attr ( $class ), esc_attr ( $id ), esc_attr ( $name ), esc_textarea ( $value ), $attr );
	...
	private function element_select($labels, $class, $id, $name, $value, $attr, $field_options, $wrapper_class) {
		...
		printf ( '<select id="%1$s" class="wpmui-field-input wpmui-field-select %2$s" name="%3$s" %4$s>%5$s</select>', esc_attr ( $id ), esc_attr ( $class ), esc_attr ( $name ), $attr, $options );
	...
	private function element_radio($labels, $class, $id, $name, $value, $attr, $field_options, $wrapper_class) {
		...
		foreach ( $field_options as $key => $option ) {
			...
			printf ( '<div class="wpmui-radio-input-wrapper %1$s wpmui-%2$s"><label class="wpmui-field-label" for="%4$s_%2$s"><input class="wpmui-field-input wpmui-radio %1$s" type="radio" name="%3$s" id="%4$s_%2$s" value="%2$s" %5$s /><span class="wpmui-radio-caption">%6$s</span>%7$s</label></div>', esc_attr ( $item_class ), esc_attr ( $key ), esc_attr ( $name ), esc_attr ( $id ), $item_attr . $checked, $item_text, $radio_desc );
		}
	...
	private function element_checkbox($labels, $class, $id, $name, $value, $attr, $itemlist, $options) {
		...
		if (! empty ( $itemlist )) {
			...
			printf ( '<div class="wpmui-checkbox-title">%1$s %2$s</div><div class="wpmui-checkbox-list wpmui-field-input">%3$s', $labels->title, $labels->tooltip, $item_desc );
			$item_desc = '';
			...
			foreach ( $itemlist as $key => $item ) {
				...
				foreach ( $tmp_items as $tmp_item ) {
					$item_label = sprintf ( '<div class="wpmui-checkbox-caption">%1$s</div>', $tmp_item ['label'] );
			...
			$item_label = '';
			if (empty ( $options ['checkbox_position'] ) || 'left' === $options ['checkbox_position']) {
				$item_label = sprintf ( '<div class="wpmui-checkbox-caption">%1$s %2$s</div>', $labels->title, $labels->tooltip );
			}
		...
		foreach ( $listitems as $item ) {
			...
			if (empty ( $item ['name'] )) {
				printf ( '<label class="wpmui-checkbox-wrapper wpmui-field-label wpmui-no-checkbox %1$s">%2$s %3$s</label>', esc_attr ( $item_class ), $item ['label'], $item_desc );
			} else {
				printf ( '<label class="wpmui-checkbox-wrapper wpmui-field-label %2$s"><input id="%1$s" class="wpmui-field-input wpmui-field-checkbox" type="checkbox" name="%3$s" value="%7$s" %4$s />%5$s %6$s</label>', esc_attr ( $id ), esc_attr ( $item_class ), esc_attr ( $item ['name'] ), $attr . $item ['checked'], $item ['label'], $item_desc, $item ['value'] );
			}
		}
	...
	private function element_button($labels, $type, $class, $id, $name, $label, $value, $attr) {
		...
		printf ( '<button class="wpmui-field-input button %1$s" type="%7$s" id="%2$s" name="%3$s" value="%6$s" %5$s>%4$s</button>', esc_attr ( $class ), esc_attr ( $id ), esc_attr ( $name ), $label, $attr, $value, $type );
	...
	private function element_image($labels, $class, $id, $name, $value, $alt, $attr) {
		...
		printf ( '<input type="image" class="wpmui-field-input wpmui-input-image %1$s" id="%2$s" name="%3$s" border="0" src="%4$s" alt="%5$s" %6$s/>', esc_attr ( $class ), esc_attr ( $id ), esc_attr ( $name ), esc_url ( $value ), esc_attr ( $alt ), $attr );
	...
	private function element_radioslider($labels, $class, $id, $name, $state, $url, $read_only, $attr, $options, $wrapper_class) {
		...
		$attr_input = '';
		if (! $read_only) {
			$attr_input = sprintf ( '<input class="wpmui-field-input wpmui-hidden" type="hidden" id="%1$s" name="%2$s" value="%3$s" />', esc_attr ( $id ), esc_attr ( $name ), esc_attr ( $value ) );
		}
		
		printf ( '<div class="wpmui-radio-slider %1$s wpmui-slider-%5$s %7$s" %6$s>%8$s<div class="wpmui-toggle" %2$s>%3$s</div>%4$s%9$s</div>', esc_attr ( $turned ), $attr, $link_url, $attr_input, esc_attr ( $id ), $read_only, esc_attr ( $class ), '<span class="before"></span>', '<span class="after"></span>' );
	...
	private function element_tagselect($labels, $class, $id, $name, $value, $field_options, $attr, $ajax_data, $empty_text, $button_text, $title_selected, $wrapper_class) {
		...
		if (empty ( $field_options )) {
			...
			printf ( '<div id="%1$s" class="wpmui-no-data wpmui-field-input %2$s">%3$s</div>', esc_attr ( $id ), esc_attr ( $class ), $empty_text );
		} else {
			...
			printf ( '<select id="_src_%1$s" class="wpmui-field-input wpmui-tag-source %2$s" %4$s>%5$s</select>', esc_attr ( $id ), esc_attr ( $src_class ), esc_attr ( $name ), $attr, $options_available );
			
			// Button: Add element from First Select to Second Select.
			printf ( '<button id="_src_add_%1$s" class="wpmui-field-input wpmui-tag-button button %2$s" type="button">%3$s</button>', esc_attr ( $id ), esc_attr ( $src_class ), $button_text );
			...
			printf ( '<select id="%1$s" class="wpmui-field-input wpmui-field-select wpmui-tag-data %2$s" multiple="multiple" readonly="readonly" %4$s>%5$s</select>', esc_attr ( $id ), esc_attr ( $class ), esc_attr ( $name ), $ajax_data, $options_selected );
		}
	...
	private function element_wp_pages($labels, $class, $id, $name, $value, $attr, $field_options, $wrapper_class) {
		...
		foreach ( $pages as $page_id => $page ) {
			...
			if (0 === strlen ( $page->post_title )) {
				$label = sprintf ( '#%1$s (%2$s)', $page->ID, $page->post_name );
			} else {
		...
		$this->element_select ( $labels, $class . ' wpmui-wp-pages', $id, $name, $value, $attr, $items, $wrapper_class );
	}
	...
	private function element_link($labels, $class, $id, $label, $url, $attr, $target) {
		...
		printf ( '<a id="%1$s" title="%2$s" class="wpmui-link %3$s" href="%4$s" target="%7$s" %6$s>%5$s</a>', esc_attr ( $id ), esc_attr ( strip_tags ( $title ) ), esc_attr ( $class ), esc_url ( $url ), $label, $attr, $target );
	...
	private function element_wrapper($labels, $class, $id, $code, $wrap, $wrapper_class) {
		...
		printf ( '<%1$s class="%2$s">%3$s</%1$s>', esc_attr ( $wrap ), esc_attr ( $class ), $code );
	 ...
	 *
	 * @param array $list
	 *        	List items as 'key => value' pairs.
	 * @param array|string $value
	 *        	The selected value.
	 * @param string $type
	 *        	Either 'default' or 'taglist'.
	 *        	
	 * @return string
	 */
	private function select_options($list, $value = '', $type = 'default') {
		...
		foreach ( $list as $key => $option ) {
			if (is_array ( $option )) {
				...
				$options .= sprintf ( '<optgroup label="%1$s">%2$s</optgroup>', $key, $this->select_options ( $option, $value, $type ) );
			} else {
				$attr = '';
				if (is_object ( $option )) {
				...
				switch ($type) {
					case 'default' :
						...
						$options .= sprintf ( '<option value="%1$s" %2$s>%3$s</option>', esc_attr ( $key ), $attr, $option );
						break;
	 ...
	 *
	 * @param string $type
	 *        	Wrapper type, class is set to 'wpmui-$type-wrapper'.
	 * @param string $classes
	 *        	String or array containing additional classes.
	 *        	All classes are extended with '-wrapper'.
	 * @param string $tag
	 *        	Optional. The tag name, default 'span'
	 * @param string $raw_classes
	 *        	String or array containing additional classes.
	...
	private function wrap_open($type, $classes = '', $tag = 'span', $raw_classes = '') {
		...
		printf ( '<%1$s class="wpmui-wrapper wpmui-%2$s-wrapper %3$s">', $tag, $type, $extra_classes );
	}
	...
	private function element_label($labels) {
		if (! empty ( $labels->title )) {
			printf ( '<%5$s for="%1$s" class="wpmui-field-label %4$s">%2$s %3$s</%5$s>', esc_attr ( $labels->id ), $labels->title, $labels->tooltip_code, esc_attr ( ' wpmui-label-' . $labels->id . ' ' . $labels->class ), esc_attr ( $labels->label_type ) );
		}
	...
	private function element_desc($labels) {
		if (! empty ( $labels->desc )) {
			printf ( '<label class="wpmui-field-description %2$s" for="%3$s">%1$s</label >', $labels->desc, esc_attr ( 'wpmui-description-' . $labels->id . ' ' . $labels->class ), esc_attr ( $labels->id ) );
		}
		
		if (! empty ( $labels->before )) {
			printf ( '<span class="wpmui-label-before">%s</span>', $labels->before );
		}
	}
	...
	private function element_hint($labels) {
		if (! empty ( $labels->after )) {
			printf ( '<span class="wpmui-label-after">%s</span>', $labels->after );
		}
	 ...
	 * @param bool $return
	 *        	Optional. If true then the HTML code is returned as
