...
class TheLib_Core extends TheLib {
	 ...
	 *       
	 * @param mixed $value
	 *        	A value that will be evaluated as a boolean.
	 ...
	 *       
	 * @param string $number
	 *        	A number in the base_from base.
	 * @param string $base_from
	 *        	List of characters
	 *        	E.g. 0123456789 to convert from decimal.
	 * @param string $base_to
	 *        	List of characters to use as destination base.
	...
	public function convert($number, $base_from = '0123456789', $base_to = '0123456789ABCDEF') {
		...
		if ('0123456789' == $base_to) {
			...
			for($i = 1; $i <= $number_len; $i += 1) {
				$retval = bcadd ( $retval, bcmul ( array_search ( $arr_number [$i - 1], $arr_base_from ), bcpow ( $base_from_len, $number_len - $i ) ) );
			}
		} else {
