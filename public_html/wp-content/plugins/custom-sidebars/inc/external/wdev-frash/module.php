...
if (! class_exists ( 'WDev_Frash' )) {
	class WDev_Frash {
		 ...
		 *        	WordPress plugin-ID (see: plugin_basename).
		 * @param string $title
		 *        	Plugin name for display.
		 * @param string $url_wp
		 *        	URL to the plugin on wp.org (domain not needed)
		 * @param string $cta_email
		 *        	Title of the Email CTA button.
		 * @param string $drip_plugin
		 *        	Optional. Plugin-param for the getdrip rule.
		 */
...
;
		 ...
		 *
		 * @since 1.0.0
		 * @return object|false string $type [rate|email] Which message type?
		 ...
		 *        	Plugin ID.
		 * @param string $type
		 *        	[rate|email] Message type.
		 * @param string $state
		 *        	[ok|ignore] Button clicked.
		 */
...
<div
	class="notice frash-notice frash-notice-<?php echo esc_attr( $info->type ); ?>"
	style="display: none">
	<input type="hidden" name="type"
		value="<?php echo esc_attr( $info->type ); ?>" /> <input type="hidden"
		name="plugin_id" value="<?php echo esc_attr( $info->plugin ); ?>" /> <input
		type="hidden" name="url_wp"
		value="<?php echo esc_attr( $plugin->url_wp ); ?>" /> <input
		type="hidden" name="drip_plugin"
		value="<?php echo esc_attr( $plugin->drip_plugin ); ?>" />
				<?php
			if ('email' == $info->type) {
...
<?php
		...
		protected function render_email_message($plugin) {
			...
			?>
<div class="frash-notice-logo">
	<span></span>
</div>
<div class="frash-notice-message">
					<?php
			printf ( $msg, '<strong>' . $plugin->title . '</strong>' );
			?>
				</div>
<div class="frash-notice-cta">
	<input type="email" name="email"
		value="<?php echo esc_attr( $admin_email ); ?>" />
	<button class="frash-notice-act button-primary"
		data-msg="<?php _e( 'Thanks :)', 'wdev_frash' ); ?>">
						<?php echo esc_html( $plugin->cta_email ); ?>
					</button>
	<button class="frash-notice-dismiss"
		data-msg="<?php _e( 'Saving', 'wdev_frash' ); ?>">
...
<div class="frash-notice-message">
					<?php
			printf ( $msg, '<strong>' . $user_name . '</strong>', '<strong>' . $plugin->title . '</strong>' );
			?>
				</div>
<div class="frash-notice-cta">
	<button class="frash-notice-act button-primary"
		data-msg="<?php _e( 'Thanks :)', 'wdev_frash' ); ?>">
						<?php
			printf ( __ ( 'Rate %s', 'wdev_frash' ), esc_html ( $plugin->title ) );
			?>
					</button>
	<button class="frash-notice-dismiss"
		data-msg="<?php _e( 'Saving', 'wdev_frash' ); ?>">
...
<?php
		 ...
		 * The callback function has the same
