...
class CustomSidebars {
	/**
	 * Prefix used for the sidebar-ID of custom sidebars.
	...
	/**
	 * Capability required to use *any* of the plugin features.
	 * If user does not
	...
	static public function instance() {
		...
		if (! did_action ( 'set_current_user' )) {
			add_action ( 'set_current_user', array (
					__CLASS__,
					'instance' 
			) );
			return null;
	...
	public function admin_init() {
		...
		if (in_array ( 'wpmudcs1', $dismissed_wp_pointers ) || wp_is_mobile ()) {
			...
			lib3 ()->ui->add ( 'core' );
			lib3 ()->html->pointer ( 'wpmudcs1', // Internal Pointer-ID
'#menu-appearance', // Point at
$plugin_title, sprintf ( __ ( 'Now you can create and edit custom sidebars in your ' . '<a href="%1$s">Widgets screen</a>!', 'custom-sidebars' ), admin_url ( 'widgets.php' ) ) ) // Body
;
		}
		...
		if (true === self::$accessibility_mode) {
			...
			lib3 ()->ui->admin_message ( sprintf ( __ ( '<strong>Accessibility mode is not supported by the
						%1$s plugin.</strong><br /><a href="%2$s">Click here</a>
						to disable accessibility mode and use the %1$s plugin!', 'custom-sidebars' ), $plugin_title, admin_url ( 'widgets.php?widgets-access=off&_wpnonce=' . urlencode ( $nonce ) ) ), 'err', 'widgets' );
		} else {
			/**
			...
			// AJAX actions
			add_action ( 'wp_ajax_cs-ajax', array (
					$this,
					'ajax_handler' 
			) );
		 ...
		 */
		add_filter ( 'plugin_action_links_' . plugin_basename ( CSB_PLUGIN ), array (
				$this,
				'add_action_links' 
		), 10, 4 );
	...
	/**
	 * If the specified variable is an array it will be returned.
	 * Otherwise
	 * an empty array is returned.
	 *
	 * @since 2.0
	 * @param mixed $val1
	 *        	Value that maybe is an array.
	 * @param mixed $val2
	 *        	Optional, Second value that maybe is an array.
	 * @return array
	 */
	...
	static public function get_options($key = null) {
		...
		if (null === $Options) {
			...
			$Options ['post_type_single'] = self::get_array ( $Options ['post_type_single'], // new name
$Options ['defaults'] ) // old name
;
			$Options ['post_type_archive'] = self::get_array ( $Options ['post_type_archive'], // new name
$Options ['post_type_pages'] ) // old name
;
			$Options ['category_single'] = self::get_array ( $Options ['category_single'], // new name
$Options ['category_posts'] ) // old name
;
			$Options ['category_archive'] = self::get_array ( $Options ['category_archive'], // new name
...
;
	 ...
	 * @since 1.0.4
	 * @param array $data
	 *        	This array will be validated and returned.
	 * @return array
	 */
	 ...
	 * 1.
	 * Add empty widget information for new sidebars.
	 ...
	 * @param int $post_id        	
	 * @param array $data
	 *        	When array is empty the meta data will be deleted.
	 */
	 ...
	 *
	 * @param string $type
	 *        	[all|cust|theme] What kind of sidebars to return.
	 */
	 ...
	 *        	Sidebar-ID.
	 * @param string $type
	 *        	[all|cust|theme] What kind of sidebars to check.
	 */
	 ...
	 *
	 * @since 2.0
	 * @param object|string $posttype
	 *        	The posttype to validate. Either the
	...
	static public function supported_post_type($posttype) {
		...
		if (null === $Ignored_types) {
			$Ignored_types = get_post_types ( array (
					'public' => false 
			), 'names' );
		...
		if (! isset ( $Response [$posttype] )) {
			...
			/**
			 * Filters the support-flag.
			 * The flag defines if the posttype supports
			 * custom sidebars or not.
			 *
			 * @since 2.0
			 *       
			 * @param bool $response
			 *        	Flag if the posttype is supported.
			 * @param string $posttype
			 *        	Name of the posttype that is checked.
			 */
	 ...
	 * @param string $type
	 *        	[names|objects] Defines details of return data.
	 ...
	 *        	Error message or reason; already translated.
	...
	public function add_action_links($actions, $plugin_file, $plugin_data, $context) {
		if (current_user_can ( 'edit_theme_options' )) {
			$actions ['widgets'] = sprintf ( '<a href="%s">%s</a>', esc_url ( admin_url ( 'widgets.php' ) ), __ ( 'Widgets', 'custom-sidebars' ) );
		}
		$url = 'https://wordpress.org/support/plugin/custom-sidebars';
		
		$actions ['support'] = sprintf ( '<a href="%s">%s</a>', esc_url ( $url ), __ ( 'Support', 'custom-sidebars' ) );
		return $actions;
	}
