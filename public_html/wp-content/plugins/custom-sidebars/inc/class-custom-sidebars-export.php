...
class CustomSidebarsExport extends CustomSidebars {
	...
	private function __construct() {
		if (is_admin ()) {
			add_action ( 'cs_widget_header', array (
					$this,
					'widget_header' 
			) );
	...
	/**
	 * Called by action 'cs_widget_header'.
	 * Output the export/import button in
	 * the widget header.
	 *
	 * @since 2.0
	 */
...
<?php
	 ...
	 * inside self::$import_data.
	 * The data is de-serialized.
	 * In error case the response object will be set to error status.
	 *
	 * @since 2.0
	 * @param object $req
	 *        	Initial response object for JSON response.
		...
		if (is_array ( $_FILES ['data'] )) {
			switch ($_FILES ['data'] ['error']) {
				...
				case UPLOAD_ERR_NO_FILE :
					return self::req_err ( $req, __ ( 'No file was uploaded', 'custom-sidebars' ) );
				
				case UPLOAD_ERR_INI_SIZE :
				case UPLOAD_ERR_FORM_SIZE :
					return self::req_err ( $req, __ ( 'Import file is too big', 'custom-sidebars' ) );
				
				default :
					return self::req_err ( $req, __ ( 'Something went wrong', 'custom-sidebars' ) );
			}
			...
			if (is_array ( $data ['meta'] ) && is_array ( $data ['sidebars'] ) && is_array ( $data ['options'] ) && is_array ( $data ['widgets'] ) && is_array ( $data ['categories'] )) {
				...
				return self::req_err ( $req, __ ( 'Unexpected import format', 'custom-sidebars' ) );
			}
		} else {
			return self::req_err ( $req, __ ( 'No file was uploaded', 'custom-sidebars' ) );
		}
	...
	private function prepare_import_data($req) {
		...
		if (is_array ( $data ['meta'] ) && is_array ( $data ['sidebars'] ) && is_array ( $data ['options'] ) && is_array ( $data ['widgets'] ) && is_array ( $data ['categories'] )) {
			...
			return self::req_err ( $req, __ ( 'Something unexpected happened and we could not finish ' . 'the import. Please try again.', 'custom-sidebars' ) );
		}
	 ...
	 * not relevant for the current blog.
	 * I.e. posttypes that are not registered
	...
	private function prepare_data() {
		...
		$valid_sidebars = array_merge ( array_keys ( $theme_sidebars ), array_keys ( self::$import_data ['sidebars'] ) );
		...
		foreach ( self::$import_data ['categories'] as $infos ) {
			$id = $infos ['term_id'];
			if (empty ( $valid_categories [$id] ) || $valid_categories [$id]->slug != $infos ['slug']) {
	...
	private function do_import($req) {
		...
		if ($sidebar_count > 0) {
			...
			$msg [] = sprintf ( __ ( 'Imported %d custom sidebar(s)!', 'custom-sidebars' ), $sidebar_count );
		}
		...
		if ($widget_count > 0) {
			wp_set_sidebars_widgets ( $def_sidebars );
			$msg [] = sprintf ( __ ( 'Imported %d widget(s)!', 'custom-sidebars' ), $widget_count );
		}
