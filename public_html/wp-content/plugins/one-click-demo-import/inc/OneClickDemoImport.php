...
class OneClickDemoImport {
	...
	protected function __construct() {
		...
		add_action ( 'wp_ajax_ocdi_import_demo_data', array (
				$this,
				'import_demo_data_ajax_callback' 
		) );
		add_action ( 'wp_ajax_ocdi_import_customizer_data', array (
				$this,
				'import_customizer_data_ajax_callback' 
		) );
		add_action ( 'wp_ajax_ocdi_after_import_data', array (
				$this,
				'after_all_import_data_ajax_callback' 
		) );
		add_action ( 'after_setup_theme', array (
				$this,
				'setup_plugin_with_filter_data' 
		) );
	...
	public function create_plugin_page() {
		...
		$this->plugin_page = add_submenu_page ( $plugin_page_setup ['parent_slug'], $plugin_page_setup ['page_title'], $plugin_page_setup ['menu_title'], $plugin_page_setup ['capability'], $plugin_page_setup ['menu_slug'], apply_filters ( 'pt-ocdi/plugin_page_display_callback_function', array (
				$this,
				'display_plugin_page' 
		) ) );
	}
	 ...
	 *
	 * @param string $hook
	 *        	holds info on which admin page you are currently loading.
	 */
	public function admin_enqueue_scripts($hook) {
		...
		if ($this->plugin_page === $hook) {
			...
			wp_enqueue_script ( 'ocdi-main-js', PT_OCDI_URL . 'assets/js/main.js', array (
					'jquery',
					'jquery-ui-dialog' 
			), PT_OCDI_VERSION );
	 ...
	 * 1).
	 * prepare import files (uploaded or predefined via filters)
	...
	public function import_demo_data_ajax_callback() {
		...
		/**
		 * 3).
		 * Import content (if the content XML file is set for this import).
	...
	private function final_response() {
		...
		if (empty ( $this->frontend_error_messages )) {
			...
			if (! apply_filters ( 'pt-ocdi/disable_pt_branding', false )) {
				...
				$response ['message'] .= sprintf ( __ ( '%1$s%6$sWasn\'t this a great One Click Demo Import experience?%7$s Created and maintained by %3$sProteusThemes%4$s. %2$s%5$sClick to Tweet!%4$s%8$s', 'pt-ocdi' ), '<div class="notice  notice-info"><p>', '<br>', '<strong><a href="https://www.proteusthemes.com/" target="_blank">', '</a></strong>', '<strong><a href="' . add_query_arg ( 'status', urlencode ( $twitter_status ), 'http://twitter.com/home' ) . '" target="_blank">', '<strong>', '</strong>', '</p></div>' );
			}
			
			$response ['message'] .= sprintf ( __ ( '%1$s%3$sThat\'s it, all done!%4$s%2$sThe demo import has finished. Please check your page and make sure that everything has imported correctly. If it did, you can deactivate the %3$sOne Click Demo Import%4$s plugin, because it has done its job.%5$s', 'pt-ocdi' ), '<div class="notice  notice-success"><p>', '<br>', '<strong>', '</strong>', '</p></div>' );
		} else {
	 ...
	 *        	The additional value that will be appended to the existing frontend_error_messages.
	 */
