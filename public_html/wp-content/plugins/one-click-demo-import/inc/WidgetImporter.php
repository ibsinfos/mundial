...
class WidgetImporter {
	 ...
	 *
	 * @param string $widget_import_file_path
	 *        	path to the widget import file.
	 */
	public static function import($widget_import_file_path) {
		...
		if (is_wp_error ( $results )) {
			...
			Helpers::append_to_file ( $error_message, $log_file_path, esc_html__ ( 'Importing widgets', 'pt-ocdi' ) );
		} else {
			ob_start ();
			...
			$log_added = Helpers::append_to_file ( $message, $log_file_path, esc_html__ ( 'Importing widgets', 'pt-ocdi' ) );
		}
	}
	 ...
	 *        	path to json file with WordPress widget export data.
	 */
	...
	private static function process_import_file($file) {
		// File exists?
		if (! file_exists ( $file )) {
			return new \WP_Error ( 'widget_import_file_not_found', __ ( 'Error: Widget import file could not be found.', 'pt-ocdi' ) );
		}
	...
	private static function import_data($data) {
		...
		if (empty ( $data ) || ! is_object ( $data )) {
			return new \WP_Error ( 'corrupted_widget_import_data', __ ( 'Error: Widget import data could not be read. Please try a different file.', 'pt-ocdi' ) );
		}
