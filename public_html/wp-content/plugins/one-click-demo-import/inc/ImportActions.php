...
class ImportActions {
	...
	public function register_hooks() {
		...
		add_action ( 'pt-ocdi/before_content_import_execution', array (
				$this,
				'before_content_import_action' 
		), 10, 3 );
		
		// After content import.
		add_action ( 'pt-ocdi/after_content_import_execution', array (
				$this,
		...
		// Customizer import.
		add_action ( 'pt-ocdi/customizer_import_execution', array (
				$this,
				'customizer_import' 
		), 10, 1 );
		
		// After full import action.
		add_action ( 'pt-ocdi/after_all_import_execution', array (
				$this,
				'after_import_action' 
		), 10, 3 );
		...
		if (apply_filters ( 'pt_ocdi/enable_custom_menu_widget_ids_fix', true )) {
			add_action ( 'pt-ocdi/widget_settings_array', array (
					$this,
	 ...
	 *        	Actual selected import files (content, widgets, customizer, redux).
	 * @param array $import_files
	 *        	The filtered import files defined in `pt-ocdi/import_files` filter.
	 * @param int $selected_index
	 *        	Selected index of import.
	 */
	 ...
	 *        	The data of selected import from `pt-ocdi/import_files` filter.
	 */
	private function do_import_action($action, $selected_import) {
		if (false !== has_action ( $action )) {
			...
			$log_added = Helpers::append_to_file ( $message, $log_file_path, $action );
		}
	}
}
