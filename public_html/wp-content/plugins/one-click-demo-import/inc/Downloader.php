...
class Downloader {
	 ...
	 * Constructor method.
	 *
	 * @param string $download_directory_path
	 *        	Full path to where the files will be saved.
	 */
	...
	private function get_content_from_url($url) {
		...
		if (empty ( $url )) {
			return new \WP_Error ( 'missing_url', __ ( 'Missing URL for downloading a file!', 'pt-ocdi' ) );
		}
		...
		if (is_wp_error ( $response ) || 200 !== $response ['response'] ['code']) {
			...
			return new \WP_Error ( 'download_error', sprintf ( __ ( 'An error occurred while fetching file from: %1$s%2$s%3$s!%4$sReason: %5$s - %6$s.', 'pt-ocdi' ), '<strong>', $url, '</strong>', '<br>', $response_error ['error_code'], $response_error ['error_message'] ) . '<br>' . apply_filters ( 'pt-ocdi/message_after_file_fetching_error', '' ) );
		}
	 ...
	 *        	Array or WP_Error or the response.
