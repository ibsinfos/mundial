...
class WXRImporter extends \WP_Importer {
			...
			switch ($reader->name) {
				case 'wp:wxr_version' :
					...
					if (version_compare ( $this->version, self::MAX_WXR_VERSION, '>' )) {
						$this->logger->warning ( sprintf ( __ ( 'This WXR file (version %s) is newer than the importer (version %s) and may not be supported. Please consider updating.', 'wordpress-importer' ), $this->version, self::MAX_WXR_VERSION ) );
					}
	 ...
	 *
	 * @param array $mapping
	 *        	List of map arrays (containing `old_slug`, `old_id`, `new_id`)
	 */
	 ...
	 *        	Parent node of post data (typically `item`).
	...
	protected function process_post($data, $meta, $comments, $terms) {
		 ...
		 *
		 * @param array $data
		 *        	Post data. (Return empty to skip.)
		 * @param array $meta
		 *        	Meta data.
		...
		if (is_wp_error ( $post_id )) {
			 ...
			 *        	Error object.
			 * @param array $data
			 *        	Raw data imported for the post.
			 * @param array $meta
			 *        	Raw meta data, already processed by {@see process_post_meta}.
			 * @param array $comments
			 *        	Raw comment data, already processed by {@see process_comments}.
			 * @param array $terms
			 *        	Raw term data, already processed.
			 */
					...
					if ('post_format' === $taxonomy) {
						...
						if (empty ( $term_id )) {
							...
							if (! is_wp_error ( $t )) {
								...
								$this->logger->warning ( sprintf ( esc_html__ ( 'Failed to import term: %s - %s', 'wordpress-importer' ), esc_html ( $taxonomy ), esc_html ( $term ['name'] ) ) );
								continue;
							}
						}
	 ...
	 *
	 * @param DOMElement $node
	 *        	Parent node of meta data (typically `wp:postmeta` or `wp:commentmeta`).
	 ...
	 *
	 * @param array $meta
	 *        	List of meta data arrays
	 * @param int $post_id
	 *        	Post to associate with
	 * @param array $post
	 *        	Post data
	...
	protected function process_post_meta($meta, $post_id, $post) {
		...
		foreach ( $meta as $meta_item ) {
			 ...
			 *        	Meta data. (Return empty to skip.)
			 * @param int $post_id
			 *        	Post the meta is attached to.
			 */
	 ...
	 *
	 * @param DOMElement $node
	 *        	Parent node of comment data (typically `wp:comment`).
	...
	protected function process_comments($comments, $post_id, $post, $post_exists = false) {
		...
		foreach ( $comments as $key => $comment ) {
			 ...
			 *
			 * @param array $comment
			 *        	Comment data. (Return empty to skip.)
			 * @param int $post_id
			 ...
			 *        	New comment ID
			 * @param array $comment
			 *        	Comment inserted (`comment_id` item refers to the original ID)
			 * @param int $post_id
			 *        	Post parent of the comment
			 * @param array $post
			 *        	Post data
			 */
			 ...
			 *        	New post ID.
			 * @param array $comment
			 *        	Raw data imported for the comment.
			 * @param array $meta
			 *        	Raw meta data, already processed by {@see process_post_meta}.
			 * @param array $post_id
			 *        	Parent post ID.
	 ...
	 *
	 * @param array $a
	 *        	Comment data for the first comment
	 * @param array $b
	 *        	Comment data for the second comment
	 * @return int
	 */
	...
	protected function process_author($data, $meta) {
		 ...
		 *
		 * @param array $data
		 *        	User data. (Return empty to skip.)
		 * @param array $meta
		 *        	Meta data.
		 */
		...
		if (is_wp_error ( $user_id )) {
			 ...
			 *        	Raw data imported for the user.
			 */
		...
		$this->logger->info ( sprintf ( __ ( 'Imported user "%s"', 'wordpress-importer' ), $userdata ['user_login'] ) );
		$this->logger->debug ( sprintf ( __ ( 'User %d remapped to %d', 'wordpress-importer' ), $original_id, $user_id ) );
	...
	protected function process_term($data, $meta) {
		 ...
		 *
		 * @param array $data
		 *        	Term data. (Return empty to skip.)
		 * @param array $meta
		 *        	Meta data.
		 */
		...
		$allowed = array (
				...
				'description' => true,
				'parent' => true 
		) // The parent_id may have already been set, so pass this back to the newly inserted term.
;
		...
		if (is_wp_error ( $result )) {
			$this->logger->warning ( sprintf ( __ ( 'Failed to import %s %s', 'wordpress-importer' ), $data ['taxonomy'], $data ['name'] ) );
			 ...
			 *        	Error object.
			 * @param array $data
			 *        	Raw data imported for the term.
			 * @param array $meta
			 *        	Meta data supplied for the term.
			 */
		...
		$this->logger->info ( sprintf ( __ ( 'Imported "%s" (%s)', 'wordpress-importer' ), $data ['name'], $data ['taxonomy'] ) );
		$this->logger->debug ( sprintf ( __ ( 'Term %d remapped to %d', 'wordpress-importer' ), $original_id, $term_id ) );
	...
	protected function process_term_meta($meta, $term_id, $term) {
		...
		foreach ( $meta as $meta_item ) {
			...
			if ($key) {
				...
				if (is_wp_error ( $result )) {
					$this->logger->warning ( sprintf ( __ ( 'Failed to add metakey: %s, metavalue: %s to term_id: %d', 'wordpress-importer' ), $key, $value, $term_id ) );
					do_action ( 'wxr_importer.process_failed.termmeta', $result, $meta_item, $term_id, $term );
				} else {
					$this->logger->debug ( sprintf ( __ ( 'Meta for term_id %d : %s => %s ; successfully added!', 'wordpress-importer' ), $term_id, $key, $value ) );
				}
		...
		foreach ( $todo as $post_id => $_ ) {
			...
			if (! empty ( $parent_id )) {
				...
				if (isset ( $this->mapping ['post'] [$parent_id] )) {
					...
					$this->logger->warning ( sprintf ( __ ( 'Could not find the post parent for "%s" (post #%d)', 'wordpress-importer' ), get_the_title ( $post_id ), $post_id ) );
					$this->logger->debug ( sprintf ( __ ( 'Post %d was imported with parent %d, but could not be found', 'wordpress-importer' ), $post_id, $parent_id ) );
				}
			}
			...
			if (! empty ( $author_slug )) {
				...
				if (isset ( $this->mapping ['user_slug'] [$author_slug] )) {
					...
					$this->logger->warning ( sprintf ( __ ( 'Could not find the author for "%s" (post #%d)', 'wordpress-importer' ), get_the_title ( $post_id ), $post_id ) );
					$this->logger->debug ( sprintf ( __ ( 'Post %d was imported with author "%s", but could not be found', 'wordpress-importer' ), $post_id, $author_slug ) );
				}
			}
			...
			if (empty ( $data )) {
				$this->logger->debug ( sprintf ( __ ( 'Post %d was marked for post-processing, but none was required.', 'wordpress-importer' ), $post_id ) );
				continue;
			}
			...
			if (is_wp_error ( $result )) {
				$this->logger->warning ( sprintf ( __ ( 'Could not update "%s" (post #%d) with mapped data', 'wordpress-importer' ), get_the_title ( $post_id ), $post_id ) );
				$this->logger->debug ( $result->get_error_message () );
				continue;
			}
	...
	protected function post_process_menu_item($post_id) {
		...
		if (! empty ( $menu_object )) {
			...
			$this->logger->warning ( sprintf ( __ ( 'Could not find the menu object for "%s" (post #%d)', 'wordpress-importer' ), get_the_title ( $post_id ), $post_id ) );
			$this->logger->debug ( sprintf ( __ ( 'Post %d was imported with object "%d" of type "%s", but could not be found', 'wordpress-importer' ), $post_id, $menu_object_id, $menu_item_type ) );
		}
	...
	protected function post_process_terms($terms_to_be_remapped) {
		...
		foreach ( $terms_to_be_remapped as $termid => $term_taxonomy ) {
			...
			if (empty ( $termid ) || ! is_numeric ( $termid )) {
				$this->logger->warning ( sprintf ( __ ( 'Faulty term_id provided in terms-to-be-remapped array %s', 'wordpress-importer' ), $termid ) );
				continue;
			}
			...
			if (empty ( $term_taxonomy )) {
				$this->logger->warning ( sprintf ( __ ( 'No taxonomy provided in terms-to-be-remapped array for term #%d', 'wordpress-importer' ), $term_id ) );
				continue;
			}
			...
			if (empty ( $parent_slug )) {
				$this->logger->warning ( sprintf ( __ ( 'No parent_slug identified in remapping-array for term: %d', 'wordpress-importer' ), $term_id ) );
				continue;
			}
			
			if (! isset ( $this->mapping ['term_slug'] [$parent_slug] ) || ! is_numeric ( $this->mapping ['term_slug'] [$parent_slug] )) {
				$this->logger->warning ( sprintf ( __ ( 'The term(%d)"s parent_slug (%s) is not found in the remapping-array.', 'wordpress-importer' ), $term_id, $parent_slug ) );
				continue;
			}
			...
			if (empty ( $termattributes )) {
				$this->logger->warning ( sprintf ( __ ( 'No data returned by get_term_by for term_id #%d', 'wordpress-importer' ), $term_id ) );
				continue;
			}
			...
			if (is_wp_error ( $result )) {
				$this->logger->warning ( sprintf ( __ ( 'Could not update "%s" (term #%d) with mapped data', 'wordpress-importer' ), $termattributes ['name'], $term_id ) );
				$this->logger->debug ( $result->get_error_message () );
				continue;
			}
			// Clear out our temporary meta key.
			delete_term_meta ( $term_id, '_wxr_import_parent' );
			$this->logger->debug ( sprintf ( __ ( 'Term %d was successfully updated with parent %d', 'wordpress-importer' ), $term_id, $mapped_parent ) );
		}
	}
	 ...
	 *
	 * @param array $data
	 *        	Comment data to mark as existing.
	 * @param int $comment_id
	 *        	Comment ID.
	 */
