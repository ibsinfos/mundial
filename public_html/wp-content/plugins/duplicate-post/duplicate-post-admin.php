...
function duplicate_post_plugin_upgrade() {
	...
	add_option ( 'duplicate_post_types_enabled', array (
			'post',
			'page' 
	) );
	...
	if (in_array ( 'post_format', $taxonomies_blacklist )) {
		...
		$taxonomies_blacklist = array_diff ( $taxonomies_blacklist, array (
				'post_format' 
		) );
	...
	if (in_array ( '_wp_page_template', $meta_blacklist )) {
		...
		$meta_blacklist = array_diff ( $meta_blacklist, array (
				'_wp_page_template' 
		) );
	}
...
function duplicate_post_show_update_notice() {
	...
	$message .= '<strong>' . sprintf ( wp_kses ( __ ( 'Help me develop the plugin and provide support by <a href="%s">donating even a small sum</a>.', 'duplicate-post' ), array (
			'a' => array (
					'href' => array () 
			) 
	) ), "https://duplicate-post.lopo.it/donate" ) . '</strong>';
	global $wp_version;
...
function duplicate_post_make_duplicate_link_row($actions, $post) {
	if (duplicate_post_is_current_user_allowed_to_copy () && duplicate_post_is_post_type_enabled ( $post->post_type )) {
		$actions ['clone'] = '<a href="' . duplicate_post_get_clone_post_link ( $post->ID, 'display', false ) . '" title="' . esc_attr__ ( "Clone this item", 'duplicate-post' ) . '">' . esc_html__ ( 'Clone', 'duplicate-post' ) . '</a>';
		$actions ['edit_as_new_draft'] = '<a href="' . duplicate_post_get_clone_post_link ( $post->ID ) . '" title="' . esc_attr__ ( 'Copy to a new draft', 'duplicate-post' ) . '">' . esc_html__ ( 'New Draft', 'duplicate-post' ) . '</a>';
	}
	return $actions;
}
...
function duplicate_post_copy_post_meta_info($new_id, $post) {
	...
	if (strpos ( $meta_blacklist_string, '*' ) !== false) {
		$meta_blacklist_string = str_replace ( array (
				'*' 
		), array (
				'[a-zA-Z0-9_]*' 
...
function duplicate_post_action_admin_notice() {
	if (! empty ( $_REQUEST ['cloned'] )) {
		...
		printf ( '<div id="message" class="updated fade"><p>' . _n ( '%s item copied.', '%s items copied.', $copied_posts, 'duplicate-post' ) . '</p></div>', $copied_posts );
...
function duplicate_post_add_bulk_filters_for_enabled_post_types() {
	if (get_option ( 'duplicate_post_show_bulkactions' ) != 1)
		return;
	$duplicate_post_types_enabled = get_option ( 'duplicate_post_types_enabled', array (
			'post',
			'page' 
	) );
...
function duplicate_post_action_handler($redirect_to, $doaction, $post_ids) {
	...
	foreach ( $post_ids as $post_id ) {
		...
		if (! empty ( $post )) {
			if (get_option ( 'duplicate_post_copychildren' ) != 1 || ! is_post_type_hierarchical ( $post->post_type ) || (is_post_type_hierarchical ( $post->post_type ) && ! duplicate_post_has_ancestors_marked ( $post, $post_ids ))) {
