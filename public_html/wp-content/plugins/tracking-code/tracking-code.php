...
if (! class_exists ( 'trackingCode' )) :
	class trackingCode {
		...
		function menuItem() {
			$page = add_submenu_page ( 'options-general.php', 'Tracking Code', 'Tracking Code', 'manage_options', 'tracking-code', array (
					$this,
					'init' 
			) );
		}
		...
		function form($data) {
			?>
<div class="wrap">
	<div id="icon-options-general" class="icon32 icon32-posts-page">
		<br />
	</div>
	<form method="post" action="">
		...
		<p>Add web tracking code to html head or footer section.</p>
                <?php if( !empty( $this->isSaved ) ) : ?>
                    <div class="updated">
			<p>
				<strong><?php echo "Saved Successfully."; ?></strong>
			</p>
		</div>
                <?php endif; ?>
                <p>
		...
		<textarea rows="20" style="width: 100%"
			name="data[tracking_head][code]"><?php echo @$data['tracking_head']['code'] ?></textarea>
		<br /> <input type="checkbox" name="data[tracking_head][disable]"
			id="tracking_head_disable"
			<?php checked( @$data['tracking_head']['disable'], 'on' ); ?> /> <label
			for="tracking_head_disable">Disable this head tracking code</label>
		</p>
		...
		<textarea rows="20" style="width: 100%"
			name="data[tracking_footer][code]"><?php echo @$data['tracking_footer']['code'] ?></textarea>
		<br /> <input type="checkbox" name="data[tracking_footer][disable]"
			id="tracking_footer_disable"
			<?php checked( @$data['tracking_footer']['disable'], 'on' ); ?> /> <label
			for="tracking_footer_disable">Disable this footer tracking code</label>
		</p>
