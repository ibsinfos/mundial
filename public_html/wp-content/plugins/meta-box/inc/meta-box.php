...
class RW_Meta_Box {
	...
	/**
	 * Specific hooks for meta box object.
	 * Default is 'post'.
	...
	public function enqueue() {
		...
		foreach ( $this->fields as $field ) {
			if ($field ['clone']) {
				wp_enqueue_script ( 'rwmb-clone', RWMB_JS_URL . 'clone.js', array (
						'jquery-ui-sortable' 
				), RWMB_VER, true );
				break;
			}
		...
		// Auto save.
		if ($this->autosave) {
			wp_enqueue_script ( 'rwmb-autosave', RWMB_JS_URL . 'autosave.js', array (
					'jquery' 
			), RWMB_VER, true );
		}
	 ...
	 *
	 * @param array $hidden
	 *        	Array of default hidden meta boxes.
	 * @param object $screen
	...
	public function show() {
		...
		// Container.
		printf ( '<div class="rwmb-meta-box" data-autosave="%s">', $this->autosave ? 'true' : 'false' );
	 ...
	 *        	Screen object. Optional. Use current screen object by default.
	 *        	
	 * @return bool
	 */
	 ...
	 *
	 * @param null|int $id
	 *        	Object ID. null means the current object ID.
	 */
