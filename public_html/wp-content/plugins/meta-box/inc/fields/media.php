...
class RWMB_Media_Field extends RWMB_File_Field {
	...
	public static function admin_enqueue_scripts() {
		wp_enqueue_media ();
		if (! is_admin ()) {
			wp_register_script ( 'media-grid', includes_url ( 'js/media-grid.min.js' ), array (
					'media-editor' 
			), '', true );
		}
	...
	public static function html($meta, $field) {
		...
		$html = sprintf ( '<input %s>
			<div class="rwmb-media-view" data-options="%s"></div>', self::render_attributes ( $attributes ), esc_attr ( wp_json_encode ( $field ['js_options'] ) ) );
