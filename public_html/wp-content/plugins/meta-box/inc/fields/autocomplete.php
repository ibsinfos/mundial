...
class RWMB_Autocomplete_Field extends RWMB_Multiple_Values_Field {
	...
	public static function admin_enqueue_scripts() {
		...
		wp_enqueue_script ( 'rwmb-autocomplete', RWMB_JS_URL . 'autocomplete.js', array (
	...
	public static function html($meta, $field) {
		...
		$html = sprintf ( '<input type="text" class="rwmb-autocomplete-search" size="%s">
		...
		if (is_array ( $field ['options'] )) {
			foreach ( $field ['options'] as $value => $label ) {
				...
				$html .= sprintf ( $tpl, esc_html ( $label ), esc_html__ ( 'Delete', 'meta-box' ), esc_attr ( $field ['field_name'] ), esc_attr ( $value ) );
			}
		} else {
