...
class RWMB_Image_Select_Field extends RWMB_Field {
	...
	public static function admin_enqueue_scripts() {
		...
		wp_enqueue_script ( 'rwmb-image-select', RWMB_JS_URL . 'image-select.js', array (
				'jquery' 
		), RWMB_VER, true );
	}
	...
	public static function html($meta, $field) {
		...
		foreach ( $field ['options'] as $value => $image ) {
			$html [] = sprintf ( $tpl, $image, $field ['multiple'] ? 'checkbox' : 'radio', $field ['field_name'], $value, checked ( in_array ( $value, $meta ), true, false ) );
		}
