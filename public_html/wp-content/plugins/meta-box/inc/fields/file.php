...
class RWMB_File_Field extends RWMB_Field {
	...
	public static function admin_enqueue_scripts() {
		...
		wp_enqueue_script ( 'rwmb-file', RWMB_JS_URL . 'file.js', array (
				'jquery-ui-sortable' 
		), RWMB_VER, true );
	...
	public static function add_actions() {
		...
		add_action ( 'wp_ajax_rwmb_delete_file', array (
				__CLASS__,
				'wp_ajax_delete_file' 
		) );
		add_action ( 'wp_ajax_rwmb_reorder_files', array (
				__CLASS__,
	...
	protected static function get_uploaded_files($files, $field) {
		...
		return sprintf ( '<ul class="rwmb-uploaded" data-field_id="%s" data-delete_nonce="%s" data-reorder_nonce="%s" data-force_delete="%s" data-max_file_uploads="%s" data-mime_type="%s">%s</ul>', $field ['id'], $delete_nonce, $reorder_nonce, $field ['force_delete'] ? 1 : 0, $field ['max_file_uploads'], $field ['mime_type'], implode ( '', $files ) );
	}
	...
	protected static function file_html($file) {
		...
		return sprintf ( '<li id="item_%s">
			...
			</li>', $file, wp_get_attachment_image ( $file, array (
				60,
				60 
		), true ), wp_get_attachment_url ( $file ), get_the_title ( $file ), $mime_type, get_edit_post_link ( $file ), $i18n_edit, $file, $i18n_delete );
	}
	 ...
	 *
	 * @param mixed $new
	 *        	The submitted meta value.
	 * @param mixed $old
	 *        	The existing meta value.
	 * @param int $post_id
	 *        	The post ID.
	 * @param array $field
	...
	/**
	 * Get the field value.
	 * Return meaningful info of the files.
	 *
	 * @param array $field
	 *        	Field parameters.
	 * @param array $args
	 ...
	 *        	Post ID. null for current post. Optional.
	 *        	
	 ...
	 *        	Files IDs.
	 * @param array $args
	 *        	Additional arguments (for image size).
	 * @return array
	 */
	 ...
	 *
	 * @param int $file
	 *        	Attachment file ID (post ID). Required.
	 * @param array $args
	 *        	Array of arguments (for size).
	 *        	
