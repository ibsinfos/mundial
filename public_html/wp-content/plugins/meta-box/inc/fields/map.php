...
class RWMB_Map_Field extends RWMB_Field {
	...
	public static function html($meta, $field) {
		...
		$html .= sprintf ( '<div class="rwmb-map-canvas" data-default-loc="%s" data-region="%s"></div>
		...
		if ($address) {
			$html .= sprintf ( '<button class="button rwmb-map-goto-address-button" value="%s">%s</button>', is_array ( $address ) ? implode ( ',', $address ) : $address, esc_html__ ( 'Find Address', 'meta-box' ) );
		}
	 ...
	 *        	Field parameters.
	 * @param array $args
	 *        	Additional arguments. Not used for these fields.
	 * @param int|null $post_id
	 *        	Post ID. null for current post. Optional.
	 *        	
	...
	public static function the_value($field, $args = array(), $post_id = null) {
		...
		$google_maps_url = apply_filters ( 'rwmb_google_maps_url', $google_maps_url );
		wp_register_script ( 'google-maps', esc_url_raw ( $google_maps_url ), array (), '', true );
		wp_enqueue_script ( 'rwmb-map-frontend', RWMB_JS_URL . 'map-frontend.js', array (
				'google-maps' 
		), '', true );
