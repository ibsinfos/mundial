...
if (! function_exists ( 'rwmb_meta' )) {
	...
	function rwmb_meta($key, $args = array(), $post_id = null) {
		...
		$meta = in_array ( $field ['type'], array (
				'oembed',
				'map' 
		), true ) ? rwmb_the_value ( $key, $args, $post_id, false ) : rwmb_get_value ( $key, $args, $post_id );
...
if (! function_exists ( 'rwmb_get_field_data' )) {
	 ...
	 *        	Object ID. null for current post. Optional.
	 *        	
	 * @return array
	 */
...
if (! function_exists ( 'rwmb_get_value' )) {
	 ...
	 *        	Field ID. Required.
	 * @param array $args
	 *        	Additional arguments. Rarely used. See specific fields for details.
	 * @param int|null $post_id
	 *        	Post ID. null for current post. Optional.
	 *        	
...
if (! function_exists ( 'rwmb_the_value' )) {
	 ...
	 *        	Post ID. null for current post. Optional.
	 * @param bool $echo
	 *        	Display field meta value? Default `true` which works in almost all cases. We use `false` for the [rwmb_meta] shortcode.
	 *        	
	 * @return string
	 */
...
if (! function_exists ( 'rwmb_meta_shortcode' )) {
	 ...
	 *
	 * @param array $atts
	 *        	Shortcode attributes, same as rwmb_meta() function, but has more "meta_key" parameter.
	 *        	
	 * @return string
	 */
...
if (! function_exists ( 'rwmb_get_storage_class_name' )) {
	 ...
	 *        	Object type. Use post or term.
	 * @return string
	 */
	function rwmb_get_storage_class_name($object_type) {
		$object_type = str_replace ( array (
				'-',
				'_' 
		), ' ', $object_type );
