...
class Favicon_By_RealFaviconGenerator_Api_Response {
	...
	/**
	 * For example: <code>array( "http://realfavicongenerator.net/files/1234f5d2s34f3ds2/apple-touch-icon.png", .
	 * .. )</code>
	 */
	...
	/**
	 * Indicate where the favicon files should be stored in the target web site.
	 * For example: <code>"/"</code>, <code>"/path/to/icons"</code>.
	 */
	...
	/**
	 * Return version of RealFaviconGenerator used to generate the favicon.
	 * Save this value to later check for updates.
	 */
		...
		if ($this->getPackageUrl () != NULL) {
			...
			if (! file_exists ( $extractedPath )) {
				if (mkdir ( $extractedPath, 0755, true ) !== true) {
					throw new InvalidArgumentException ( sprintf ( __ ( 'Cannot create directory %s to store the favicon package content', FBRFG_PLUGIN_SLUG ), $extractedPath ) );
				}
			}
	...
	public function downloadFilesDirectly($outputDirectory, $filesURLs) {
		foreach ( $this->getFilesURLs () as $fileURL ) {
			...
			$filePath = $outputDirectory . DIRECTORY_SEPARATOR . basename ( $parts ['path'] );
	...
	public function downloadZipFile($packageDirectory, $extractedPath, $packageUrl) {
		...
		if ($result !== true) {
			$explanation = (is_wp_error ( $result )) ? $result->get_error_message () : __ ( 'Unknown reason', Favicon_By_RealFaviconGenerator_Common::PLUGIN_SLUG );
			if (get_class ( $wp_filesystem ) != 'WP_Filesystem_Direct') {
				$explanation .= ' ' . __ ( "Apparently WordPress has no direct access to the file system (it uses another mean such as FTP). " . "This may be the root cause of this issue.", Favicon_By_RealFaviconGenerator_Common::PLUGIN_SLUG );
			}
			throw new InvalidArgumentException ( sprintf ( __ ( 'Error while unziping the favicon package %s to directory %s', Favicon_By_RealFaviconGenerator_Common::PLUGIN_SLUG ), $packagePath, $extractedPath ) . ': ' . $explanation );
		}
	...
	/**
	 * Directory where the compressed files are stored.
	...
	/**
	 * Directory where the uncompressed files are stored.
	 * Method <code>downloadAndUnpack</code> must be called first in order to populate this field.
	 * Can be <code>NULL</code>.
	 */
	...
	private function downloadFile($localPath, $url) {
		...
		if (($resp == NULL) || ($resp == false) || (is_wp_error ( $resp )) || ($resp ['response'] == NULL) || ($resp ['response'] ['code'] == NULL) || ($resp ['response'] ['code'] != 200)) {
			...
			throw new InvalidArgumentException ( sprintf ( __ ( 'Cannot download file at %s', Favicon_By_RealFaviconGenerator_Common::PLUGIN_SLUG ), $url ) . $explanation );
		}
	...
	public function append_directory_separator($filename) {
		return $filename . (($this->ends_with ( $filename, DIRECTORY_SEPARATOR )) ? '' : DIRECTORY_SEPARATOR);
	}
