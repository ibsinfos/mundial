...
require_once plugin_dir_path ( __FILE__ ) . '..' . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'class-favicon-by-realfavicongenerator-common.php';
...
class Favicon_By_RealFaviconGenerator_Admin extends Favicon_By_RealFaviconGenerator_Common {
	...
	private function __construct() {
		...
		add_filter ( 'genesis_pre_load_favicon', array (
				$this,
				'return_empty_favicon_for_genesis' 
		) );
		...
		// Check for updates
		add_action ( Favicon_By_RealFaviconGenerator_Common::ACTION_CHECK_FOR_UPDATE, array (
				$this,
				'check_for_updates' 
		) );
	}
	...
	public function register_admin_actions() {
		...
		add_action ( 'admin_menu', array (
				$this,
				'create_favicon_settings_menu' 
		) );
		
		add_action ( 'wp_ajax_' . Favicon_By_RealFaviconGenerator_Common::PLUGIN_PREFIX . '_install_new_favicon', array (
				$this,
				'install_new_favicon' 
		) );
		add_action ( 'wp_ajax_nopriv_' . Favicon_By_RealFaviconGenerator_Common::PLUGIN_PREFIX . '_install_new_favicon', array (
				$this,
				'install_new_favicon' 
		) );
	...
	public function create_favicon_settings_menu() {
		add_theme_page ( __ ( 'Favicon', Favicon_By_RealFaviconGenerator_Common::PLUGIN_SLUG ), __ ( 'Favicon', Favicon_By_RealFaviconGenerator_Common::PLUGIN_SLUG ), 'manage_options', __FILE__ . 'favicon_appearance_menu', array (
				$this,
				'create_favicon_appearance_page' 
		) );
		
		add_options_page ( __ ( 'Favicon Settings', Favicon_By_RealFaviconGenerator_Common::PLUGIN_SLUG ), __ ( 'Favicon', Favicon_By_RealFaviconGenerator_Common::PLUGIN_SLUG ), 'manage_options', __FILE__ . 'favicon_settings_menu', array (
				$this,
				'create_favicon_settings_page' 
		) );
	}
	public function create_favicon_settings_page() {
		...
		$display_update_notifications = ! $this->get_boolean_user_option ( Favicon_By_RealFaviconGenerator_Common::META_NO_UPDATE_NOTICE );
	...
	public function create_favicon_appearance_page() {
		...
		wp_enqueue_media ();
		wp_enqueue_style ( 'fbrfg_admin_style', plugins_url ( 'assets' . DIRECTORY_SEPARATOR . 'css' . DIRECTORY_SEPARATOR . 'admin.css', __FILE__ ) );
		
		// Template time!
		include_once (plugin_dir_path ( __FILE__ ) . 'views' . DIRECTORY_SEPARATOR . 'appearance.php');
	}
...
<?php
	...
	public function rewrite_pictures_url($pic_dir) {
		foreach ( scandir ( $pic_dir ) as $file ) {
			if (! is_dir ( $pic_dir . DIRECTORY_SEPARATOR . $file )) {
				add_rewrite_rule ( str_replace ( '.', '\.', $file ), trim ( $this->get_picture_path (), '/' ) . '/' . $file );
			}
		}
	}
	...
	/**
	 * Indicate if WP is installed in the root of the web site (eg.
	 * http://mysite.com) or not (eg. http://mysite.com/blog).
	 */
	...
	public function is_manual_update_notice_to_be_displayed() {
		...
		$this->log_info ( 'Compare current version (' . $this->get_favicon_current_version () . ') and latest update (' . $this->get_latest_manual_available_update () . '): ' . ($result ? 'true' : 'false') );
...
printf ( __ ( 'You might want to <a href="%s">generate your favicon again</a>', FBRFG_PLUGIN_SLUG ), admin_url ( 'themes.php?page=' . __FILE__ . 'favicon_appearance_menu' ) )?>
	</p>

	<p>
		<a
			href="<?php echo $this->add_parameter_to_current_url( Favicon_By_RealFaviconGenerator_Admin::DISMISS_UPDATE_NOTIICATION . '=0' ) ?>">
			<?php _e( 'Hide this notice', FBRFG_PLUGIN_SLUG)?>
		</a> | <a
			href="<?php echo $this->add_parameter_to_current_url( Favicon_By_RealFaviconGenerator_Admin::DISMISS_UPDATE_ALL_UPDATE_NOTIICATIONS . '=0' ) ?>">
...
<div class="update-nag">
	<p>
		<strong><?php _e( 'Your favicon was updated automatically:', FBRFG_PLUGIN_SLUG ) ?></strong>
	</p>
	...
	<p>
		<a
			href="<?php echo $this->add_parameter_to_current_url( Favicon_By_RealFaviconGenerator_Admin::DISMISS_AUTOMATIC_UPDATE_NOTIICATION . '=0' ) ?>">
			<?php _e( 'Hide this notice', FBRFG_PLUGIN_SLUG)?>
		</a> | <a
			href="<?php echo $this->add_parameter_to_current_url( Favicon_By_RealFaviconGenerator_Admin::DISMISS_UPDATE_ALL_UPDATE_NOTIICATIONS . '=0' ) ?>">
			<?php _e( 'Do not warn me again in case of update', FBRFG_PLUGIN_SLUG)?>
...
<?php
	...
	public function process_ignored_notice() {
		...
		if (isset ( $_REQUEST [Favicon_By_RealFaviconGenerator_Admin::DISMISS_UPDATE_NOTIICATION] ) && '0' == $_REQUEST [Favicon_By_RealFaviconGenerator_Admin::DISMISS_UPDATE_NOTIICATION]) {
		...
		if (isset ( $_REQUEST [Favicon_By_RealFaviconGenerator_Admin::DISMISS_AUTOMATIC_UPDATE_NOTIICATION] ) && '0' == $_REQUEST [Favicon_By_RealFaviconGenerator_Admin::DISMISS_AUTOMATIC_UPDATE_NOTIICATION]) {
			$versions = $this->get_most_recent_automatic_update ();
			$this->log_info ( 'Disable automatic update notice for ' . $versions [0] . '-' . $versions [1] );
			$this->set_boolean_user_option ( Favicon_By_RealFaviconGenerator_Common::META_NO_AUTOMATIC_UPDATE_NOTICE_FOR_VERSION . $versions [0] . '_' . $versions [1], true );
		}
		
		$no_notices = NULL;
		if ((isset ( $_REQUEST [Favicon_By_RealFaviconGenerator_Admin::DISMISS_UPDATE_ALL_UPDATE_NOTIICATIONS] ) && '0' == $_REQUEST [Favicon_By_RealFaviconGenerator_Admin::DISMISS_UPDATE_ALL_UPDATE_NOTIICATIONS])) {
			// The "no more notifications" link was clicked in the notification itself
			$no_notices = true;
		}
		if (isset ( $_REQUEST [Favicon_By_RealFaviconGenerator_Admin::SETTINGS_FORM] ) && '1' == $_REQUEST [Favicon_By_RealFaviconGenerator_Admin::SETTINGS_FORM]) {
			// The settings form was validated
			$no_notices = (! isset ( $_REQUEST [Favicon_By_RealFaviconGenerator_Admin::DISMISS_UPDATE_ALL_UPDATE_NOTIICATIONS] ) || ('0' == $_REQUEST [Favicon_By_RealFaviconGenerator_Admin::DISMISS_UPDATE_ALL_UPDATE_NOTIICATIONS]));
		}
		if ($no_notices !== NULL) {
