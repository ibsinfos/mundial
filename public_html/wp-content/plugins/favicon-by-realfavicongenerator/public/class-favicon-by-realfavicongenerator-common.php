...
class Favicon_By_RealFaviconGenerator_Common {
	...
	public function log_info($message) {
		if (((constant ( 'WP_DEBUG' ) == 'true') || (constant ( 'WP_DEBUG' ) == 1)) && ((constant ( 'Favicon_By_RealFaviconGenerator_Common::RFG_DEBUG' ) == 'true') || (constant ( 'Favicon_By_RealFaviconGenerator_Common::RFG_DEBUG' ) == 1))) {
	...
	public function set_most_recent_automatic_update($from_version, $to_version) {
		update_option ( Favicon_By_RealFaviconGenerator_Common::OPTION_MOST_RECENT_AUTOMATIC_UPDATE_FROM, $from_version );
		update_option ( Favicon_By_RealFaviconGenerator_Common::OPTION_MOST_RECENT_AUTOMATIC_UPDATE_TO, $to_version );
	}
	...
	public function get_most_recent_automatic_update() {
		...
		return (($from == NULL) || ($to == NULL)) ? NULL : array (
				$from,
				$to 
		);
	}
	public function set_latest_manual_available_update($version) {
		update_option ( Favicon_By_RealFaviconGenerator_Common::OPTION_LATEST_MANUAL_AVAILABLE_UPDATE, $version );
	}
	...
	/**
	 * Returns the data of a particular update.
	 * Format is similar to what is returned by
	 ...
	 * For example, if versions are know from 0.3 to 0.11 and $from = '0.6' and $to = '0.10',
	...
	public function get_updates_description($from, $to) {
		...
		if ($versions == NULL) {
			...
			return sprintf ( __ ( "Visit the <a href=\"%s\">RFG's change log</a> to view the content of the update" ), 'https://realfavicongenerator.net/change_log?since=' . $from );
		}
		...
		foreach ( $versions as $version ) {
			...
			array_push ( $descriptions, '<p>' . $update ['description'] . '</p>' );
		}
	...
	public function set_favicon_configured($configured, $favicon_in_root, $version, $non_interactive_api_request = NULL) {
		update_option ( Favicon_By_RealFaviconGenerator_Common::OPTION_FAVICON_CONFIGURED, $configured ? 1 : 0 );
		update_option ( Favicon_By_RealFaviconGenerator_Common::OPTION_FAVICON_IN_ROOT, $favicon_in_root ? 1 : 0 );
		...
		if ($non_interactive_api_request != NULL) {
			update_option ( Favicon_By_RealFaviconGenerator_Common::OPTION_FAVICON_NON_INTERACTIVE_API_REQUEST, $non_interactive_api_request );
		}
	}
	...
	public function set_preview_file_name($preview_file_name) {
		update_option ( Favicon_By_RealFaviconGenerator_Common::OPTION_PREVIEW_FILE_NAME, $preview_file_name );
	}
	...
	public function load_plugin_textdomain() {
		...
		load_plugin_textdomain ( $domain, FALSE, basename ( plugin_dir_path ( dirname ( __FILE__ ) ) ) . DIRECTORY_SEPARATOR . 'languages' . DIRECTORY_SEPARATOR );
	}
	...
	public function run_non_interactive_api_request($request) {
		$resp = wp_remote_post ( 'https://realfavicongenerator.net/api/favicon', array (
				'body' => $request,
				'timeout' => 45 
		) );
		if (is_wp_error ( $resp )) {
