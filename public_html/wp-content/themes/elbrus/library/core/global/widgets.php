...
class ElbrusFlickrWidget extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array (
				...
				'description' => esc_html__ ( 'Display images from flickr', 'elbrus' ) 
		);
...
<p>
	...
	<input id="<?php echo esc_attr($this->get_field_id('flickr_id')); ?>"
		type="text"
...
<p>
	...
	<input id="<?php echo esc_attr($this->get_field_id('flickr_nr')); ?>"
		type="text"
		name="<?php echo esc_attr($this->get_field_name('flickr_nr')); ?>"
		value="<?php echo esc_attr($instance['flickr_nr']); ?>"
		class="widefat" />
</p>
<?php
	}
...
class ElbrusTotalpostsWidget extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array (
				...
				'description' => esc_html__ ( 'Retrieve and display popular/latest posts/latest comments.', 'elbrus' ) 
		);
		...
		if ($myposts) :
			foreach ( $myposts as $post ) :
				...
				<h5 class="media-heading">
					<a href="<?php esc_url(the_permalink())?>"><?php echo elbrus_limit_words( get_the_title(), 9)?></a>
				</h5>
				<time datetime="<?php echo esc_attr(get_the_time('Y-m-d')); ?>">
					<i class="icon-calendar fa fa-calendar"></i><?php echo get_the_time('F d, Y'); ?></time>
			</div>
		</div>
						<?php
...
endforeach
	...
	<div class="tab-pane fade" id="pix_totalposts_com">
		...
		foreach ( $comments as $comment ) :
			...
			<a
				href="<?php echo esc_url(get_permalink($comment->ID).'#comment-'.$comment->comment_ID)?>"
				title="<?php echo esc_attr(strip_tags($comment->comment_author).' '.esc_html__('on ', 'elbrus').' '.$comment->post_title)?>"
				class="pull-left">
...
<p>
	...
	<input id="<?php echo esc_attr($this->get_field_id('post_title')); ?>"
		type="text"
		name="<?php echo esc_attr($this->get_field_name('post_title')); ?>"
		value="<?php echo esc_attr($instance['post_title']); ?>"
		class="widefat" />
</p>

<p>
	<label for="<?php echo esc_attr($this->get_field_id('post_count')); ?>"><?php esc_html_e('Number of Posts to show', 'elbrus'); ?></label>
	<input id="<?php echo esc_attr($this->get_field_id('post_count')); ?>"
		type="text"
...
<p>
	<label
		for="<?php echo esc_attr($this->get_field_id('post_category')); ?>"><?php esc_html_e('Category (Leave Blank to show from all categories)', 'elbrus'); ?></label>
	<input
		id="<?php echo esc_attr($this->get_field_id('post_category')); ?>"
		type="text"
		name="<?php echo esc_attr($this->get_field_name('post_category')); ?>"
		value="<?php echo esc_attr($instance['post_category']); ?>"
		class="widefat" />
</p>
<?php
	}
...
class ElbrusCatsWidget extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array (
				...
				'description' => esc_html__ ( 'Display Portfolio Categories', 'elbrus' ) 
		);
	...
	function widget($args, $instance) {
		...
		foreach ( $categories as $category ) {
			...
			?>
<li <?php echo wp_kses_post($class)?>><a
	href="<?php echo esc_url(get_category_link( $category->term_id )); ?>"><?php echo wp_kses_post($category->name); ?></a></li>
<?php
		}
		echo '</ul>';
		echo wp_kses_post ( $after_widget );
...
<p>
	...
	<input id="<?php echo esc_attr($this->get_field_id('cats_title')); ?>"
		type="text"
		name="<?php echo esc_attr($this->get_field_name('cats_title')); ?>"
		value="<?php echo esc_attr($instance['cats_title']); ?>"
		class="widefat" />
</p>
