...
if (! function_exists ( 'elbrus_setup' )) :
	function elbrus_setup() {
		...
		if (function_exists ( 'add_theme_support' )) {
			...
			$width = ((elbrus_get_option ( 'portfolio_settings_trumb_width', '555' )) && is_numeric ( elbrus_get_option ( 'portfolio_settings_trumb_width', '555' ) ) && elbrus_get_option ( 'portfolio_settings_trumb_width', '555' ) > 0) ? elbrus_get_option ( 'portfolio_settings_trumb_width', '555' ) : 555;
			$height = ((elbrus_get_option ( 'portfolio_settings_trumb_height', '555' )) && is_numeric ( elbrus_get_option ( 'portfolio_settings_trumb_height', '555' ) ) && elbrus_get_option ( 'portfolio_settings_trumb_height', '555' ) > 0) ? elbrus_get_option ( 'portfolio_settings_trumb_height', '555' ) : 555;
		...
		if (function_exists ( 'register_nav_menus' )) {
			register_nav_menus ( array (
					'primary_menu' => esc_html__ ( 'Primary Menu', 'elbrus' ) 
			) );
		}
...
function elbrus_import_files() {
	return array (
			array (
					'import_file_name' => esc_html__ ( 'Elbrus Theme', 'elbrus' ),
					'import_file_url' => esc_url ( 'http://envato.templines.com/import-demo/elbrus/elbrus.xml' ),
