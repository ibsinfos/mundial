					...
					<div class="controls">
						<ul>
							...
							<?php if ( $elbrus_all_works_link != '' ) : ?>
								<li><a href="<?php echo esc_url($elbrus_all_works_link); ?>"><span
									class="fa fa-th"></span></a></li>
							<?php else : ?>
								<li><a
								href="<?php echo esc_url( get_post_type_archive_link( 'portfolio' ) ); ?>"><span
									class="fa fa-th"></span></a></li>
							<?php endif; ?>
					...
					<div class="col-md-8">
						<div class="work-image">
							...
							if ($elbrus_portfolio_post_type == 'image') :
								?>
												<div
									class="single-portfolio-carousel owl-carousel enable-owl-carousel owl-theme"
									data-auto-play="4000" data-single-item="true">
									<div class="item">
								...
								if ($elbrus_portfolio_gallery) :
									...
									<a href="<?php echo esc_url($slide['url']); ?>"
										rel="prettyPhoto[pp_gal_<?php echo esc_attr($post->ID); ?>]"><img
										src="<?php echo esc_url($slide['url']); ?>"
										width="<?php echo esc_attr($slide['width']); ?>"
										height="<?php echo esc_attr($slide['height']); ?>"
										alt="<?php echo esc_attr($slide['alt']); ?>"
										title="<?php echo esc_attr($slide['title']); ?>" /></a>
								</div>
								...
								?>
												<div class="controls">
									<div class="big-view">
										<a href="<?php echo esc_url($elbrus_portfolio_link); ?>"
											rel="prettyPhoto[pp_gal_<?php echo esc_attr($post->ID); ?>]"><span
							...
							if ($elbrus_portfolio_post_type == 'video') :
								...
								if ($elbrus_portfolio_video_href != '') :
									...
									<div class="big-view">
										<a
											href="<?php echo esc_url($elbrus_portfolio_video_href.'?width='.esc_attr($elbrus_portfolio_video_width).'&amp;height='. esc_attr($elbrus_portfolio_video_height)) ?>"
											rel="prettyPhoto[pp_video_<?php echo esc_attr(get_the_id());?>]"><span
											class="icon icon-Media"></span></a>
									</div>
								</div>
								...
								<div class="col-md-12 col-sm-12 work-body-left">
									...
									<div class="rtd"><?php the_content(); ?></div>
												<?php if ( $elbrus_portfolio_button_link != '') : ?>
													<a
										href="<?php echo esc_url($elbrus_portfolio_button_link); ?>"
										class="btn btn-default" target="_blank"><?php esc_html_e( 'View project', 'elbrus' ); ?></a>
												<?php endif; ?>
											</div>
							</div>
						</div>
											...
											<div class="info pull-right text-right">
												<p class="no-margin">
																<?php if ( $elbrus_portfolio_client_link != '') : ?>
																	<a
														href="<?php echo esc_url($elbrus_portfolio_client_link); ?>"
														target="_blank">
...
<?php
	...
	$portfolio_taxterms = wp_get_object_terms ( $post->ID, 'portfolio_category', array (
			'fields' => 'ids' 
	) );
	// arguments
	$args = array (
	...
	<div class="container">
		<div class="vc_row_anchor js_stretch_anchor anchor-effect">
			<div class="wrap-anchor">
				<a class="wrap-anchor-link" href="#portfolio_related_posts"><div
						class="section-icon">
		...
		while ( $the_query->have_posts () ) :
			...
			$elbrus_portfolio_thumbnail = get_the_post_thumbnail ( get_the_id (), 'elbrus-portfolio-thumb', array (
			...
			?>
						...
						<div class="portfolio-item">
							<div class="portfolio-image">
								<a
									href="<?php echo esc_url( get_permalink( get_the_ID() ) ); ?>"><?php echo wp_kses_post($elbrus_portfolio_thumbnail); ?></a>
