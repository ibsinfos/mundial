...
$elbrus_post_format = ! in_array ( $elbrus_post_format, array (
		"quote",
		"gallery",
		"video",
		"link",
		"audio" 
) ) ? "standared" : $elbrus_post_format;
...
<!-- Blog post-->
<div id="post-<?php esc_attr( the_ID() ); ?>"
	<?php post_class( 'wrap-blog-post' . esc_attr($elbrus_blog_css_animation) ); ?>>
	...
	<div class="wrap-post-description">
		<?php if ( elbrus_get_option( 'blog_show_author', 'on' ) == 'on' ) : ?>
			<?php if ( get_the_author_meta( 'user_url' ) != '' ) : ?>
				<a class="post-avatar"
			href="<?php esc_url( the_author_meta( 'user_url' ) ); ?>"
			target="_blank"> <img src="<?php echo esc_url($elbrus_src); ?>"
			alt="<?php esc_attr( the_author_meta( 'display_name' ) ); ?>">
		</a>
			<?php else : ?>
				<a class="post-avatar"
			href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ), get_the_author_meta( 'user_nicename' ) ) ); ?>">
			<img src="<?php echo esc_url($elbrus_src); ?>"
			alt="<?php esc_attr( the_author_meta( 'display_name' ) ); ?>">
		</a>
			<?php endif; ?>
		<?php endif; ?>
		...
		<div class="meta <?php echo esc_attr($elbrus_add_meta_class); ?>">
			...
			<div class="meta-item">
				<span class="icon icon-Tag"></span><?php elbrus_post_terms( array( 'taxonomy' => 'post_tag' ) ); ?></div>
			<?php endif; ?>
			<?php if ( elbrus_get_option( 'blog_show_date', 'on' ) == 'on' ) : ?>
			<div class="meta-item">
				<span class="icon icon-Agenda"></span><a
					href="<?php esc_url( the_permalink() ); ?>"><?php echo get_the_date(); ?></a>
			</div>
			<?php endif; ?>
			<?php if ( comments_open() ) : ?>
			<div class="meta-item">
				<span class="icon icon-Message"></span><?php comments_popup_link( esc_html__( 'Post a Comment', 'elbrus' ), esc_html__( '1 comment', 'elbrus' ), esc_html__( '% comments', 'elbrus' ), "comments-link"); ?></div>
			<?php endif; ?>
		</div>
	</div>
		...
		<div class="row">
			<div class="col-md-12 clearfix">
				<a href="<?php esc_url( the_permalink() ); ?>"
					...
					title="<?php esc_attr( the_title() ); ?>"><?php echo esc_attr( $elbrus_text_readmore ); ?></a>
			</div>
		</div>
	</div>
	<?php endif; ?>
